...
\section{Deterministic Putinar, SparsePutinar, SparseFullBox, And FullBox Selection}
...
For this one-dimensional degree-two scalar residual, order one uses two \term{psd} Gram blocks followed by three exact coefficient identities. Order two matches a degree-four target and stores two \term{psd} blocks followed by five identities. The original \code{direct} object remains unchanged, and the selection made from \code{polya} replaces P\'olya by rebuilding the FullBox constraints from the original residual. Likewise, selecting Putinar from \code{boxHigher} rebuilds from the original residual and clears the full-box selection.
...
\begin{tikzpicture}[>=Latex,node distance=7mm and 10mm,
  ...
  \node[axes,below=of e2] (a2) {
    physical cells: $c\in\{1\}$ \hfill (1)\\
    ordinary row (one stored row) \hfill (1)\\
...
The numbers (6) and (2) are certificate-assembly counts that state how many finite semidefinite constraints represent \code{E1} and \code{E2}. Solver success requires the separate \code{sol.problem} check, and the finite objective is checked after the shared \code{optimize} call.
