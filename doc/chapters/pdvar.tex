...
\hyperref[sec:pdvar-table]{\code{bernTable}},
...
\code{pdvar} exposes the complete read-only inherited \code{pdbase} property set listed in \cref{sec:pdbase-fields}. Their private set access rejects user assignment. In particular, decision coefficients are inspected through \code{P.LocalValues}, while \code{P.Degree}, \code{P.IsContinuous}, and \code{P.RateBounds} describe their representation and rate metadata.
...
Use public algebra to create a modified value. Direct assignments such as \code{P.Degree = 1} raise MATLAB's private-set property error.
...
\descriptionhead
The storage queries use the shared \code{pdbase} cell and label order and return the symbolic \code{sdpvar} coefficient payloads of the selected decision expression. \code{evaluate} reconstructs the affine symbolic matrix at one in-bounds parameter point, while a derivative or rate-row object returns a $1$-by-$N_v$ cell row in stored distinct-vertex order, where $N_v=\code{P.NumRateRows}$. \code{elevate} returns a new \code{pdvar} of the same dynamic class with the same represented expression, physical cells, YALMIP variables, metadata, rate-row order, and decision freedom at the higher degree. The original \code{P} remains unchanged. The optional positional validation mode and its errors follow the canonical contract in \cref{sec:pdbase-elevate}. \Cref{sec:bernstein-degree-elevation} derives the sparse elevation map used for this exact representation change.

\examplehead
The one-line examples in this chapter print the \code{Expression} column returned by \code{bernTable(...,}\allowbreak\code{"oneLine")} so long symbolic formulas can wrap while preserving their terms. YALMIP's \code{internal(...)} identifiers are local to the clean MATLAB session and may differ in another session.
...
elevatedCoefficientTableSize = 1×2 double
     1     4
    "(1-α)^3*[internal(1), internal(3)] + 3(1-α)^2α*[0.666666666667*internal(1)+0.333333333333*internal(5), 0.666666666667*internal(3)+0.333333333333*internal(7)] + 3(1-α)α^2*[0.333333333333*internal(1)+0.666666666667*internal(5), 0.333333333333*internal(3)+0.666666666667*internal(7)] + α^3*[internal(5), internal(7)]"
    "(1-α)^3*[internal(2), internal(4)] + 3(1-α)^2α*[0.666666666667*internal(2)+0.333333333333*internal(6), 0.666666666667*internal(4)+0.333333333333*internal(8)] + 3(1-α)α^2*[0.333333333333*internal(2)+0.666666666667*internal(6), 0.333333333333*internal(4)+0.666666666667*internal(8)] + α^3*[internal(6), internal(8)]"
\end{lstlisting}
...
elevatedDegree = 1×2 double
     2     2
(1-α1)^2 * (1-α2)^2*[internal(1)] + (1-α1)^2 * 2(1-α2)α2*[internal(1)] + (1-α1)^2 * α2^2*[internal(1)] + 2(1-α1)α1 * (1-α2)^2*[0.5*internal(1)+0.5*internal(2)] + 2(1-α1)α1 * 2(1-α2)α2*[0.5*internal(1)+0.5*internal(2)] + 2(1-α1)α1 * α2^2*[0.5*internal(1)+0.5*internal(2)] + α1^2 * (1-α2)^2*[internal(2)] + α1^2 * 2(1-α2)α2*[internal(2)] + α1^2 * α2^2*[internal(2)]
\end{lstlisting}
...
\textit{Differentiation changes local coefficients. Rate substitution creates rows. Both operations preserve the physical-cell count.}
\end{center}
...
\section{\texorpdfstring{\code{bernTable}}{bernTable}}
