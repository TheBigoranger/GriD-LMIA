...
\ideastep{Basic idea} Let \(\mathcal H_c=[\rho_1^{(c)},\rho_1^{(c+1)}]\), with width \(h_1^{(c)}=\rho_1^{(c+1)}-\rho_1^{(c)}>0\). Its forward local coordinate is
\[
  \alpha=\frac{\rho_1-\rho_1^{(c)}}{h_1^{(c)}}\in[0,1].
\]
...
\[
  ...
  \qquad
  \alpha_s=\frac{\rho_s-\rho_s^{(c_s)}}{h_s^{(\mathbf c)}},
  \qquad s=1,\ldots,\ell.
\]
...
\ideastep{Formula} For one parameter, on physical cell \(\mathbf c\) of width \(h_1^{(\mathbf c)}\),
\[
  \frac{\partial P}{\partial\rho}
=\frac{m}{h_1^{(\mathbf c)}}\sum_{i=0}^{m-1}
    \bigl(C^{(\mathbf c)}[i+1]-C^{(\mathbf c)}[i]\bigr)B_i^{m-1}(\alpha).
\]
