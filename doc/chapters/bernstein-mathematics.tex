...
\[
X=\sum_{i=0}^{m}w_iX_i,\qquad w_i\ge0,\qquad \sum_{i=0}^{m}w_i=1.
\]
...
\[
  u^{\mathsf T}Xu=\sum_iw_i\,u^{\mathsf T}X_i u\ge0
...
\ideastep{Basic idea} Higher degree adds interior shape controls while keeping the same physical cell.  It does not add physical grid nodes.

\ideastep{Formula} For degree \(m\), define
\[
...
Thus
\[
  C^{(c)}(\alpha)=\sum_{i=0}^{m}B_i^m(\alpha)C^{(c)}[i]
\]
...
\[
  U_m^{(c)}[i]=\binom mi C^{(c)}[i],
  \qquad
  E_{M-m}[j]=\binom{M-m}{j},
  \qquad
  \widehat U_M^{(c)}[k]=\binom Mk\widehat C^{(c)}[k].
\]
Then
\[
  \widehat U_M^{(c)}=U_m^{(c)}*E_{M-m},
  \qquad
  \widehat C^{(c)}[k]
  =\frac{(U_m^{(c)}*E_{M-m})[k]}{\binom Mk},
\]
where \(*\) is ordinary one-dimensional discrete convolution and out-of-range entries are zero. Thus elevation first binomially scales the coefficient row, convolves it with the fixed binomial kernel of degree \(M-m\), and divides by the degree-\(M\) binomial weight. The new coefficients are fixed convex combinations of the old ones. Elevation therefore changes representation but adds no decision freedom. Constructing a new higher-degree \code{pdvar} does enlarge the decision parameterization.

For a tensor coefficient array of degree $\mathbf m=(m_1,\ldots,m_\ell)$, elevate independently to any componentwise target $\mathbf M\ge\mathbf m$. With
\[
  U_{\mathbf m}^{(\mathbf c)}[\mathbf i]
  =\left(\prod_{s=1}^{\ell}\binom {m_s}{i_s}\right)
  C^{(\mathbf c)}[\mathbf i],
  \qquad
  E_{\mathbf M-\mathbf m}[\mathbf j]
...
\[
  ...
  =
  \bigl(U_{\mathbf m}^{(\mathbf c)}*_{\ell}E_{\mathbf M-\mathbf m}\bigr)[\mathbf k],
\]
where $*_{\ell}$ denotes $\ell$-dimensional discrete convolution. The same identity defines a sparse linear operator without materializing zeros. Enumerate source labels $\mathbf i$ and target labels $\mathbf k$ in repository order, and define $E_{\mathbf M\leftarrow\mathbf m}\in\mathbb R^{N_{\mathbf M}\times N_{\mathbf m}}$ by
\[
\left[E_{\mathbf M\leftarrow\mathbf m}\right]_{\mathbf k,\mathbf i}=
\begin{cases}
...
\end{cases}
\]
Here $N_{\mathbf d}=\prod_s(d_s+1)$. Each source label contributes to the target labels in the translated gap box $\mathbf i+\prod_s\{0,\ldots,M_s-m_s\}$, so the operator has exactly $N_{\mathbf m}\prod_s(M_s-m_s+1)$ positive entries. All remaining entries are zero.

If each coefficient is an $a$-by-$b$ matrix, pack one cell and rate row horizontally as $C_{\mathrm{pack}}=[C[\mathbf i_1]\ \cdots\ C[\mathbf i_{N_{\mathbf m}}]]$. The elevated payload is then
\[
\widehat C_{\mathrm{pack}}=C_{\mathrm{pack}}\left(E_{\mathbf M\leftarrow\mathbf m}^{\mathsf T}\otimes I_b\right).
\]
...
For example, elevation from $m=1$ to $M=3$ gives
\[
E_{3\leftarrow1}=
\begin{bmatrix}
1&0\\
...
\]
The counts \(k_s\) may differ. Thus \(\mathcal G=\mathcal G_1\times\cdots\times\mathcal G_\ell\) has \(\prod_s k_s\) physical nodes. A physical hypercube is identified separately by the one-based multi-index \(\mathbf c=(c_1,\ldots,c_\ell)\), with \(\mathcal H_{\mathbf c}=\prod_s[\rho_s^{(c_s)},\rho_s^{(c_s+1)}]\). Define its directional width and forward affine map by
\[
\begin{aligned}
  h_s^{\mathbf c}
  ...
  \boldsymbol\alpha
    &=\phi_{\mathbf c}^{-1}(\boldsymbol\rho),
    \qquad s=1,\ldots,\ell.
\end{aligned}
\]
For any cellwise quantity \(C\), write \(C^{(\mathbf c)}=C\circ\phi_{\mathbf c}\) for its pullback to the unit box. For direction-wise degree \(\mathbf m=(m_1,\ldots,m_\ell)\),
\[
C^{(\mathbf c)}(\boldsymbol\alpha)=
  \sum_{\mathbf i\in\prod_s\{0,\ldots,m_s\}}
B_{\mathbf i}^{\mathbf m}(\boldsymbol\alpha)
  C^{(\mathbf c)}[\mathbf i],
  \qquad
B_{\mathbf i}^{\mathbf m}=
  \prod_{s=1}^{\ell}B_{i_s}^{m_s}(\alpha_s).
\]
There are \(\prod_s(k_s-1)\) physical hypercubes and \(\prod_s(m_s+1)\) zero-based local Bernstein labels per hypercube. A zero component \(m_s=0\) leaves only label \(i_s=0\), so the polynomial is constant in direction \(s\). Physical node and hypercube indices remain one-based. The familiar \((m+1)^\ell\) count is the uniform-degree specialization \(\mathbf m=m\boldsymbol1\).
...
\ideastep{Formula} First consider one parameter and fix the one-based physical cell \(c\).  Let
\[
P=\sum_{i=0}^{m}B_i^mP^{(c)}[i],\qquad Q=\sum_{j=0}^{n}B_j^nQ^{(c)}[j].
\]
...
\[
  \widetilde P^{(c)}[i]=\binom miP^{(c)}[i],
  \qquad
  \widetilde Q^{(c)}[j]=\binom njQ^{(c)}[j],
  \qquad
  \widetilde R^{(c)}[k]=\binom{m+n}{k}(PQ)^{(c)}[k].
\]
Then
\[
  \widetilde R^{(c)}
...
\]
The convolution remains ordered for matrix payloads: each summand is \(\widetilde P^{(c)}[i]\widetilde Q^{(c)}[j]\), never the reversed product.
...
\textit{Direct formula labels sit outside the plot regions.  Solid/circle, dashed/square, and dotted/triangle treatments distinguish the three functions without relying on color.}
\end{center}
...
\textit{Only the panel-to-panel arrows encode flow.  Contributions are listed inside their anti-diagonal group, so no edges cross labels or equations.}
\end{center}

\needspace{14\baselineskip} For tensor degrees $\mathbf m$ and $\mathbf n$, labels add componentwise and the product has degree $\mathbf m+\mathbf n$:
\[
(PQ)^{(\mathbf c)}[\mathbf k]=
  \sum_{\mathbf i+\mathbf j=\mathbf k}
P^{(\mathbf c)}[\mathbf i] Q^{(\mathbf c)}[\mathbf j]
  \prod_{s=1}^{\ell}
...
\[
  ...
  P^{(\mathbf c)}[\mathbf i]\right\}_{\mathbf i}
  *_{\ell}
  \left\{\left(\prod_{s=1}^{\ell}\binom {n_s}{j_s}\right)
  Q^{(\mathbf c)}[\mathbf j]\right\}_{\mathbf j}
  \right)[\mathbf k].
\]
...
R[\mathbf k]=
\begin{bmatrix}
\omega_1 A[\mathbf i_1]&\cdots&\omega_t A[\mathbf i_t]
\end{bmatrix}
\begin{bmatrix}
X[\mathbf j_1]\\ \vdots\\ X[\mathbf j_t]
\end{bmatrix},
\qquad t=|\mathcal P_{\mathbf k}|.
\]
...
\]
the contraction multiplies one stacked affine vector of matrix blocks by the fixed numeric block row $\tfrac12[A[1]\ A[0]]$. It creates the same affine YALMIP expression as the two-term sum and introduces no new decision variable.
...
\[
R[\mathbf k]=\sum_{r=1}^{|\mathcal P_{\mathbf k}|}\omega_r P[\mathbf i_r]Q[\mathbf j_r]
\]
...
The protected \code{prodVals} kernel owns the three routes and reuses their numeric plans across the complete cell-local trees. All routes preserve the same product polynomial and output degree. Numeric tensor shapes, pair tables, and weights may be reused because they contain no YALMIP variables. The input coefficient payloads and their existing decision variables remain attached to their own physical cells and rate rows.

\needspace{14\baselineskip}
\section{Differentiation And Rate Vertices}
\label{sec:bernstein-differentiation}

\ideastep{Basic idea} Differentiate inside each physical cell.  Adjacent control differences produce the derivative coefficients; multiplying by a bounded rate then creates one stored row per rate-box vertex.  Value continuity of \(P\) does not force the cell-local derivatives to match across a face.

\ideastep{Formula} For one parameter, on physical cell \(c\) of width \(h_1^c\),
\[
  \frac{\partial P}{\partial\rho}
=\frac{m}{h_1^c}\sum_{i=0}^{m-1}
    \bigl(C^{(c)}[i+1]-C^{(c)}[i]\bigr)B_i^{m-1}(\alpha).
\]
\ideastep{Worked example} For the worked polynomial \([1,2,6]\) on \([0,2]\), \(m=2\) and \(h=2\), so the derivative coefficients are
\[
  \left[\frac{2}{2}(2-1),\frac{2}{2}(6-2)\right]=[1,4].
\]
This represents \(dp/d\rho=1+3\alpha\).

In \(\ell\) dimensions with tensor degree \(\mathbf m\), differentiation with respect to \(\rho_s\) replaces each coefficient by
\[
  \frac{m_s}{h_s^{\mathbf c}}
  \left(C^{(\mathbf c)}[\mathbf i+\mathbf e_s]
       -C^{(\mathbf c)}[\mathbf i]\right),
\]
when \(m_s>0\), reduces only that component to \(m_s-1\), and leaves the other label components unchanged. A zero-degree axis has identically zero partial derivative. For two or more parameters, the implementation elevates every nonzero partial exactly from \(\mathbf m-\mathbf e_s\) back to the common tensor degree \(\mathbf m\) before adding them; in one parameter it retains the established reduced-degree result. If every component is zero, \code{rhodiff} stores a complete zero coefficient row for every rate vertex.

Finally,
\[
  \dot P=\sum_{s=1}^{\ell}
  \nu_s\frac{\partial P}{\partial\rho_s},
...
\end{array}
\]
The last parameter direction changes fastest.  This is the same tensor ordering used by the package's coefficient and cell traversal, so \code{pdlmi} can constrain every rate row without reconstructing the rate box.
