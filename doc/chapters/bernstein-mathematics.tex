...
For a tensor coefficient array, apply the same rule in every direction. With
\[
  U_m^{(\mathbf c)}[\mathbf i]
  =\left(\prod_{s=1}^{\ell}\binom m{i_s}\right)
  C^{(\mathbf c)}[\mathbf i],
  \qquad
  E_{M-m}[\mathbf j]
  =\prod_{s=1}^{\ell}\binom{M-m}{j_s},
\]
the elevated array satisfies
\[
  \left(\prod_{s=1}^{\ell}\binom M{k_s}\right)
  \widehat C^{(\mathbf c)}[\mathbf k]
  =
  \bigl(U_m^{(\mathbf c)}*_{\ell}E_{M-m}\bigr)[\mathbf k],
\]
where \(*_{\ell}\) denotes \(\ell\)-dimensional discrete convolution. This tensor convolution is deterministic and preserves the represented polynomial exactly.
...
\end{aligned}
\]
For any cellwise quantity \(C\), write \(C^{(\mathbf c)}=C\circ\phi_{\mathbf c}\) for its pullback to the unit box. For the package's scalar degree \(m\),
\[
...
\needspace{14\baselineskip} For tensor labels,
\[
...
\[
  \left(\prod_{s=1}^{\ell}\binom{m+n}{k_s}\right)
  (PQ)^{(\mathbf c)}[\mathbf k]
  =
  \left(
  \left\{\left(\prod_{s=1}^{\ell}\binom m{i_s}\right)
  P^{(\mathbf c)}[\mathbf i]\right\}_{\mathbf i}
  *_{\ell}
  \left\{\left(\prod_{s=1}^{\ell}\binom n{j_s}\right)
...
In \(\ell\) dimensions, differentiation with respect to \(\rho_s\) replaces each coefficient by
\[
  \frac{m}{h_s^{\mathbf c}}
