...
For example, take $X_0=1$, $X_1=4$, and weights $0.75$ and $0.25$. Their combination is $1.75$, which remains between the two inputs.
...
Let $\mathcal H_c=[\rho_1^{(c)},\rho_1^{(c+1)}]$, with width $h_1^{c}=\rho_1^{(c+1)}-\rho_1^{(c)}>0$. Its forward map and inverse coordinate are
\begin{gather*}
...
For example, $C^{(c)}[0]=2$ and $C^{(c)}[1]=4$ give $C^{(c)}(0.25)=0.75(2)+0.25(4)=2.5$, exactly as in \cref{sec:pdmat-evaluate}.
\begin{center}
...
Higher degree adds interior shape controls while keeping the same physical cell and the same physical grid nodes.

In one parameter, write the degree vector as $\vect{m}=(m_1)$ and define
\begin{gather*}
...
\end{gather*}
There are $\prod_{s=1}^{\ell}(k_s-1)$ physical hypercubes and $\prod_{s=1}^{\ell}(m_s+1)$ local Bernstein labels per hypercube, with $i_s=0,\ldots,m_s$. A zero component $m_s=0$ leaves only label $i_s=0$, so the polynomial is constant in direction $s$. Physical node and hypercube indices remain one-based. If every component is the common scalar $\bar m$, then $\vect{m}=\bar m\vect{1}$ and the label count specializes to $(\bar m+1)^\ell$.
...
Multiplying two Bernstein polynomials creates every ordered pair of input coefficients. Pairs with the same label sum contribute to the same output. For matrix payloads, ``ordered'' matters because the left factor stays on the left.

First consider one parameter, write the two degree vectors as $\vect{m}=(m_1)$ and $\vect{n}=(n_1)$, and fix the one-based physical cell $c$. Let
\begin{gather*}
...
For example, take quadratic coefficients $P=[1,2,6]$ and affine coefficients $Q=[2,4]$. The anti-diagonals give
\begin{gather*}
\begin{aligned}
  R[0]&=1(2)=2,\\
...
The following coefficient-flow diagram summarizes the same calculation with panel-to-panel arrows that keep its labels clear.
...
For tensor degrees $\vect{m}$ and $\vect{n}$, labels add componentwise and the product has degree $\vect{m}+\vect{n}$:
\begin{gather*}
(PQ)^{(\vect{c})}[\vect{k}]=
...
Differentiate inside each physical cell. Adjacent control differences produce the derivative coefficients. Multiplying by a bounded rate then creates one stored row per rate-box vertex. Value continuity of $P$ permits distinct cell-wise derivatives across a shared face.

For one parameter, write $\vect{m}=(m_1)$ and consider physical cell $c$ of width $h_1^c$:
\begin{gather*}
  \frac{\partial P}{\partial\rho}
=\frac{m_1}{h_1^c}\sum_{i=0}^{m_1-1}
    \bigl(C^{(c)}[i+1]-C^{(c)}[i]\bigr)B_i^{m_1-1}(\alpha).
\end{gather*}
For the worked polynomial $[1,2,6]$ on $[0,2]$, $\vect{m}=(2)$ and $h_1^c=2$, so the derivative coefficients are
\begin{gather*}
...
The two-parameter flow below shows how the partial derivatives generate one stored expression for each rate vertex.
