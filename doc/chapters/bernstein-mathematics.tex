...
\ideastep{Basic idea} Let a physical cell be \([\rho_1^{(c)},\rho_1^{(c+1)}]\), with width \(h_c=\rho_1^{(c+1)}-\rho_1^{(c)}>0\).  Its forward local coordinate is
\[
...
\ideastep{Worked example} For example, \(C^{(c)}[0]=2\) and \(C^{(c)}[1]=4\) give \(M_c(0.25)=0.75(2)+0.25(4)=2.5\), exactly as in \cref{sec:pdmat-evaluate}.
...
\]
The new coefficients are fixed convex combinations of the old ones.  Elevation therefore changes representation but adds no decision freedom.  Constructing a new higher-degree \code{pdvar} does enlarge the decision parameterization.
...
\]
The counts \(k_s\) may differ.  Thus \(\mathcal G=\mathcal G_1\times\cdots\times\mathcal G_\ell\) has \(\prod_s k_s\) physical nodes.  A physical hypercube is identified separately by the one-based multi-index \(\mathbf c=(c_1,\ldots,c_\ell)\).  On that hypercube,
\[
  \alpha_s=
...
\ideastep{Formula} For one parameter, on physical cell \(c\) of width \(h_c\),
\[
...
Let \(\code{RateBounds}(s,:)=[\underline{\dot\rho}_s\ \overline{\dot\rho}_s]\).  The rate box has the finite vertex set
\[
  \mathcal V=\prod_{s=1}^{\ell}\{\underline{\dot\rho}_s,\overline{\dot\rho}_s\},
  \qquad |\mathcal V|=2^\ell.
\]
For a derivative object \(D=\code{rhodiff}(P,\code{RateBounds})\) and one physical hypercube \(c=(c_1,\ldots,c_\ell)\), the leaf \(\code{D.LocalValues}\{c_1\}\cdots\{c_\ell\}\) is a \(2^\ell\)-by-\((\code{D.Degree}+1)^\ell\) cell array.  Its rows enumerate \(\mathcal V\); its columns enumerate the local Bernstein labels in \code{combRows} order.  Each entry already contains the rate-weighted sum \(\sum_s v_s\,\partial P^{(c)}/\partial\rho_s\) at that row's vertex \(v\).  The rows add rate alternatives inside one existing hypercube; they do not add physical cells.

For \(\ell=2\), the lower/upper tensor order is
\[
\begin{array}{c|cc}
\text{row} & \dot\rho_1 & \dot\rho_2\\
\hline
1 & \underline{\dot\rho}_1 & \underline{\dot\rho}_2\\
2 & \underline{\dot\rho}_1 & \overline{\dot\rho}_2\\
3 & \overline{\dot\rho}_1 & \underline{\dot\rho}_2\\
4 & \overline{\dot\rho}_1 & \overline{\dot\rho}_2
\end{array}
\]
