...
\]
where \(*_{\ell}\) denotes \(\ell\)-dimensional discrete convolution. This tensor convolution is deterministic and preserves the represented polynomial exactly. Public \code{elevate} and \code{elevVals} accept a scalar uniform shorthand or a direction-wise increment vector.
...
This is the algorithm behind protected \code{bernProd} (\cref{sec:pdbase-bernprod}) and public \code{pdmat}/\code{pdvar} multiplication (\cref{sec:pdmat-operator-entries,sec:pdvar-algebra}).
...
\[
  ...
  *_{\ell}
  ...
  \right)[\mathbf k].
\]
Labels add componentwise; physical-cell identity does not change, and every matrix product retains the written left-to-right factor order.
...
\]
This matrix expression is affine in \(\boldsymbol\nu\). Every value inside the rate box \(\mathcal R\) is therefore a convex combination of the \(2^\ell\) vertex matrices in \(\mathcal V_{\mathcal R}\). Because the positive- and negative-semidefinite cones are convex, the requested semidefinite sign holds on all of \(\mathcal R\) whenever it holds at every vertex. \code{rhodiff} stores one row for each vertex.
...
\[
  ...
  \qquad |\mathcal V_{\mathcal R}|=2^\ell.
\]
For a derivative object \(D=\code{rhodiff}(P,\code{RateBounds})\) and one physical hypercube \(\mathbf c=(c_1,\ldots,c_\ell)\), the leaf \(\code{D.LocalValues}\{c_1\}\cdots\{c_\ell\}\) is a \(2^\ell\)-by-\(\prod_s(\code{D.Degree}(s)+1)\) cell array. Its rows enumerate vertices \(\boldsymbol\nu^{(q)}\in\mathcal V_{\mathcal R}\); its columns enumerate local Bernstein labels in \code{combRows} order. Each entry already contains \(\sum_s\nu_s^{(q)}\,\partial P^{(\mathbf c)}/\partial\rho_s\). The rows add rate alternatives inside one existing hypercube; they do not add physical cells.
