...
Here \code{r} is scalar shorthand or a direction-wise absolute Bernstein Gram order \(\mathbf r\). The no-argument form uses the dimension-dependent minima above; an explicit order must satisfy the same applicable componentwise bound. In the multivariate branch, the unweighted block \(S_0\) uses tensor basis degree \(\mathbf r\), while \(S_s\) uses degree \(\mathbf r-\boldsymbol e_s\). A nominal block with a negative component is omitted. The target is degree-elevated without changing its represented polynomial, and the multivariate Gram sum is matched exactly at tensor degree \(2\mathbf r\).
...
Q_JZ_{\mathbf a_J}(\boldsymbol\alpha),
  \qquad Q_J\succeq0.
\]
Terms for which \(\mathbf a_J\) has a negative component are omitted.
