...
\begin{tikzpicture}[node distance=4mm and 4mm]
  ...
  \node[dp/card,text width=22mm,minimum height=17mm,below=of direct,xshift=12mm] (sparse)
    {\textbf{SparseFullBox}\\block-band Gram matrices};
...
\code{UseSparseFullBoxPreorder}, \code{SparseFullBoxOrder}, \code{BandWidth} & SparseFullBox selector, absolute Gram order, and bandwidth \(w\) of the symmetric block-band Gram matrices \(Q_J^{(w)}\). \\
\code{UseFullBoxPreorder}, \code{FullBoxOrder} & Full-box selector and absolute Gram order. \\
\bottomrule
\end{tabularx}

For example, \code{C.Relation}, \code{C.Residual.Degree}, and \code{C.UseSparseFullBoxPreorder} inspect a wrapper. Apply methods return a rebuilt value; assigning these properties is unsupported. Selected P\'olya and Gram parameters are stored as \(1\)-by-\(\ell\) rows. An intermediate SparseFullBox result stores all three sparse properties. A width-one request canonicalizes to actual Direct state, where those fields are \code{false}, zero, and zero; a request with \code{BandWidth >= max(SparseFullBoxOrder + 1)} canonicalizes to actual FullBox state, where only \code{UseFullBoxPreorder}/\code{FullBoxOrder} record the selection.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \code{BandWidth=w} remains one finite positive integer bandwidth for the symmetric block-band Gram matrices \(Q_J^{(w)}\). \code{SparseFullBoxOrder=r} accepts scalar shorthand or an \(\ell\)-element vector and is stored as a \(1\)-by-\(\ell\) absolute Gram order. Its default and minimum are \(\lfloor M/2\rfloor\) in one parameter and componentwise \(\lceil\mathbf M/2\rceil\) otherwise, where \(\mathbf M=\code{C.Residual.Degree}\). Width one canonicalizes to actual Direct state after order validation; width at least \(\max_s(r_s+1)\) canonicalizes to actual FullBox state; intermediate widths select SparseFullBox state.
...
\textit{P\'olya degree is a representation increment that may differ by direction; it is not a new decision degree, a grid refinement, or a Gram order.}
\end{center}

The mathematical background, its interpretation as lossless tensor degree elevation, and the limits of fixed-increment coefficient positivity are developed in \cref{sec:certificate-polya}.
...
\code{applyFullBoxPreorder}. Let \(Z_r(\alpha)=\mathbf b_r(\alpha)\otimes I_n\), where \(\mathbf b_r=[B_0^r,\ldots,B_r^r]^{\mathsf T}\). For selected absolute order \(r\), the even form matches at degree \(2r\),
\[
 S^{(c)}=Z_r^{\mathsf T}Q_0Z_r +\alpha(1-\alpha)
      Z_{r-1}^{\mathsf T}Q_1Z_{r-1},
\]
where the second block is omitted at \(r=0\).  The odd form matches at degree \(2r+1\),
\[
 S^{(c)}=(1-\alpha)Z_r^{\mathsf T}Q_LZ_r +\alpha Z_r^{\mathsf T}Q_UZ_r.
\]
...
For two or more parameters, the selector instead uses the fixed-order, box-specific singleton-generator Bernstein--Gram quadratic module. After normalizing the relation so that \(S^{(\mathbf c)}(\boldsymbol\alpha)\) is positive semidefinite, it represents
\[
S^{(\mathbf c)}(\boldsymbol\alpha)=S_0(\boldsymbol\alpha) +\sum_{s=1}^{\ell}\alpha_s(1-\alpha_s) S_s(\boldsymbol\alpha).
\]
...
Here \code{r} is scalar shorthand or a direction-wise absolute Bernstein Gram order \(\mathbf r\). The no-argument form uses the dimension-dependent minima above; an explicit order must satisfy the same applicable componentwise bound. In the multivariate branch, the unweighted block \(S_0\) uses tensor-product basis degree \(\mathbf r\), while \(S_s\) uses degree \(\mathbf r-\boldsymbol e_s\). A nominal block with a negative component is omitted. The target is degree-elevated without changing its represented polynomial, and the multivariate Gram sum is matched exactly at tensor degree \(2\mathbf r\).
...
\remarkshead
For the multivariate backend, the smallest useful worked case is \(\ell=2\). Let \(g_s=\alpha_s(1-\alpha_s)\). In the uniform specialization \(\mathbf r=(r,r)\), Putinar uses exactly the singleton-generator module
\[
  S^{(\mathbf c)}=S_0+g_1S_1+g_2S_2,
\]
...
\begin{tikzpicture}[node distance=7mm and 9mm,
  ...
  r/.style={dp/result,font=\scriptsize,align=center,minimum width=29mm,
    minimum height=11mm}]
  \node[n] (target) {positive target \(S^{(\mathbf c)}\)\\elevate to degree \(2r\)};
...
\code{applyPutinar} is a value-class method: it returns a new \code{pdlmi}, leaves \code{C} unchanged, and rebuilds from the original \code{Residual} and \code{Relation}. Reapplication replaces any earlier P\'olya, Putinar, SparseFullBox, or FullBox selection. It adds no implicit margin and makes no solver call. \Cref{sec:certificate-markov-lukacs,sec:certificate-putinar-schmudgen} derive the one-dimensional parity form and the multivariate singleton-generator module used by this selector.
...
\index{SparseFullBox}\index{block-band Gram matrix}\index{applySparseFullBoxPreorder@\texttt{applySparseFullBoxPreorder}}\index{BandWidth@\texttt{BandWidth}}
\label{sec:pdlmi-sparse-full-box}

SparseFullBox is FullBox with each dense Gram matrix \(Q_J\) replaced by a symmetric block-band Gram matrix \(Q_J^{(w)}\) of scalar bandwidth \(w\). It uses the same one-parameter parity terms, multivariate box-generator masks, relation orientation, and exact target-coefficient identities as FullBox. Only the admissible entries of the Gram matrices change: the cell residual \(\mathcal F^{(\mathbf c)}\), physical grid, decision degree \(\mathbf m\), assembled residual degree \(\mathbf M\), generator weights, and coefficient identities remain unchanged.

\syntaxhead
\begin{lstlisting}[style=dpsyntax]
Csp = C.applySparseFullBoxPreorder()
Csp = C.applySparseFullBoxPreorder(w)
Csp = C.applySparseFullBoxPreorder(w, r)
Csp = C.applySparseFullBoxPreorder(..., ValidationMode=mode)
\end{lstlisting}

\arghead
\code{w} is one finite positive integer bandwidth for \(Q_J^{(w)}\) and defaults to two. \code{r} is scalar shorthand or an optional direction-wise absolute SparseFullBox Gram order. It defaults to \(\lfloor M/2\rfloor\) for a one-parameter residual of degree \(M\), or componentwise \(\lceil\mathbf M/2\rceil\) for two or more parameters. Every explicit order is validated against that same componentwise minimum before endpoint normalization.

\descriptionhead
For selected absolute order \(\mathbf r\), use the paper notation \(\boldsymbol\chi_J\) for the indicator of \(J\) and \(\mathbf a_J=\mathbf r-\boldsymbol\chi_J\). SparseFullBox uses
\[
S^{(\mathbf c)}(\boldsymbol\alpha)=
  ...
  \qquad Q_J^{(w)}\succeq0,
\]
where every \(Q_J^{(w)}\) is symmetric block-band with bandwidth \(w\), and terms for which \(\mathbf a_J\) has a negative component are omitted. The one-parameter parity-specific FullBox formulas use the same replacement \(Q_J\mapsto Q_J^{(w)}\). Exact coefficient matching is unchanged. In one parameter, \(w=1\) is block diagonal and the implementation canonicalizes it to Direct, \(w=2\) is block tridiagonal, and \(w=3\) is block pentadiagonal. Increasing \(w\) gives a nested sufficient hierarchy, but feasibility need not improve strictly on a particular model. \Cref{sec:certificate-sparse-full-box} explains how the retained tensor windows determine the banded Gram map and why the two endpoint requests rebuild canonical Direct or FullBox state.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Every physical cell and active rate row owns independent Gram matrices \(Q_J^{(w)}\) and coefficient identities. Semidefinite mode builds matrix Gram terms; entry-wise mode builds an independent scalar certificate for every matrix entry in MATLAB column-major order.
  \item Width one canonicalizes to actual Direct state. A width satisfying \code{w >= max(r + 1)} canonicalizes to actual FullBox state. For \(1<w<\max_s(r_s+1)\), the result records \code{UseSparseFullBoxPreorder=true}, \code{SparseFullBoxOrder=r}, and \code{BandWidth=w}.
...
The mathematical construction is a full-box preordering in the unit-box local coordinates \(\boldsymbol\alpha=\phi_{\mathbf c}^{-1}(\boldsymbol\rho)\in[0,1]^\ell\); the software state is \code{FullBox}. It is a box-specific Bernstein--Gram representation, not a general Putinar certificate, general-domain SOS interface, or full-space SOS representation. It adds no implicit positivity margin. For an entry-wise inequality, every column-major scalar entry receives an independent copy of the selected Gram construction. Equality wrappers reject FullBox selection.
...
The no-argument form selects the smallest admissible absolute order. The explicit form accepts scalar shorthand or a direction-wise \code{r}; it does not add \code{r} to the residual degree. Both forms are value-class selectors: they return a new \code{pdlmi}, leave \code{C} unchanged, and rebuild from \code{C.Residual}. Reapplication therefore replaces an earlier FullBox order or a P\'olya, Putinar, or SparseFullBox selection rather than compounding it.

The idea is to represent the oriented positive target as a sum of PSD Gram forms multiplied by functions that are nonnegative on the local box. For selected direction-wise absolute order \(\mathbf r\), let \(\boldsymbol\chi_J\) be the indicator of \(J\), set \(\mathbf a_J=\mathbf r-\boldsymbol\chi_J\), and define \(Z_{\mathbf a_J}=\mathbf b_{\mathbf a_J}\otimes I_n\):
\[
S^{(\mathbf c)}(\boldsymbol\alpha)=
  \sum_{J\subseteq\{1,\ldots,\ell\}}
  \left(\prod_{s\in J}\alpha_s(1-\alpha_s)\right)
  Z_{\mathbf a_J}(\boldsymbol\alpha)^{\mathsf T}
Q_JZ_{\mathbf a_J}(\boldsymbol\alpha),
  \qquad Q_J\succeq0.
\]
Terms for which \(\mathbf a_J\) has a negative component are omitted. In one parameter the implementation uses the even/odd Markov--Luk\'acs forms, with minimum absolute order \(\lfloor M/2\rfloor\) for assembled residual degree \(M\). For two or more parameters it uses every subset product of the \(\ell\) box generators and componentwise minimum order \(\lceil\mathbf M/2\rceil\). The selected order fixes the dense Bernstein Gram bases; it is not added to \(\mathbf M\).
...
\remarkshead
For \(\ell=2\), the FullBox state differs from Putinar by retaining the product generator. With \(g_s=\alpha_s(1-\alpha_s)\), the uniform specialization \(\mathbf r=(r,r)\) has the exact backend form
\[
S^{(\mathbf c)}=S_0+g_1S_1+g_2S_2+g_1g_2S_{12}.
\]
