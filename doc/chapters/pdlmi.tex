...
With \code{UsePolya=true}, \code{pdlmi} first elevates the stored residual by \code{PolyaDegree} in every parameter direction, then constrains every elevated local coefficient and every stored rate row. Thus, if the assembled residual degree is \(M\), the count is \(N_c (M+d+1)^\ell\) for ordinary expressions and \(N_c (M+d+1)^\ell N_v\) for rate-row expressions. An increment \(d=0\) is valid and has the same coefficient count as Direct. For an entry-wise inequality, elevation preserves the global mode and the oriented sign is applied independently to every column-major scalar entry of each elevated coefficient. Equality wrappers reject this method.
...
\index{SparseFullBox}\index{sparse full-box tensor-window certificate}\index{applySparseFullBoxPreorder@\texttt{applySparseFullBoxPreorder}}\index{BandWidth@\texttt{BandWidth}}
\label{sec:pdlmi-sparse-full-box}

The sparse full-box tensor-window certificate, stored as software state \code{SparseFullBox}, uses the same one-parameter parity forms and multivariate box-generator masks as FullBox, but replaces every dense Gram block by free positive-semidefinite blocks on all axis-aligned sliding windows of its tensor Bernstein basis. It changes Gram support only: the original residual \(\mathcal F\), physical grid, decision degree \(m\), assembled residual degree \(M\), box-generator weights, relation orientation, and exact coefficient identities remain the same.
