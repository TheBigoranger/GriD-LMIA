...
Use signs, sums, and differences to build known matrix expressions. Compatible physical bounds are refined to the union grid, and both operands are elevated exactly to the componentwise maximum degree before matching coefficients are combined. The corresponding MATLAB overload names are \code{plus}, \code{minus}, \code{uplus}, and \code{uminus}.
...
Parenthesis indexing selects a matrix block from every coefficient. Assignment writes the selected block into every coefficient after compatible promotion and exact alignment to the componentwise maximum degree. It does not select a physical cell; use \code{coeffs} for cell inspection.
