...
Use \code{mtimes} for a matrix product. The inner payload dimensions must match. Two parameter-dependent factors produce the componentwise degree sum. A numeric or bare affine \code{sdpvar} factor is parameter independent and contributes degree zero in every component. A mixed coefficient-backed \code{pdmat}/\code{sdpvar} product returns \code{pdvar} because it contains YALMIP decisions.
...
Here \code{X} is a real affine two-dimensional \code{sdpvar}. Numeric and \code{pdmat} products return \code{pdmat}. A mixed \code{pdmat}/\code{sdpvar} product returns \code{pdvar}, preserves the known grid and degree, supports scalar broadcasting and ordinary rectangular matrix products, and reuses exactly the YALMIP decisions already present in \code{X}. Payload matrices multiply in the written order. \Cref{sec:bernstein-product-math} derives the parameter-dependent coefficient convolution.

The multiplication backend selects one of two paths. A numeric or \code{pdmat} operand uses the shared alignment and \code{prodVals} path in \cref{fig:alignment-call-path}. A bare \code{sdpvar} uses a class-restricted affine bridge that validates the symbolic matrix and maps the existing known coefficient tree into \code{pdvar} storage.

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=5mm and 7mm]
  \node[dp/call-entry] (entry) {\code{mtimes}};
  \node[dp/call-branch,right=of entry] (kind) {operand kind};
  \node[dp/call-step,above right=of kind] (prod) {alignment and\\\code{prodVals}};
  \node[dp/call-step,below right=of kind] (bridge) {affine \code{sdpvar}\\bridge};
  \node[dp/call-result,right=of prod] (known) {\code{pdmat} result};
  \node[dp/call-result,right=of bridge] (affine) {\code{pdvar} result};
  \draw[dp/call-arrow] (entry) -- (kind);
  \draw[dp/call-arrow] (kind) -- node[above left,font=\scriptsize]{numeric or \code{pdmat}} (prod);
  \draw[dp/call-arrow] (kind) -- node[below left,font=\scriptsize]{\code{sdpvar}} (bridge);
  \draw[dp/call-arrow] (prod) -- (known);
  \draw[dp/call-arrow] (bridge) -- (affine);
\end{tikzpicture}
\caption{Multiplication call paths. Parameter-dependent factors use the ordered binomial-weighted convolution, while a bare affine \code{sdpvar} maps the known coefficient tree into \code{pdvar} and preserves matrix order.}
...
variableCounts = [numel(getvariables(L.evaluate(0))) ...
    numel(getvariables(R.evaluate(0)))]
\end{lstlisting}

\begin{lstlisting}[style=dpoutput]
leftClass = 'pdvar'
rightClass = 'pdvar'
mixedSize = 1×2 double
     2     2
...
\remarkshead
Incompatible operands for \code{+}, \code{-}, and \code{*} raise \codeerr{pdmat}{InvalidAddition}, \codeerr{pdmat}{InvalidSubtraction}, or \codeerr{pdmat}{InvalidMultiplication}. A mixed symbolic product requires a real affine two-dimensional \code{sdpvar}. Parameter counts or outer bounds that reject a common grid raise \codeerr{pdmat}{MixedGrid}. Coefficient algebra on a function-only object raises \code{pdmat:}\allowbreak\code{FunctionOnly}%
