...
\begin{tikzpicture}[node distance=7mm and 7mm,
  ...
  \node[dp/call-result,below=25mm of fork] (store) {\hyperref[sec:pdbase-constructor]{\code{pdbase}}\\cell-wise storage};
...
\code{pdmat} adds the read-only \code{FunctionHandle} property to the inherited \code{pdbase} properties in \cref{sec:pdbase-fields}.  All use private set access.  \code{FunctionHandle} contains the supplied handle for function and function-Bernstein sources and is empty for cell-backed sources. The inherited properties describe the grid, matrix size, Bernstein degree, cell-wise evidence, continuity, and origin.
...
\descriptionhead
The storage queries use the shared \code{pdbase} cell and label order. \code{cells} returns one one-based physical-cell subscript per row, \code{lbls} returns the zero-based tensor labels $\vect{i}\in\prod_{s=1}^{\ell}\{0,\ldots,m_s\}$, and \code{coeffs} returns the selected flat coefficient leaf. \code{ncell}, \code{ncoeff}, and \code{npar} count physical cells, $\prod_{s=1}^{\ell}(m_s+1)$ coefficients per cell, and parameter directions. \code{elevate} accepts a scalar shorthand or direction-wise increment and returns a new \code{pdmat} at the componentwise degree sum while preserving the represented known function, metadata, physical cells, and stored rate-row order. The optional positional validation mode and its errors follow the canonical contract in \cref{sec:pdbase-elevate}. The sparse elevation map and its reuse are derived in \cref{sec:bernstein-degree-elevation}.
