...
The package performs the analogous matching in tensor Bernstein coordinates. For a tensor degree \(\mathbf a=(a_1,\ldots,a_\ell)\in\mathbb N^\ell\), define
\[
...
\]
adds \(2\boldsymbol\chi_J\) to that coordinatewise degree. Consequently, a box-weighted term matched at degree \(2r\boldsymbol1\) uses \(\mathbf a=r\boldsymbol1-\boldsymbol\chi_J\). In the odd one-dimensional interval form, the endpoint weights \(\alpha\) and \(1-\alpha\) instead add degree one. Expanding each weighted Bernstein Gram form and equating it to the target Bernstein array produces affine coefficient identities, just as in the monomial example.
...
\ideastep{Formula} After any rate rows have been stacked, write the residual and target coefficients on physical cell \(\mathbf c\) as \(\mathcal F^{(\mathbf c)}=\sum_{\mathbf i}B_{\mathbf i}^{M}\mathcal C^{(\mathbf c)}[\mathbf i]\) and \(C^{(\mathbf c)}[\mathbf i]=-\mathcal C^{(\mathbf c)}[\mathbf i]\). Then
\[
...
\[
  ...
  \qquad
  S_0=Z_{r\boldsymbol1}^{\mathsf T}Q_0
       Z_{r\boldsymbol1},\quad
  S_s=Z_{r\boldsymbol1-\boldsymbol e_s}^{\mathsf T}Q_s
       Z_{r\boldsymbol1-\boldsymbol e_s},
\]
...
For \(\ell\ge2\) and absolute order \(r\), use the already defined tensor Bernstein lift \(Z_{\mathbf a}=\mathbf b_{\mathbf a}\otimes I_n\). The implemented form is
\[
  S^{(\mathbf c)}=
  \sum_{J\subseteq\{1,\ldots,\ell\}}
  \left(\prod_{s\in J}g_s\right)
  Z_{r\boldsymbol1-\boldsymbol\chi_J}^{\mathsf T}
  Q_J
  Z_{r\boldsymbol1-\boldsymbol\chi_J},
  \qquad Q_J\succeq0.
\]
...
\]
For positive integer bandwidth \(w\), set \(w_s=\min(w,d_s+1)\). For every start label \(\boldsymbol t\) with \(0\le t_s\le d_s-w_s+1\), define
\[
  W_{\boldsymbol t}=\prod_{s=1}^{\ell}\{t_s,\ldots,t_s+w_s-1\}.
\]
If \(Z_{W_{\boldsymbol t}}\) contains exactly the Bernstein basis functions whose tensor labels lie in \(W_{\boldsymbol t}\), SparseFullBox replaces the dense term by
\[
  \sum_{\boldsymbol t}
  h_J(\boldsymbol\alpha)\,
  Z_{W_{\boldsymbol t}}(\boldsymbol\alpha)^{\mathsf T}
  Q_{J,\boldsymbol t}
  Z_{W_{\boldsymbol t}}(\boldsymbol\alpha),
...
\ideastep{Worked example} For one parameter, even assembled residual degree \(M=4\), and absolute order \(r=2\), the unweighted basis has labels \(\{0,1,2\}\) and the \(\alpha(1-\alpha)\)-weighted basis has labels \(\{0,1\}\). Width \(w=2\) therefore creates unweighted windows \(\{0,1\}\) and \(\{1,2\}\), plus one complete weighted window \(\{0,1\}\). The finite certificate has three PSD blocks followed by five exact coefficient identities. The unweighted support is block tridiagonal. Width \(w=3\) includes every basis label in the unweighted block and reaches the dense FullBox endpoint.
...
\begin{tikzpicture}[node distance=6mm and 7mm,
  ...
  \node[box,right=of direct] (tri) {\(w=2\)\\intermediate band support};
...
\begin{tikzpicture}[>=Latex,node distance=5mm,
  ...
  \node[box,below=of polya] (band) {SparseFullBox bandwidth \(w\)\\larger Gram support};
...
SparseFullBox bandwidth \(w\) & band-limited Gram support and PSD clique sizes & decision degree or physical grid \\
\bottomrule
\end{tabularx}

When a certificate fails, do not infer continuous infeasibility. Decide separately whether the model needs a richer decision degree \(m\), a finer physical grid, a larger P\'olya increment \(d\), a wider SparseFullBox window \(w\), or a higher absolute Gram order \(r\).
