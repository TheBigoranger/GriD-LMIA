...
\section{The Implemented SparseFullBox Band-Limited State}
\label{sec:certificate-sparse-full-box}
\index{SparseFullBox}\index{tensor window}\index{bandwidth}

\ideastep{Basic idea} SparseFullBox is the FullBox certificate with band-limited Gram support: it retains every FullBox parity or generator-mask term and every exact target-coefficient identity. The implementation realizes the support by replacing one dense Gram matrix with a sum of free PSD clique matrices on overlapping axis-aligned windows of the tensor Bernstein basis. Harris and Parrilo provide a close univariate Bernstein precedent, while factor-width and chordal matrix-SOS work provide broader sparse-Gram context \cite{harrisparrilo2023,gouveiakovacecsaee2022,zhengfantuzzi2023}; none of those sources defines this package's band map or window realization.
...
\begin{tikzpicture}[node distance=6mm and 7mm,
  ...
  \node[box,right=of direct] (tri) {\(b=2\)\\intermediate band support};
...
\begin{tikzpicture}[>=Latex,node distance=5mm,
  ...
  \node[box,below=of polya] (band) {SparseFullBox bandwidth \(b\)\\larger Gram support};
...
SparseFullBox bandwidth \(b\) & band-limited Gram support and PSD clique sizes & decision degree or physical grid \\
\bottomrule
\end{tabularx}
