...
Fix one physical cell $\mathcal H_{\vect{c}}$ and one stored rate row. Write the theoretical cell residual as $\mathcal F^{(\vect{c})}\prec0$ and define the sign-normalized positive target $S^{(\vect{c})}=-\mathcal F^{(\vect{c})}$. The theoretical problem is
\begin{gather*}
 S^{(\vect{c})}(\vect{\alpha})\succ0
  \quad\text{for every }\vect{\alpha}\in[0,1]^\ell.
\end{gather*}
The package's Direct and Gram constraints prove non-strict inequalities. A strict model must state its own margin, for example $S^{(\vect{c})}-\varepsilon I\succeq0$. The package uses a zero implicit margin.
...
\end{gather*}
with only finitely many nonzero coefficients. Its total degree is $\deg p=\max\{|\vect{\eta}|:p_{\vect{\eta}}\ne0\}$, where $|\vect{\eta}|=\sum_{s=1}^{\ell}\eta_s$. A scalar polynomial is a sum of squares (SOS) if $p=q_1^2+\cdots+q_t^2$, which immediately implies $p\ge0$.
...
\ideastep{Formula} After any rate rows have been stacked, write the residual and target coefficients on physical cell $\vect{c}$ as $\mathcal F^{(\vect{c})}=\sum_{\vect{i}\in\mathcal I_{\vect{M}}=\prod_{s=1}^{\ell}\{0,\ldots,M_s\}}B_{\vect{i}}^{\vect{M}}\mathcal C^{(\vect{c})}[\vect{i}]$ and $C^{(\vect{c})}[\vect{i}]=-\mathcal C^{(\vect{c})}[\vect{i}]$. Then
\begin{gather*}
 S^{(\vect{c})}(\vect{\alpha})=
