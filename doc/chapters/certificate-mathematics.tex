...
\]
Equating this affine map to each target coefficient gives exact linear identities, while $Q\succeq0$ supplies the LMIs. For one parameter, \code{usePutinar} and \code{useFullBox} implement these exact parity forms. \code{useSpPut} and \code{useSpBox} apply their respective sliding tensor-window constructions to the active Putinar or FullBox terms. Their minimum absolute order is $\lfloor M/2\rfloor$, where $M$ is the assembled residual degree.
...
For one parameter, \code{usePutinar(r)} dispatches to the same exact even/odd Markov--Luk\'acs forms as \code{useFullBox(r)}. Its absolute default and minimum are $r=\lfloor M/2\rfloor$. Even targets use the unweighted and $\alpha(1-\alpha)$-weighted Gram blocks and match at degree $2r$, while odd targets use the two endpoint-weighted Gram blocks and match at degree $2r+1$.
...
\ideastep{Worked example} Consider one scalar parameter, even target degree four, absolute order $r=2$, and clique size $b=2$. The unweighted Putinar term has Gram degree $d=2$, window size two, and the two windows $\{0,1\}$ and $\{1,2\}$. The weighted term has Gram degree $d=1$ and one window $\{0,1\}$. SparsePutinar therefore has three PSD blocks of dimension two and five exact coefficient identities. The label pair $(1,1)$ belongs to both unweighted windows, so both independent blocks contribute to target label two.
...
\ideastep{Worked example} For one parameter, even assembled residual degree $M=4$, and absolute order $r=2$, the unweighted term has basis labels $\{0,1,2\}$ and the weighted term has labels $\{0,1\}$. At $w=2$, the unweighted windows are $\{0,1\}$ and $\{1,2\}$, while the weighted term has one window $\{0,1\}$. The certificate therefore has three independent PSD blocks of dimension two and five exact coefficient identities. The pair $(1,1)$ appears in both unweighted windows, so both diagonal Gram entries contribute to target label two.
