...
\]
with only finitely many nonzero coefficients. Its total degree is \(\deg p=\max\{|\vect{\eta}|:p_{\vect{\eta}}\ne0\}\), where \(|\vect{\eta}|=\sum_s\eta_s\). A scalar polynomial is a sum of squares (SOS) if \(p=q_1^2+\cdots+q_t^2\), which immediately implies \(p\ge0\).
...
\]
The vector $\vect{b}_{\vect{a}}$ has $\prod_s(a_s+1)$ entries. Therefore $Z_{\vect{a}}$ has $\prod_s(a_s+1)$ block rows of size $n$-by-$n$, equivalently $n\prod_s(a_s+1)$ scalar rows, and $Q$ has scalar dimension $n\prod_s(a_s+1)$. Each $n$-by-$n$ block $Q[\vect{\eta},\vect{\zeta}]$ multiplies one ordered pair of tensor Bernstein basis functions. When the Gram form is expanded, all ordered pairs that reach the same target label are added before that target coefficient is equated to the corresponding residual coefficient. The unweighted form $Z_{\vect{a}}^{\mathsf T}QZ_{\vect{a}}$ has coordinatewise degree $2\vect{a}$. Multiplication by
\[
  g_J(\vect{\alpha})
...
Fix one weighted Gram term with tensor basis degree $\vect{d}$, alpha power $\vect{p}$, and one-minus-alpha power $\vect{q}$. Let $\mathcal B$ be either the complete tensor label set $\prod_s\{0,\ldots,d_s\}$ or one sliding tensor window, and partition the symmetric Gram matrix $Q$ into $n$-by-$n$ blocks $Q[\vect{\eta},\vect{\zeta}]$ indexed by labels in $\mathcal B$. The common target degree is $\vect{h}=2\vect{d}+\vect{p}+\vect{q}$. For $\vect{k}\in\prod_s\{0,\ldots,h_s\}$, define
\[
...
For the small scalar basis $z=[B_0^1,B_1^1]^{\mathsf T}$ with unit generator weight,
\[
z^{\mathsf T}Qz
...
\ideastep{Formula} After any rate rows have been stacked, write the residual and target coefficients on physical cell \(\vect{c}\) as \(\mathcal F^{(\vect{c})}=\sum_{\vect{i}}B_{\vect{i}}^{\vect{M}}\mathcal C^{(\vect{c})}[\vect{i}]\) and \(C^{(\vect{c})}[\vect{i}]=-\mathcal C^{(\vect{c})}[\vect{i}]\), where \(\vect{i}\in\prod_s\{0,\ldots,M_s\}\). Then
\[
 S^{(\vect{c})}(\vect{\alpha})=
...
\[
  ...
  \qquad p=\vect{b}_1^{\mathsf T}Q\vect{b}_1.
\]
...
\[
  ...
  \qquad
  S_0=Z_{\vect{r}}^{\mathsf T}Q_0
       Z_{\vect{r}},\quad
  S_s=Z_{\vect{r}-\vect{e}_s}^{\mathsf T}Q_s
       Z_{\vect{r}-\vect{e}_s},
\]
...
\]
The window basis has $\prod_s w_s$ labels, so an $n$-by-$n$ polynomial-matrix term uses an independent PSD block $Q_{\vect{u}}$ of scalar dimension
\[
  n\prod_{s=1}^{\ell}w_s
...
\ideastep{Exact accumulated identity} For multi-indices, write $\binom{\vect{a}}{\vect{k}}=\prod_s\binom{a_s}{k_s}$. The contribution of one window to target label $\vect{k}$ is
\[
...
\]
Let $t$ index the active Putinar terms and let $\widetilde C^{(\vect{c})}[\vect{k}]$ be the sign-normalized target coefficient after exact elevation to their common degree $\vect{h}$. SparsePutinar imposes
\[
  \widetilde C^{(\vect{c})}[\vect{k}]
  =\sum_t\sum_{\vect{u}\in\mathcal U_t}
    \mathcal A^{t}_{\vect{k},\vect{u}}(Q_{t,\vect{u}})
  \qquad
  \text{for every }\vect{k}\in\prod_s\{0,\ldots,h_s\}.
\]
The sum is literal. If two windows contain the same ordered pair $(\vect{\eta},\vect{\zeta})$, their independent Gram blocks both contribute to the same target coefficient. For a symmetric scalar Gram matrix, an off-diagonal entry represents both ordered pairs. For a polynomial matrix, the two ordered block terms $Q[\vect{\eta},\vect{\zeta}]$ and $Q[\vect{\zeta},\vect{\eta}]$ provide the corresponding symmetry accounting. The implementation assembles every local PSD block first and then creates exactly $\prod_s(h_s+1)$ coefficient identities.
...
\[
  ...
  Z_{\vect{r}-\vect{\chi}_J}^{\mathsf T}
  Q_J
...
\ideastep{Formula} For one active FullBox term $J$ with Gram degree $\vect{a}_J$, choose the scalar side length $w$ and set $q_{J,s}=\min\{w,a_{J,s}+1\}$. Every start $\vect{u}$ in $\prod_s\{0,\ldots,a_{J,s}-q_{J,s}+1\}$ defines the basis window
\[
...
\[
S^{(\vect{c})}(\vect{\alpha})=
\sum_J g_J(\vect{\alpha})\sum_{\vect{u}}
\left(\vect{z}_{J,\vect{u}}(\vect{\alpha})\otimes I_n\right)^{\mathsf T}
Q_{J,\vect{u}}
