...
\ideastep{Formula} For \(\ell\ge2\), use the paper notation \(\mathbf a_J=r\boldsymbol1-\boldsymbol\chi_J\) with the FullBox generators:
\[
  S^{(\mathbf c)}=
  \sum_{J\subseteq\{1,\ldots,\ell\}}
  \left(\prod_{s\in J}g_s\right)
...
When a certificate fails, do not infer continuous infeasibility. Decide separately whether the model needs a richer decision degree \(m\), a finer physical grid, a larger P\'olya increment \(d\), a larger SparseFullBox bandwidth \(w\), or a higher absolute Gram order \(r\).
...
\code{applySparseFullBoxPreorder([w[,r]])} & automatic or adaptive sparse-pattern selection \\
