...
\ideastep{Basic idea} A real symmetric matrix $Q$ belongs to the \termdef{psd} cone when
\begin{gather*}
  u^\top Qu\ge0\qquad\text{for every vector }u.
\end{gather*}
\ideastep{Formula} Let $\vect{y}=(y_1,\ldots,y_N)$ collect the scalar decision coordinates. An LMI is an affine matrix expression constrained to the PSD cone:
\begin{gather*}
...
\ideastep{Basic idea} A polynomial in $\vect{\xi}=(\xi_1,\ldots,\xi_\ell)$ is a finite sum
\begin{gather*}
  p(\vect{\xi})=
...
\ideastep{Formula} After any rate rows have been stacked, write the residual and target coefficients on physical cell $\vect{c}$ as $\mathcal F^{(\vect{c})}=\sum_{\vect{i}\in\prod_{s=1}^{\ell}\{0,\ldots,M_s\}}B_{\vect{i}}^{\vect{M}}\mathcal C^{(\vect{c})}[\vect{i}]$ and $C^{(\vect{c})}[\vect{i}]=-\mathcal C^{(\vect{c})}[\vect{i}]$. Then
\begin{gather*}
 S^{(\vect{c})}(\vect{\alpha})=
...
\ideastep{Basic idea} P\'olya selection preserves the residual polynomial. It rewrites that same polynomial at a higher Bernstein degree and then retries the direct coefficient test.
...
\ideastep{Basic idea} On one interval, endpoint factors can encode the domain exactly. Even and odd target degrees require different weighted Gram patterns, so parity is a structural choice with distinct coefficient maps.

\ideastep{Formula} In one variable, write the Gram order as $\vect{r}=(r_1)$. Interval nonnegativity has exact parity-specific descriptions \cite{lukacs1918,deklerkhesslaurent2017}. If $\deg p\le2r_1$,
\begin{gather*}
p\ge0\text{ on }[0,1]
  \Longleftrightarrow
...
\ideastep{Worked example} A cubic residual has component degree $2r_1+1$ with $\vect{r}=(1)$. It therefore uses two degree-one Gram bases, one multiplied by $1-\alpha$ and one by $\alpha$. This is the generic one-dimensional odd-degree full-box pattern.
...
\ideastep{Window construction} Fix one active weighted Putinar term with Gram degree $\vect{d}$, weight $\vect{\alpha}^{\vect{p}}(\vect{1}-\vect{\alpha})^{\vect{q}}$, and common target degree
\begin{gather*}
...
\end{gather*}
\ideastep{Exact accumulated identity} For multi-indices, write $\binom{\vect{a}}{\vect{k}}=\prod_{s=1}^{\ell}\binom{a_s}{k_s}$. The contribution of one window to target label $\vect{k}$ is
\begin{gather*}
...
\ideastep{Finite SDP example: $\ell=2$, $\vect{M}=(3,3)$} Let $(\alpha_1,\alpha_2)\in[0,1]^2$ be the local coordinates of one cell, and suppose the sign-normalized target is
\begin{gather*}
  S^{(\vect{c})}(\vect{\alpha})=
...
\ideastep{Worked example} For one parameter, take the even assembled residual degree $\vect{M}=(4)$ and absolute order $\vect{r}=(2)$. The unweighted term has basis labels $\{0,1,2\}$ and the weighted term has labels $\{0,1\}$. At $w=2$, the unweighted windows are $\{0,1\}$ and $\{1,2\}$, while the weighted term has one window $\{0,1\}$. The certificate therefore has three independent PSD blocks of dimension two and five exact coefficient identities. The pair $(1,1)$ appears in both unweighted windows, so both diagonal Gram entries contribute to target label two.
