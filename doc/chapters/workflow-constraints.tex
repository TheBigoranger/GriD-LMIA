...
Direct tests coefficient signs. P\'olya elevates the residual before testing. For this one-parameter residual, the degree-three default order is one, so width two reaches the dense endpoint and SparseFullBox canonicalizes to actual FullBox state; Putinar also delegates to the same parity construction, and all three wrappers store 24 constraints. At intermediate widths, SparseFullBox replaces each dense Gram block by all axis-aligned tensor-basis windows of the requested side length. In multiple parameters, Putinar uses the singleton-generator box module, whereas SparseFullBox and FullBox use all box-generator products with sparse-window or dense Gram support. All five paths are finite sufficient certificates for the continuous inequality. Failure of a selected certificate to yield feasibility does not prove that the continuous inequality itself is infeasible. \Cref{sec:certificate-direct,sec:certificate-polya,sec:certificate-markov-lukacs,sec:certificate-putinar-schmudgen,sec:certificate-sparse-full-box,sec:certificate-full-box} give the corresponding mathematical constructions, while \cref{sec:certificate-boundary} states the implemented boundary.
