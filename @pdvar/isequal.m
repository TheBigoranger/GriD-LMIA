function tf = isequal(varargin)
    %ISEQUAL True when pdvar objects have identical metadata and coefficients.
    %
    %   Syntax:
    %     tf = isequal(P, Q)
    %
    %   Output:
    %     tf - True when compatible coefficient expressions match exactly.
    %
    %   Example:
    %     P = pdvar(1, {[0 1]});
    %     tf = isequal(P, P);

    if nargin < 2
        tf = true;
        return
    end

    ref = varargin{1};
    if ~isa(ref, "pdvar")
        tf = builtin("isequal", varargin{:});
        return
    end

    tf = true;
    for k = 2:nargin
        val = varargin{k};
        if ~isa(val, "pdvar") || ~sameOne(ref, val)
            tf = false;
            return
        end
    end
end

function tf = sameOne(a, b)
    %SAMEONE Compare the complete pdvar representation contract.
    tf = builtin("isequal", a.MatrixSize, b.MatrixSize) && ...
        builtin("isequal", a.Degree, b.Degree) && ...
        builtin("isequal", a.GridInfo.Vectors, b.GridInfo.Vectors) && ...
        builtin("isequal", a.LocalValues, b.LocalValues) && ...
        builtin("isequal", a.Continuity, b.Continuity) && ...
        builtin("isequal", a.ContainsDecision, b.ContainsDecision) && ...
        builtin("isequal", a.NumRateRows, b.NumRateRows) && ...
        builtin("isequal", a.RateBounds, b.RateBounds) && ...
        builtin("isequal", a.SourceSummary, b.SourceSummary);
end
