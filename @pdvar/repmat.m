...
function sz = parseRep(args)
    ...
    elseif numel(args) == 2
        sz = zeros(1, 2);
        for k = 1:2
            sz(k) = helper.chk(args{k}, "pdvar:InvalidRepmat", ...
                "Repetition counts must be positive integer scalars.", ...
                "numeric", "real", "scalar", "finite", "integer", "positive");
        end
        return
    else
        error("pdvar:InvalidRepmat", ...
            "pdvar repmat expects a scalar, a two-element size vector, or two size scalars.");
    end

    if numel(sz) ~= 2
        error("pdvar:InvalidRepmat", ...
            "pdvar repmat supports exactly two matrix repetition counts.");
    end
    for k = 1:2
        sz(k) = helper.chk(sz(k), "pdvar:InvalidRepmat", ...
            "Repetition counts must be positive integer scalars.", ...
            "numeric", "real", "scalar", "finite", "integer", "positive");
    end
end
