function out = mean(obj, varargin)
    %MEAN Average pdvar coefficient payloads along a matrix dimension.
    %
    %   Syntax:
    %     M = mean(P)
    %     M = mean(P, dim)
    ...
    if numel(varargin) ~= 1
        error("pdvar:InvalidMean", "pdvar mean accepts at most one dimension argument.");
    end
    ...
    dim = helper.chk(dim, "pdvar:InvalidMean", ...
