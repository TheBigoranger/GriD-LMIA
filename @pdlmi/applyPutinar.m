function out = applyPutinar(obj, order)
    ...
    %
    %   The default order is ceil(Residual.Degree/2). Reapplication rebuilds
    %   from Residual and replaces any Pólya, Putinar, or full-box selection.
    %
    %   In every physical cell and active rate row, the sign-normalized target
    %   is represented as S0 + sum_s alpha_s(1-alpha_s)Ss. No positivity margin
    %   is inserted and this method does not call a solver. Invalid orders raise
