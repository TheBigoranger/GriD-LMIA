function tests = test_gram_equiv
    %TEST_GRAM_EQUIV Compare planned maps to the legacy double loop.
...
function testDenOneDimFulBox(testCase)
...
function testSpaTenWin(testCase)
    grid = {[0 1], [-1 1]};
...
function testAniFulBox(testCase)
...
function testAniPutZerAxi(testCase)
...
function testAniSpaWin(testCase)
...
function testSpaPutTenWin(testCase)
...
function veriLegCer(testCase, wrapper, targetDegree, specs, bandWidth)
    ...
    elevated = wrapper.Residual.elevate( ...
        targetDegreeVector - wrapper.Residual.Degree);
    cells = wrapper.Residual.cells();
    target = helper.cellGet(elevated.LocalValues, cells(1, :));
    target = target(1, :);
    if wrapper.Relation == "<="
