function tests = test_use_putinar
    % Behavioral regressions for pdlmi.use_putinar.
...
function test_constructor_forms_and_inactive_defaults(testCase)
...
function test_default_order_by_residual_degree(testCase)
...
function test_interval_selector_intentionally_shares_fullbox_s(testCase)
...
function test_order_validation_and_disabled_selection(testCase)
...
function test_malformed_and_conflicting_options(testCase)
    P = pdvar(1, {[0 1]}, Degree=2);
...
function test_replacement_from_other_families(testCase)
    P = pdvar(1, {[0 1]}, Degree=2);
    direct = P >= 0;
...
function test_order_zero_omits_both_nominal_negative(testCase)
...
function test_putinar_only_empty_admissible_singleton_masks(testCase)
...
function test_singleton_multiplier_absent_when_axis_order(testCase)
...
function test_degree_interval_targets_two_endpoint_psd(testCase)
    P = pdvar(1, {[0 1]}, Degree=2);
    lower = P >= 0;
    upper = P <= 0;
...
function test_independent_scalar_certificate_follows_each_matlab(testCase)
...
function test_rectangular_derivative_rows_receive_independent(testCase)
