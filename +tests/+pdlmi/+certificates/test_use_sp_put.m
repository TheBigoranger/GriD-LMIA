function tests = test_use_sp_put
    % Behavioral regressions for pdlmi.use_sp_put.
...
function test_constructor_selection_and_immutability(testCase)
...
function test_endpoint_normalization_bypass_explicit_order(testCase)
...
function test_singleton_and_dense_limits(testCase)
...
function test_anisotropic_orders_and_zero_degree(testCase)
    grid = {[0 1], [10 20]};
...
function test_validation_and_parser_precedence(testCase)
    P = pdvar(1, {[0 1]}, Degree=4);
    direct = P >= 0;
...
function test_replace_certificate_families(testCase)
    P = pdvar(1, {[0 1]}, Degree=4);
    direct = P >= 0;
...
function test_differentiation_lowers_degree_8_local_certificates(testCase)
    P = pdvar(1, {[0 1]}, Degree=4);
    lowerDirect = P >= 0;
    upperDirect = P <= 0;
