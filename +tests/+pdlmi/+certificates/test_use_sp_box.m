function tests = test_use_sp_box
    % Behavioral regressions for pdlmi.use_sp_box.
...
function test_constructor_forms_and_immutability(testCase)
...
function test_endpoint_normalization_bypass_validation_explicit(testCase)
    P = pdvar(1, {[0 1]}, Degree=4);
    direct = P >= 0;
    widthOne = direct.useSpBox(1, 2);
    denseEndpoint = direct.useSpBox(3, 2);
    denseReference = direct.useFullBox(2);
    ...
    testCase.verifyError(@() direct.useSpBox(1, 1), ...
...
function test_width_order_and_validation(testCase)
    P = pdvar(1, {[0 1]}, Degree=4);
    direct = P >= 0;
    ...
    testCase.verifyError(@() direct.useSpBox(2, 1), ...
        "pdlmi:SparseFullBoxOrderTooLow");
    testCase.verifyError(@() pdlmi(P, ">=", ...
...
function test_rebuild_from_each_family(testCase)
    P = pdvar(1, {[0 1]}, Degree=4);
    direct = P >= 0;
    sparse = direct.useSpBox(2, 2);
    polya = sparse.usePolya(1);
    putinar = sparse.usePutinar(2);
    full = sparse.useFullBox(2);
    fromPolya = polya.useSpBox(2, 2);
    fromPutinar = putinar.useSpBox(2, 2);
    fromFull = full.useSpBox(2, 2);
...
function test_window_widths_and_cone_sizes(testCase)
    P = pdvar(1, {[0 1]}, Degree=6);
    direct = P >= 0;
    tridiagonal = direct.useSpBox(2, 3);
    pentadiagonal = direct.useSpBox(3, 3);

    verifySparse(testCase, tridiagonal, 3, 2, 2 * ones(1, 5), 7);
    verifySparse(testCase, pentadiagonal, 3, 3, 3 * ones(1, 3), 7);
    veriDisGraVar(testCase, tridiagonal, 1:5);
    veriDisGraVar(testCase, pentadiagonal, 1:3);
end

function test_tensor_corners_identify_both_axes_without(testCase)
    % Tensor corners identify both axes without relying on flattened adjacency.
    P = pdvar(1, {[0 1], [0 1]}, Degree=[4 4]);
    direct = P >= 0;
    C = direct.useSpBox(2, 2);
...
function test_width_becomes_dense_only_it_saturates(testCase)
    % Width becomes dense only after it saturates every anisotropic axis.
    grid = {[0 1], [10 20]};
    P = pdvar(1, grid, Degree=[2 6]);
    direct = P >= 0;
    sparse = direct.useSpBox(2);
    dense = direct.useSpBox(4);
