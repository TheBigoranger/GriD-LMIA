function tests = test_use_full_box
    % Behavioral regressions for pdlmi.use_full_box.
...
function test_dimensional_tensor_defaults_documented_formulas(testCase)
    ...
    boxApplied = pdlmi(boxExpr, ">=").useFullBox();

    testCase.verifyEqual(box.FullBoxOrder, [2 2]);
    testCase.verifyEqual(boxApplied.FullBoxOrder, [2 2]);
end

function test_explicit_absolute_orders_retained_while_insufficient(testCase)
    ...
    applied = pdlmi(expr, ">=").useFullBox(2);
    ...
    base = pdlmi(expr, ">=");
    testCase.verifyError(@() base.useFullBox(1), ...
        "pdlmi:FullBoxOrderTooLow");
    testCase.verifyError(@() base.useFullBox(1.5), ...
        "pdlmi:InvalidFullBoxOrder");
end

function test_odd_interval_certificates_repeat_scalar_endpoint(testCase)
...
function test_each_cell_rate_row_entry_owns(testCase)
...
function test_semidefinite_mode_still_lifts_matrix_dimension(testCase)
...
function test_each_gram_block_prod_order_mask(testCase)
    ...
    direct = P >= 0;
    testCase.verifyError(@() direct.useFullBox([0 2]), ...
        "pdlmi:FullBoxOrderTooLow");
    bad = {[], [1 2 3], [1 2; 3 4], -1, 0.5, Inf, NaN};
    for k = 1:numel(bad)
        testCase.verifyError(@() direct.useFullBox(bad{k}), ...
            "pdlmi:InvalidFullBoxOrder");
    end
end

function test_masks_order_mask_below_zero_absent(testCase)
