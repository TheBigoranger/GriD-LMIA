function tests = test_value
    % Behavioral regressions for pdvar.value.
    tests = functiontests(localfunctions);
end

function test_assigned_tensor_derivative_elevation_exports_all_rate_controls(testCase)
    % Explicit polynomial coefficients give an independent numeric recovery oracle.
    grid = {[0 2 5],[-3 1 6]};
    P = pdvar(2,3,grid,'full',Degree=[0 2]);
    A = [1 2 -3;5 -7 11]; B = [-2 3 5;7 11 -13]; C = [2 -5 7;11 -17 19];
    nodes = [-3 1 6];
    for j = 1:2
        c = P.coeffs([1 j]);
        lo = nodes(j); hi = nodes(j+1);
        assign(c{1},A+B*lo+C*lo^2);
        assign(c{2},A+B*(lo+hi)/2+C*lo*hi);
        assign(c{3},A+B*hi+C*hi^2);
    end
    D = elevate(rhodiff(P,[3 3;-2 5]),[1 1]);
    rows = value(D);
    ...
    for row = 1:2
        ...
        for i = 1:2
            for j = 1:2
                ...
                for first = 0:1
                    for second = 0:3
                        y = nodes(j)+(nodes(j+1)-nodes(j))*second/3;
                        expected = rates(row)*(B+2*C*y);
                        testCase.verifyEqual(c{first*4+second+1},expected,AbsTol=1e-10);
                    end
                end
            end
        end
...
function test_ordinary_assigned_decisions_complete_bernstein(testCase)
...
function test_proven_symbolic_cancellations_become_ordinary(testCase)
...
function test_unsolved_yalmip_decisions_leak_nan_payloads(testCase)
...
function test_ratebounds_metadata_alone_does_make_ordinary(testCase)
...
function test_scalar_rate_rows_split_into_lower(testCase)
...
function test_fixed_rate_exports_pdmat_while_preserving(testCase)
...
function test_tensor_rate_cells_helper_combrows_lower(testCase)
...
function test_assigned_values_stale_source_metadata_determine(testCase)
...
function test_assigned_cancellation_can_recover_continuity(testCase)
...
function test_value_full_tensor_scale_aware_continuity(testCase)
...
function test_partial_assignment_and_reassignment(testCase)
    P=pdvar(1,[0 2 5],Degree=1);
    c=P.coeffs(1); last=P.coeffs(2);
    assign(c{1},2);
    testCase.verifyError(@() value(P),'pdvar:UnassignedValue');
    assign([c{:} last{2}],[2 5 11]);
    first=value(P);
    testCase.verifyEqual(first.coeffs(1),{2,5});
    testCase.verifyEqual(first.coeffs(2),{5,11});
    assign([c{:} last{2}],[-3 7 19]);
    second=value(P);
    testCase.verifyEqual(second.coeffs(1),{-3,7});
    testCase.verifyEqual(second.coeffs(2),{7,19});
    testCase.verifyEqual(first.coeffs(2),{5,11});
end
