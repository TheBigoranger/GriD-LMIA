function tests = test_tril
    % Public pdvar.tril behavioral contracts.
    tests = functiontests(localfunctions);
end

function test_tensor_upper_offset_mask_mixed(testCase)
...
function test_all_cells_and_rate_rows(testCase)
    ...
    tests.infrastructure.verify_transform(testCase, "pdvar", @(x) tril(x, -1));
end

function test_invalid_arguments_leave_source_unchanged(testCase)
    A=tests.infrastructure.fixture("pdvar",true);
    saved=A.LocalValues;
    testCase.verifyError(@() tril(A,0.5),'pdvar:InvalidTriangularPart');
