function tests = test_mean
    % Public pdbase.mean behavioral contracts.
...
function test_all_cells_and_rate_rows(testCase)
    ...
    tests.infrastructure.verify_transform(testCase, "pdbase", @(x) mean(x, 1));
end

function test_invalid_arguments_leave_source_unchanged(testCase)
    A=tests.infrastructure.fixture("pdbase",true);
    saved=A.LocalValues;
    testCase.verifyError(@() mean(A,[1 1]),'pdbase:InvalidMean');
