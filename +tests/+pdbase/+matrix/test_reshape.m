function tests = test_reshape
    % Public pdbase.reshape behavioral contracts.
...
function test_all_cells_and_rate_rows(testCase)
    ...
    tests.infrastructure.verify_transform(testCase, "pdbase", @(x) reshape(x, 1, 4));
end

function test_invalid_arguments_leave_source_unchanged(testCase)
    A=tests.infrastructure.fixture("pdbase",true);
    saved=A.LocalValues;
    testCase.verifyError(@() reshape(A,3,3),'pdbase:InvalidReshape');
