function tests = test_rot90
    % Public pdbase.rot90 behavioral contracts.
...
function test_all_cells_and_rate_rows(testCase)
    ...
    tests.infrastructure.verify_transform(testCase, "pdbase", @(x) rot90(x, 3));
end

function test_invalid_arguments_leave_source_unchanged(testCase)
    A=tests.infrastructure.fixture("pdbase",true);
    saved=A.LocalValues;
    testCase.verifyError(@() rot90(A,0.5),'pdbase:InvalidRot90');
