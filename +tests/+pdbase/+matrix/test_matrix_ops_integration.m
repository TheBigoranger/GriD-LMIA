function tests = test_matrix_ops_integration
    % Behavioral regressions for pdbase.matrix_ops_integration.
    tests = functiontests(localfunctions);
end

function test_mixed_rate_tensor_composed_matrix_maps(testCase)
    % Composition crosses reduction and reshape boundaries without mixing rows.
    tests.infrastructure.verify_tensor_transform(testCase,"pdbase", ...
        @(x) reshape(cumsum(rot90(x,-1),2,'reverse'),2,3), "mixed");
end

function test_unary_family_preserve_rate_row_bernstein(testCase)
...
function test_legacy_affine_payloads_fields_parameter_rate(testCase)
...
function test_vecdim_reductions_map_each_rate_row(testCase)
...
function test_reverse_accumulation_coefficient_local_dim_2(testCase)
...
function test_valid_2_d_transforms_preserve_order(testCase)
...
function test_matrix_diagonals_map_directly_while_empty(testCase)
...
function test_trace_scalar_vectors_unsupported_reduction_flags(testCase)
