function tests = test_tril
    % Public pdbase.tril behavioral contracts.
...
function test_all_cells_and_rate_rows(testCase)
    ...
    tests.infrastructure.verify_transform(testCase, "pdbase", @(x) tril(x, -1));
end

function test_invalid_arguments_leave_source_unchanged(testCase)
    A=tests.infrastructure.fixture("pdbase",true);
    saved=A.LocalValues;
    testCase.verifyError(@() tril(A,0.5),'pdbase:InvalidTriangularPart');
