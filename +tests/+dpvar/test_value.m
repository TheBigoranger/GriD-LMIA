function tests = test_value
    %TEST_VALUE Assigned dpvar conversion to coefficient-backed dpmat data.
...
function testUnassignedCoefficientFailsClearly(testCase)
    % Unsolved YALMIP decisions must not leak NaN payloads into dpmat.
    P = dpvar(1, {[0 1]}, Degree=2);

    testCase.verifyError(@() value(P), "dpvar:UnassignedValue");
end

function testOrdinaryRateMetadataStillReturnsOneDpmat(testCase)
    % RateBounds metadata alone does not make ordinary coefficients rate rows.
    P = dpvar(1, {[0 1]}, Degree=2, RateBounds=[-1 2]);
    cp = P.coeffs(1);
...
function testScalarDerivativeReturnsOrderedDpmatCells(testCase)
    % Scalar rate rows split into lower/upper dpmat outputs without warnings.
    P = dpvar(1, {[0 2]}, Degree=2);
    ...
    verifyRateDpmat(testCase, rows{1}, 1, [-6, -10]);
    verifyRateDpmat(testCase, rows{2}, 1, [9, 15]);
end
...
function testTensorDerivativeReturnsCombRowsOrder(testCase)
    ...
    for k = 1:4
        verifyRateDpmat(testCase, rows{k}, 2, ...
...
function verifyNumericCoeffs(testCase, actual, expected)
    % Numeric dpmat coefficients should retain local order and payload values.
...
function verifyRateDpmat(testCase, obj, degree, expected)
    % Converted derivative rows are known, discontinuous, and rate-metadata free.
    testCase.verifyClass(obj, "dpmat");
