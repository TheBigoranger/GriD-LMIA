function tests = test_constructor
    % Behavioral regressions for pdmat.constructor.
    tests = functiontests(localfunctions);
end
function test_certified_tensor_function_survives_composed_coefficient_use(testCase)
    % Evaluating a transformed object prevents its retained handle masking bad fits.
    f = @(x,y,z) [y^2+z, y*z; 2-y^2, 3*z-y];
    grid = {[0 2 5], [-1 0 3], [4 6 9]};
    for mode = ["fast", "strict"]
        A = pdmat(grid, f, Degree=[0 2 1], ...
            RateBounds=[3 3; -2 5; 0 0], ValidationMode=mode);
        B = sum(A,2);
        testCase.verifyEmpty(B.FunctionHandle);
        for x = [0 2 5]
            for y = [-1 -.5 0 1.5 3]
                for z = [4 5 6 7.5 9]
                    testCase.verifyEqual(B.evaluate([x y z]), ...
                        sum(f(x,y,z),2), AbsTol=1e-10);
                end
            end
        end
    end
...
function test_function_backed_construction_exact_handle(testCase)
...
function test_explicit_degree_tensor_functions_populate_bernstein(testCase)
...
function test_polynomial_handles_become_coefficient_backed_when(testCase)
...
function test_each_cell_order_fitted_controls_physical(testCase)
...
function test_numeric_fallback_certify_matrix_polynomials_cell(testCase)
...
function test_function_only_construction_probe_size_without(testCase)
...
function test_scalar_global_bernstein_data_form_overlapping(testCase)
...
function test_tensor_global_cell_data_preserve_grid(testCase)
...
function test_public_global_grid_construction_anisotropic_matrix(testCase)
...
function test_misaligned_nested_localvalues_warn_without_changing(testCase)
...
function test_3_by_3_global_degree_grid(testCase)
...
function test_explicit_tensor_local_values_continuous_when(testCase)
...
function test_faces_package_s_scale_aware_numerical(testCase)
...
function test_only_explicit_multidimensional_scalar_degree(testCase)
...
function test_global_shape_infers_each_axis_flat(testCase)
...
function test_global_grids_accept_implicit_singleton_axes(testCase)
...
function test_constructor_reject_unsupported_source_payload_types(testCase)
    % Constructor should reject unsupported source payload types.
    testCase.verifyError(@() pdmat({[0 1]}, eye(2)), "pdmat:InvalidSource");
end

function test_global_coefficient_grids_imply_degree_compatible(testCase)
...
function test_global_payloads_share_finite_real_matrix(testCase)
...
function test_explicit_localvalues_validate_nesting_degree_payload(testCase)
...
function test_function_handles_match_parameter_arity_return(testCase)
...
function test_explicit_degree_function_data_reject(testCase)
...
function test_over_degree_matrix_entry_reject_complete(testCase)
...
function test_numeric_probing_fallback_still_reject_non(testCase)
...
function test_bernstein_probing_reject_handles_inconsistent_output(testCase)
...
function test_fixed_representation_flags_stay_unavailable_while(testCase)
...
function test_constructor_option_syntax_rejects_missing_nontext(testCase)
...
function test_callback_failures_normalized_at_lower_bound(testCase)
...
function test_explicit_degree_rejects_empty_non_scalar(testCase)
...
function test_explicit_leaves_row_or_row_distinct(testCase)
...
function test_mode_selection_transient_leaves_represented_data(testCase)
...
function test_source_normalization_reject_malformed_later_cells(testCase)
...
function test_pdmat_owns_all_validationmode_diagnostics_at(testCase)
...
function test_slicing_away_only_discontinuous_entry_restores(testCase)
