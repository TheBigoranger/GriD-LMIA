function out = reshape(obj, varargin)
    %RESHAPE Reshape each coefficient payload of a dpvar expression.
    %
    %   Syntax:
    %     Q = reshape(P, m, n)
    %     Q = reshape(P, [m n])
...
function sz = parseSz(args, total)
    ...
    if numel(raw) ~= 2
        error("dpvar:InvalidReshape", ...
            "dpvar reshape expects exactly two matrix dimensions.");
    end
    ...
    for k = 1:2
        ...
        sz(k) = helper.chk(raw{k}, "dpvar:InvalidReshape", ...
