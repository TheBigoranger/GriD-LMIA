function varargout = subsref(obj, S)
    %SUBSREF Matrix block indexing for dpvar while preserving dot access.
    %
    %   Syntax:
    %     Q = P(rows, cols)
    %     c = P.coeffs(cellSubs)
    ...
    if strcmp(S(1).type, "()")
        ...
        vals = helper.mapVals(obj.LocalValues, @(a) a(rows, cols), ...
            obj.GridInfo.Vectors);
        out = dpvar(mkInit(obj.GridInfo.Vectors, [numel(rows), numel(cols)], ...
