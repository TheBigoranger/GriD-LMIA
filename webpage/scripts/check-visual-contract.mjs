...
check("v0.4.4 public metadata and final-patch history", /current:\s*"v0\.4\.4"/.test(versionData) && /Version 0\.4\.4/.test(pdlmiOverview) && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 4 && ["v0.1.0", "v0.2.12", "v0.3.6", "v0.4.4"].every((version) => versionHistory.includes(`version: "${version}"`)) && ["63c06ee", "98b63e5", "fe74a65"].every((commit) => versionHistory.includes(commit)) && !["v0.2.0", "v0.3.0", "v0.3.5", "v0.4.1", "v0.4.2", "v0.4.3"].some((version) => versionHistory.includes(`version: "${version}"`)) && /version:\s*"v0\.4\.4"[\s\S]*date:\s*"2026-07-18"/.test(versionHistory));
