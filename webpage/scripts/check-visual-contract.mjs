...
const interfaceRegularity = component("InterfaceRegularityDiagram.astro");
...
check("five-node forward-only journey", (journey.match(/<circle cx=/g)?.length ?? 0) === 5 && ["Model the general DPD-LMI", "Grid and impose interface regularity", "Expand in the Bernstein basis", "Select a certificate", "Export, solve, recover"].every((label) => journey.includes(label)) && /general continuum DPD-LMI/.test(journey) && /unequal one-sided derivatives/.test(journey) && /cell-local Bernstein expansion/.test(journey) && /#general-dpd-lmi-template/.test(journey) && /marker-end: url\(#journey-arrow\)/.test(journey) && !/feedback|journey-return|solved matrix|LPV induced-L2-gain model|Reduce rates and grid|Pull back and assemble coefficients/.test(journey));
...
check("five welcome stages and required mathematics", stepCount === 5 && ["Model the general DPD-LMI problem", "titleMath: \"\\\\mathcal P\"", " and impose interface regularity", "Expand each cell in the Bernstein basis", "Select a finite certificate", "Export, solve, and recover the decision matrix"].every((label) => home.includes(label)) && ["\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N", "\\\\boldsymbol y(\\\\boldsymbol\\\\rho)=\\\\bigl(y_1(\\\\boldsymbol\\\\rho),\\\\ldots,y_N(\\\\boldsymbol\\\\rho)\\\\bigr)^\\\\top", "\\\\sum_{k=1}^{N}\\\\sum_{s=1}^{\\\\ell}T_{k,s}(\\\\boldsymbol\\\\rho)\\\\frac{\\\\partial y_k}{\\\\partial\\\\rho_s}(\\\\boldsymbol\\\\rho)", "F_0(\\\\boldsymbol\\\\rho)+\\\\sum_{k=1}^{N}F_k(\\\\boldsymbol\\\\rho)y_k(\\\\boldsymbol\\\\rho)\\\\prec0", "\\\\forall\\\\boldsymbol\\\\rho\\\\in\\\\mathcal P", "\\\\mathcal H_{\\\\mathbf c}", "\\\\boldsymbol\\\\alpha=\\\\phi_{\\\\mathbf c}", "y^{(\\\\mathbf c)}", "S^{(\\\\mathbf c)}", "toYalmip", "optimize", "diagnostics.problem == 0", "value(P)"].every((fragment) => home.includes(fragment)));
check("welcome uses one inline definition and one displayed paper Eq. (5)", firstHomeStep.includes('inlineMath: "\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N,\\\\qquad \\\\boldsymbol y(\\\\boldsymbol\\\\rho)=\\\\bigl(y_1(\\\\boldsymbol\\\\rho),\\\\ldots,y_N(\\\\boldsymbol\\\\rho)\\\\bigr)^\\\\top."') && firstHomeStep.includes('math: "\\\\mathcal F(\\\\boldsymbol\\\\rho)=\\\\sum_{k=1}^{N}\\\\sum_{s=1}^{\\\\ell}T_{k,s}(\\\\boldsymbol\\\\rho)\\\\frac{\\\\partial y_k}{\\\\partial\\\\rho_s}(\\\\boldsymbol\\\\rho)+F_0(\\\\boldsymbol\\\\rho)+\\\\sum_{k=1}^{N}F_k(\\\\boldsymbol\\\\rho)y_k(\\\\boldsymbol\\\\rho)\\\\prec0,\\\\qquad\\\\forall\\\\boldsymbol\\\\rho\\\\in\\\\mathcal P."') && (firstHomeStep.match(/\binlineMath:/g)?.length ?? 0) === 1 && (firstHomeStep.match(/\bmath:/g)?.length ?? 0) === 1 && !/math:\s*\[|text(?:Html)?:|\\\\text\{|Y_1|\\\\mathcal F_a|S_a|negative semidefinite|optional bounded rate box|\\\\mathcal P\\\\times\\\\mathcal R|\\\\mathsf T/.test(firstHomeStep) && /#general-dpd-lmi-template/.test(firstHomeStep));
check("welcome grids first, then explains interface regularity", secondHomeStep.includes('text: "Grid the parameter domain into axis-aligned cells."') && secondHomeStep.includes('math: "\\\\mathcal G_s=\\\\{\\\\rho_s^{(1)},\\\\ldots,\\\\rho_s^{(k_s)}\\\\},\\\\qquad\\\\mathcal H_{\\\\mathbf c}=\\\\prod_{s=1}^{\\\\ell}[\\\\rho_s^{(c_s)},\\\\rho_s^{(c_s+1)}]."') && secondHomeStep.includes('regularityText: "Across each shared boundary, decision values must remain continuous, but adjacent one-sided derivatives may differ."') && (secondHomeStep.match(/\bmath:/g)?.length ?? 0) === 1 && !/math:\s*\[|\\\\mathcal P=\\\\prod|\\\\mathcal R=\\\\prod|\\\\lim_/.test(secondHomeStep) && /visual: "regularity"/.test(secondHomeStep) && /InterfaceRegularityDiagram/.test(home) && /number: "03"[\s\S]*visual: "storage"/.test(home));
check("shared-boundary regularity visual", /continuous piecewise-polynomial decision variable/.test(interfaceRegularity) && /unequal one-sided derivatives/.test(interfaceRegularity) && /shared value/.test(interfaceRegularity) && /left limiting slope/.test(interfaceRegularity) && /right limiting slope/.test(interfaceRegularity) && /role="img"/.test(interfaceRegularity) && /\.axis\s*\{[^}]*stroke-width:\s*1\.25/.test(interfaceRegularity) && /\.boundary\s*\{[^}]*stroke-width:\s*1/.test(interfaceRegularity) && /\.cell\s*\{[^}]*stroke-width:\s*3\.5/.test(interfaceRegularity) && /\.tangent\s*\{[^}]*stroke-width:\s*2[^}]*stroke-dasharray:\s*7 5/.test(interfaceRegularity) && /\.shared-value\s*\{[^}]*stroke-width:\s*2/.test(interfaceRegularity) && /x="220" y="105"[\s\S]*left limiting slope/.test(interfaceRegularity) && /x="520" y="122"[\s\S]*right limiting slope/.test(interfaceRegularity) && /x="420" y="78"[\s\S]*shared value/.test(interfaceRegularity));
...
check("compact welcome math remains mobile-safe", /step-inline-math[\s\S]*renderInlineMath\(step\.inlineMath\)/.test(home) && /\.step-inline-math\s*\{[^}]*overflow-x:\s*auto[^}]*white-space:\s*nowrap/.test(home) && !/math:\s*\[/.test(firstHomeStep + secondHomeStep) && (home.match(/math: \[/g)?.length ?? 0) >= 1 && /\.math-strip\s*\{[^}]*min-width:\s*0/.test(home) && /item\.formula\.map\(renderDisplayMath\)/.test(chooserWrapper) && /option\.mathHtml\.map/.test(chooser) && /key:\s*"putinar"[\s\S]*?formula:\s*\[/.test(certData) && !/key:\s*"putinar"[^\n]*\\qquad/.test(certData));
