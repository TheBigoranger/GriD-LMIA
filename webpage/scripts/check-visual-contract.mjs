...
check("five-node forward-only journey", (journey.match(/<circle cx=/g)?.length ?? 0) === 5 && ["Form the DPD-LMI", "Grid the ", "Build constraints in each hypercube", "Select a certificate", "Export, solve, recover"].every((label) => journey.includes(label)) && /math: "\\\\rho"/.test(journey) && /marker-end: url\(#journey-arrow\)/.test(journey) && !/feedback|journey-return|solved matrix/.test(journey));
...
check("shared certificate detail navigation", [pdlmiCtor, pdlmiPolya, pdlmiPutinar, pdlmiSparseFullbox, pdlmiFullbox].every((source) => /CertificateNavigation/.test(source) && /showSections=\{false\}/.test(source)) && [[pdlmiCtor, "direct"], [pdlmiPolya, "polya"], [pdlmiPutinar, "putinar"], [pdlmiSparseFullbox, "sparsefullbox"], [pdlmiFullbox, "fullbox"]].every(([source, selected]) => new RegExp(`selected="${selected}"`).test(source)) && ["direct", "polya", "putinar", "sparsefullbox", "fullbox"].every((selected) => new RegExp(`selected="${selected}"`).test(sosMath)));
...
check("certificate flow tabs on welcome and interactive reference overview", /CertificateFlow/.test(home) && /CertificateFlow/.test(certificateFlow) && /Stored residual/.test(certificateFlowTabs) && /role="tablist"/.test(certificateFlowTabs) && /role="tabpanel"/.test(certificateFlowTabs) && /ArrowLeft[\s\S]*ArrowRight[\s\S]*Home[\s\S]*End/.test(certificateFlowTabs) && /Open the \{option\.label\} reference/.test(certificateFlowTabs) && /CertificateChooser/.test(pdlmiOverview) && /useState\(0\)/.test(chooser) && /role="tablist"/.test(chooser) && /selected\.toYalmip\(\)/.test(chooser));
check("Pólya card presents transform before coefficient test and index set", certData.includes('cardFormula: "\\\\widetilde F^{(\\\\mathbf c,v)}(\\\\boldsymbol\\\\alpha)=F^{(\\\\mathbf c,v)}(\\\\boldsymbol\\\\alpha)\\\\prod_{s=1}^{\\\\ell}') && certificateFlow.includes('polya: "\\\\begin{gathered}\\\\widetilde C') && certificateFlow.indexOf("\\\\widetilde C") < certificateFlow.indexOf("\\\\mathcal I_{m+p}=") && certificateFlowTabs.indexOf('className="certificate-flow-panel__formula"') < certificateFlowTabs.indexOf('className="certificate-flow-panel__notation"') && certificateFlowTabs.indexOf("option.formula") < certificateFlowTabs.indexOf("option.notation"));
check("equal-height compact certificate tabs and readout", /grid-auto-rows:\s*1fr/.test(manualCss) && /block-size:\s*5rem/.test(manualCss) && /certificate-shape-readout p\s*\{[^}]*min-block-size:\s*4\.25rem[^}]*margin:\s*0/.test(manualCss) && !/certificate-default|>Default</.test(chooser) && !/showDefault|default-marker/.test(certificate));
check("shared certificate navigation contract", ["direct", "polya", "putinar", "sparse-full-box", "full-box"].every((anchor) => certData.includes(`anchor: "${anchor}"`)) && ["L", "L.applyPolya(p)", "L.applyPutinar()", "L.applySparseFullBoxPreorder()", "L.applyFullBoxPreorder()"].every((command) => certData.includes(`command: "${command}"`)) && /CertificateNavigation/.test(pdlmiOverview) && /showSections \? `#\$\{item\.anchor\}`/.test(certificateNavigation) && /item\.detailRoute/.test(certificateNavigation) && /aria-current=\{isSelected \? "page"/.test(certificateNavigation));
check("five certificate tabs and sparse endpoint controls", /repeat\(5, minmax\(0, 1fr\)\)/.test(manualCss) && /bandWidth/.test(chooser) && /effectiveSelector/.test(chooser) && /ArrowLeft[\s\S]*ArrowRight[\s\S]*Home[\s\S]*End/.test(chooser) && /width one[\s\S]*Direct/i.test(pdlmiSparseFullbox) && /(?:r|SparseFullBoxOrder) \+ 1[\s\S]*Full Box/i.test(pdlmiSparseFullbox));
...
check("cell-indexed DPD-LMI workflow", ["\\\\mathcal F(\\\\rho,\\\\dot\\\\rho)", "\\\\mathcal H_{\\\\mathbf c}=", "renderInlineMath(\"\\\\mathcal V=\\\\prod_{s=1}^{\\\\ell}", "F^{(\\\\mathbf c,v)}(\\\\boldsymbol\\\\alpha)\\\\preceq0\\\\quad\\\\forall\\\\,\\\\mathbf c,\\\\;\\\\forall\\\\,v\\\\in\\\\mathcal V,\\\\;\\\\forall\\\\,\\\\boldsymbol\\\\alpha\\\\in[0,1]^\\\\ell.", "selected.toYalmip()"].every((fragment) => home.includes(fragment)) && /every cell-and-rate pair/.test(home) && !/\\\\begin\{gathered\}F\^\{\(\\\\mathbf c,v\)\}/.test(home));
...
check("nested operation and certificate sidebars", [/label: "pdmat"[\s\S]*label: "Matrix Operations"[\s\S]*items: \[[\s\S]*documents\/reference\/pdmat\/matrix-operations/, /label: "pdvar"[\s\S]*label: "Matrix Operations"[\s\S]*items: \[[\s\S]*documents\/reference\/pdvar\/matrix-operations/, /label: "pdlmi"[\s\S]*label: "toYalmip"[\s\S]*label: "Certificates"[\s\S]*label: "Pólya"[\s\S]*label: "Putinar"[\s\S]*label: "Sparse Full Box"[\s\S]*label: "Full Box"/].every((pattern) => pattern.test(astroConfig)));
...
check("v1.1.0 public metadata with retained v1.0 line", /current:\s*"v1\.1\.0"/.test(versionData) && /Version 1\.1\.0/.test(pdlmiOverview) && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 6 && ["v0.1.0", "v0.2.12", "v0.3.6", "v0.4.7", "v1.0.0", "v1.1.0"].every((version) => versionHistory.includes(`version: "${version}"`)) && ["63c06ee", "98b63e5", "fe74a65", "c6a2e25", "6d19619"].every((commit) => versionHistory.includes(commit)) && /version:\s*"v1\.1\.0"[\s\S]*date:\s*"2026-07-26"/.test(versionHistory) && /version:\s*"v1\.0\.0"[\s\S]*date:\s*"2026-07-19"[\s\S]*6d19619/.test(versionHistory));
