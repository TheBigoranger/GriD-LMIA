...
check("five welcome stages and required mathematics", stepCount === 5 && ["Model the general DPD-LMI problem", "Partition the supported parameter box", "Fix one cell and use the Bernstein basis", "Select a finite certificate for the positive target", "Assemble the panorama, solve, and validate status"].every((label) => home.includes(label)) && ["\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N", "\\\\dot\\\\rho_sT_{k,s}", "\\\\mathcal P=\\\\prod", "\\\\mathcal R=\\\\prod", "\\\\mathcal H_{\\\\mathbf c}", "\\\\boldsymbol\\\\alpha=\\\\phi_{\\\\mathbf c}^{-1}", "B_{\\\\mathbf i}^{\\\\mathbf m}", "S^{(\\\\mathbf c)}", "toYalmip", "optimize", "sol.problem == 0", "value(P)"].every((fragment) => home.includes(fragment)));
check("welcome states general DPD-LMI and one L2 link", firstHomeStep.includes('inlineMath: "\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N,\\\\qquad \\\\boldsymbol y(\\\\boldsymbol\\\\rho)=\\\\bigl(y_1(\\\\boldsymbol\\\\rho),\\\\ldots,y_N(\\\\boldsymbol\\\\rho)\\\\bigr)^\\\\top."') && /\\\\mathcal F\(\\\\boldsymbol\\\\rho,\\\\dot\{\\\\boldsymbol\\\\rho\};y\)/.test(firstHomeStep) && /\\\\dot\\\\rho_sT_\{k,s\}[\s\S]*\\\\frac\{\\\\partial y_k\}\{\\\\partial\\\\rho_s\}/.test(firstHomeStep) && /Induced-\$\{renderInlineMath\("L_2"\)\} gain is one representative application/.test(firstHomeStep) && (firstHomeStep.match(/\binlineMath:/g)?.length ?? 0) === 1 && (firstHomeStep.match(/\bmath:/g)?.length ?? 0) === 1 && /#general-dpd-lmi-template/.test(firstHomeStep));
check("welcome separates exact partition and approximation", /tensor grid partitions/.test(secondHomeStep) && /exactly/.test(secondHomeStep) && /approximation enters only through the restricted decision-function space/.test(secondHomeStep) && /affine rate-vertex reduction is a separate exact step/.test(secondHomeStep) && /visual: "grid"/.test(secondHomeStep) && /GridPartitionExplorer client:visible/.test(home));
check("accessible fitted nonuniform grid explorer", /role="tablist"/.test(gridExplorer) && /ArrowLeft[\s\S]*ArrowRight[\s\S]*Home[\s\S]*End/.test(gridExplorer) && /1: \[0\.35\][\s\S]*2: \[0\.4, 0\.25\][\s\S]*3: \[0\.3, 0\.55, 0\.75\]/.test(gridExplorer) && /type="range"/.test(gridExplorer) && /aria-live="polite"/.test(gridExplorer) && /onPointerDown[\s\S]*onPointerMove[\s\S]*onPointerUp/.test(gridExplorer) && /Rotate left[\s\S]*Rotate right[\s\S]*Tilt up[\s\S]*Tilt down[\s\S]*Reset view/.test(gridExplorer) && /enumerateCells[\s\S]*getCellBounds[\s\S]*projectPoint[\s\S]*fitProjection[\s\S]*mapProjection/.test(gridExplorer) && /KNOT_MIN = 0\.1[\s\S]*KNOT_MAX = 0\.9[\s\S]*PITCH_LIMIT = 70[\s\S]*fitProjection[\s\S]*mapProjection/.test(gridPartition));
...
check("official React islands", /@astrojs\/react/.test(packageJson) && /import react from "@astrojs\/react"/.test(astroConfig) && /react\(\)/.test(astroConfig) && /CellStorageExplorer client:visible/.test(home) && /GridPartitionExplorer client:visible/.test(home) && /client:visible/.test(pdmatEval) && /client:visible/.test(chooserWrapper) && /client:visible/.test(certificateFlow));
check("accessible mathematical explorers", [rateExplorer, elevationExplorer, tensorExplorer].every((source) => /role="alert"/.test(source) && /aria-live="polite"/.test(source)) && /role="alert"/.test(additionExplorer) && /aria-live="polite"/.test(additionExplorer) && /RateVertexExplorer client:visible/.test(pdvarDiff) && /DegreeElevationExplorer client:visible/.test(coefficientAlgebra) && /DegreeElevationExplorer client:visible/.test(page("reference/pdmat/storage-and-elevation.mdx")) && /TensorTraversalExplorer client:visible/.test(griddingMath) && /CertificateChooser/.test(sosMath));
...
check("v1.1.5 documentation and v1.1.0 Release metadata", /current:\s*"v1\.1\.5"/.test(versionData) && /Documentation v1\.1\.5/.test(pdlmiOverview) && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 7 && ["v0.1.0", "v0.2.12", "v0.3.6", "v0.4.7", "v1.0.0", "v1.1.0", "v1.1.5"].every((version) => versionHistory.includes(`version: "${version}"`)) && /version:\s*"v1\.1\.5"[\s\S]*status:\s*"current documentation snapshot"/.test(versionHistory) && /version:\s*"v1\.1\.0"[\s\S]*status:\s*"latest GitHub Release"/.test(versionHistory) && /No v1\.1\.5 tag or GitHub Release exists/.test(versionPage) && /Latest GitHub Release \(v1\.1\.0\)/.test(install) && /v1\.1\.0 release gate completed 280 of 280 tests/.test(install) && /version:\s*"v1\.0\.0"[\s\S]*date:\s*"2026-07-19"[\s\S]*6d19619/.test(versionHistory));
