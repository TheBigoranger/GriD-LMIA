...
check("five welcome stages and required mathematics", stepCount === 5 && ["Model the general DPD-LMI problem", "Partition the supported parameter box", "Fix one cell and use the Bernstein basis", "Select a finite certificate for the positive target", "Assemble the panorama, solve, and validate status"].every((label) => home.includes(label)) && ["\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N", "\\\\mathcal F_{\\\\mathrm{alg}}", "\\\\mathcal F_{\\\\mathrm{rate}}", "\\\\dot\\\\rho_sT_{k,s}", "\\\\mathcal P&=\\\\prod", "\\\\mathcal R&=\\\\prod", "\\\\mathcal H_{\\\\mathbf c}&:=", "\\\\boldsymbol\\\\alpha&=\\\\phi_{\\\\mathbf c}^{-1}", "B_{\\\\mathbf i}^{\\\\mathbf m}", "S^{(\\\\mathbf c)}", "toYalmip", "optimize", "sol.problem == 0", "value(P)"].every((fragment) => home.includes(fragment)));
check("welcome states general DPD-LMI then one compact decision map and one L2 link", firstHomeStep.includes('inlineMath: "\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N."') && /inlineNoteHtml:[^\n]*decision function[^\n]*locally Lipschitz continuous/.test(firstHomeStep) && /\\\\mathcal F_\{\\\\mathrm\{alg\}\}[\s\S]*\\\\mathcal F_\{\\\\mathrm\{rate\}\}[\s\S]*\\\\mathcal F&=/.test(firstHomeStep) && /\\\\dot\\\\rho_sT_\{k,s\}[\s\S]*\\\\frac\{\\\\partial y_k\}\{\\\\partial\\\\rho_s\}/.test(firstHomeStep) && !/\btextHtml:/.test(firstHomeStep) && /manualLabel: "Open the induced-L₂-gain specialization ↗"/.test(firstHomeStep) && (firstHomeStep.match(/dpd-lmi-and-lpv-l2-gain/g)?.length ?? 0) === 1 && (firstHomeStep.match(/\binlineMath:/g)?.length ?? 0) === 1 && (firstHomeStep.match(/\bmath:/g)?.length ?? 0) === 1 && /step\.visual === "export"[\s\S]*step\.inlineMath[\s\S]*step\.inlineNoteHtml/.test(home));
check("welcome separates exact partition and approximation between split displays", /tensor grid partitions/.test(secondHomeStep) && /exactly/.test(secondHomeStep) && /Approximation comes only from restricting the decision-function space/.test(secondHomeStep) && /affine rate-vertex reduction remains exact/.test(secondHomeStep) && /visual: "grid"/.test(secondHomeStep) && /step\.math\.slice\(0, 1\)[\s\S]*step\.textHtml[\s\S]*step\.math\.slice\(1\)/.test(home) && /GridPartitionExplorer client:visible/.test(home));
...
check("pdmat storage diagram links geometry to public accessors", /href="#pdmat-cells"[\s\S]*href="#pdmat-coeffs"[\s\S]*href="#pdmat-lbls"[\s\S]*href="#pdmat-ncoeff"/.test(component("CellStorageDiagram.astro")) && /cells\(A\)[\s\S]*coeffs\(A,cellSubscript\)[\s\S]*lbls\(A\)[\s\S]*ncell[^]*ncoeff[^]*npar/.test(pdmatStorage));
check("pdmat elevate follows the appendix definition, kernel, and tensor convolution", /## What degree elevation does/.test(pdmatElevate) && /\\widehat C\^\{\(\\boldsymbol c\)\}\[\\boldsymbol k\][\s\S]*\\binom\{M_s-m_s\}\{k_s-i_s\}/.test(pdmatElevate) && /\\mathcal E_\{\\boldsymbol M-\\boldsymbol m\}\[\\boldsymbol j\][\s\S]*\\prod_\{s=1\}\^\{\\ell\}[\s\S]*\\binom\{M_s-m_s\}\{j_s\}/.test(pdmatElevate) && /\\ast_\{\\ell\}/.test(pdmatElevate) && !/W_\{\\boldsymbol i\}|K_\{\\boldsymbol j\}|\\mathcal I\(\\boldsymbol k\)/.test(pdmatElevate) && /### One-dimensional coefficient rule and example[\s\S]*\\widetilde a_i[\s\S]*A = pdmat/.test(pdmatElevate) && pdmatElevate.includes("`ncoeff(A)` increases") && /not grid refinement, interpolation, or an approximation step/.test(pdmatElevate));
...
check("dedicated rhodiff detail reuse", /lower\/upper Cartesian-product order[\s\S]*direction-wise decision degree/.test(rhodiff) && [pdvarDiff, pdvarTable, rateInterfaceMath, coefficientAlgebra].every((source) => /RhodiffDiagram/.test(source)));
...
check("official React islands", /@astrojs\/react/.test(packageJson) && /import react from "@astrojs\/react"/.test(astroConfig) && /react\(\)/.test(astroConfig) && /CellStorageExplorer client:visible/.test(home) && /GridPartitionExplorer client:visible/.test(home) && /CellBernsteinExpressionExplorer client:visible/.test(pdmatCtor) && /client:visible/.test(pdmatEval) && /client:visible/.test(chooserWrapper) && /client:visible/.test(certificateFlow));
check("accessible mathematical explorers", [rateExplorer, elevationExplorer, tensorExplorer].every((source) => /role="alert"/.test(source) && /aria-live="polite"/.test(source)) && /role="alert"/.test(additionExplorer) && /aria-live="polite"/.test(additionExplorer) && /RateVertexExplorer client:visible/.test(pdvarDiff) && /DegreeElevationExplorer client:visible/.test(coefficientAlgebra) && /DegreeElevationExplorer client:visible/.test(pdmatElevate) && /TensorTraversalExplorer client:visible/.test(griddingMath) && /CertificateChooser/.test(sosMath));
...
check("storage selection uses HTML labels and inline TeX values", cellExplorer.includes("<strong>Selected hypercube:</strong>") && cellExplorer.includes("renderInlineMath(`c=(${cell.c1},1)`)") && cellExplorer.includes("<strong>Physical domain:</strong>") && cellExplorer.includes("renderInlineMath(cell.domainTex)") && (cellBernstein.match(/domainTex: "\\\\rho_1\\\\in\[/g)?.length ?? 0) === 2);
...
check("certificate flow tabs on welcome and interactive reference overview", /CertificateFlow compact/.test(home) && /compact=\{compact\}/.test(certificateFlow) && /Theoretical positive target/.test(certificateFlowTabs) && /S\^\{\(\\\\mathbf c\)\}\(\\\\boldsymbol\\\\alpha\)\\\\succ0/.test(certificateFlowTabs) && !/F\^\{\(\\\\mathbf c,v\)\}/.test(certificateFlowTabs) && /role="tablist"/.test(certificateFlowTabs) && /role="tabpanel"/.test(certificateFlowTabs) && /ArrowLeft[\s\S]*ArrowRight[\s\S]*Home[\s\S]*End/.test(certificateFlowTabs) && /option\.formula/.test(certificateFlowTabs) && /option\.notation/.test(certificateFlowTabs) && /MATLAB selector/.test(certificateFlowTabs) && /Open the \{option\.label\} reference/.test(certificateFlowTabs) && /CertificateChooser/.test(pdlmiOverview) && /useState\(0\)/.test(chooser) && /role="tablist"/.test(chooser) && /selected\.toYalmip\(\)/.test(chooser));
...
check("compact storage equations use semantic TeX sizing", /import \{ renderDisplayMath, renderInlineMath \}/.test(cellExplorer) && (cellExplorer.match(/renderDisplayMath\(/g)?.length ?? 0) === 2 && !/\\\\footnotesize/.test(cellExplorer) && /\\\\mathcal I_2[\s\S]*\\\\beta_\{\\\\mathbf i\}[\s\S]*T_\{\\\\mathbf i\}/.test(cellExplorer) && /storageMath: "\\\\begin\{aligned\}A\(\\\\rho_1,\\\\rho_2\)&=/.test(home) && !/storageMath: "[^"]*\\\\footnotesize/.test(home) && !/cell-summary__basis \.katex\s*\{/.test(manualCss) && !/cell-bernstein-formula \.katex\s*\{/.test(manualCss) && !/cell-storage-compact \.cell-explorer \.katex\s*\{/.test(manualCss));
...
check("compact welcome math remains mobile-safe", /step-inline-math[\s\S]*renderInlineMath\(step\.inlineMath\)/.test(home) && /\.step-inline-math\s*\{[^}]*min-width:\s*0[^}]*padding-block/.test(home) && !/\.step-inline-math\s*\{[^}]*overflow-x:\s*auto|\.math-strip__row\s*\{[^}]*overflow-x:\s*auto/.test(home) && !/math:\s*\[/.test(firstHomeStep) && /math:\s*\[/.test(secondHomeStep + thirdHomeStep + fourthHomeStep) && /\.math-strip\s*\{[^}]*min-width:\s*0/.test(home) && /item\.formula\.map\(renderDisplayMath\)/.test(chooserWrapper) && /option\.mathHtml\.map/.test(chooser) && /key:\s*"putinar"[\s\S]*?formula:\s*\[/.test(certData) && !/key:\s*"putinar"[^\n]*\\qquad/.test(certData));
check("ordinary formulas use rendered containment instead of scrollbars", /"check:geometry":\s*"node scripts\/check-rendered-geometry\.mjs"/.test(packageJson) && !/\.katex-display\s*\{[^}]*overflow-x:\s*auto/.test(mathStyleSources) && !/cell-(?:summary__formula|summary__basis|basis-expression__formula|basis-readout p)\s*\{[^}]*overflow-x:\s*auto/.test(manualCss));
