...
check("pdmat evaluate gives the general tensor Bernstein formula", /\\mathcal H_\{\\vect c\}[\s\S]*\\prod_\{s=1\}\^\{\\ell\}[\s\S]*\\vect\\rho&\\in\\mathcal H_\{\\vect c\}/.test(pdmatEval) && /\\alpha_s[\s\S]*\\rho_s\^\{\(c_s\)\}/.test(pdmatEval) && /\\vect m=\(m_1,\\ldots,m_\\ell\)/.test(pdmatEval) && /A\^\{\(\\vect c\)\}\(\\vect\\alpha\)[\s\S]*\\sum_\{\\vect i\\in\\mathcal I_\{\\vect m\}=\\prod_\{s=1\}\^\{\\ell\}\\\{0,\\ldots,m_s\\\}\}/.test(pdmatEval) && /b_s\(i_s,\\alpha_s\)[\s\S]*\\binom\{m_s\}\{i_s\}[\s\S]*\\alpha_s\^\{i_s\}\(1-\\alpha_s\)\^\{m_s-i_s\}[\s\S]*B_\{\\vect i\}\^\{\\vect m\}\(\\vect\\alpha\)[\s\S]*\\prod_\{s=1\}\^\{\\ell\}b_s\(i_s,\\alpha_s\)/.test(pdmatEval) && !/For degree-one scalar data/.test(pdmatEval));
...
check("welcome storage uses build-time formula markup wrappers", /import \{ InlineMath \} from "\.\/RenderedMath\.tsx"/.test(cellExplorer) && /<InlineMath[\s\S]*markup=/.test(cellExplorer) && !/\btex=|renderMath|renderToString|dangerouslySetInnerHTML/.test(cellExplorer) && /basis:\s*renderMath\("B_\{\\\\vect i\}\^\{\\\\vect m\}\(\\\\vect\\\\alpha\)=\\\\prod/.test(home) && /bernstein:\s*renderMath\(`\\\\vect m=\(2,2\),\\\\qquad A\^\{\(\$\{cell\.c1\},1\)\}/.test(home) && /storageMath: "A\(\\\\rho_1,\\\\rho_2\)=\\\\begin\{bmatrix\}1\+\\\\rho_1\+\\\\rho_2/.test(home) && !/a_\{11\}|a_\{12\}|a_\{22\}/.test(thirdHomeStep));
...
check("canonical notation", /id="domains-and-rate-vertices"/.test(notationMath) && /id="grids-cells-and-local-coordinates"/.test(notationMath) && /id="bernstein-labels-and-degrees"/.test(notationMath) && /id="residual-and-positive-target"/.test(notationMath) && /E_\{\\vect M\\leftarrow\\vect m\}/.test(coefficientAlgebra) && /\\dot\{\\vect\\rho\}\(t\)\\in\\mathcal R/.test(modelMath));
...
check("source-indexed API inventory matches v1.3.3", referenceEntries.length === 192 && inventoryCounts["pdbase-backend"] === 49 && inventoryCounts.pdmat === 55 && inventoryCounts.pdvar === 52 && inventoryCounts.pdlmi === 22 && inventoryCounts["shared-helpers"] === 13 && inventoryCounts.setup === 1);
...
check("Learn, Reference, and nested API sidebars", /label: "Learn"[\s\S]*Learn And Reference Portal[\s\S]*Task And Example Lookup[\s\S]*label: "Notation"[\s\S]*DPD-LMI And LPV L2-Gain Model[\s\S]*Rate-Box Reduction And Interface Analysis[\s\S]*Gridding And Local Coordinates[\s\S]*Bernstein Basis, Continuity, And Storage[\s\S]*Coefficient Algebra[\s\S]*Certificate Map And Selection Guide[\s\S]*Direct And Pólya[\s\S]*Markov–Lukács And Putinar[\s\S]*SparsePutinar Tensor Windows[\s\S]*SparseFullBox And FullBox[\s\S]*label: "Reference"[\s\S]*Generated API Lookup[\s\S]*label: "Examples"[\s\S]*label: "About"[\s\S]*label: "Thanks"/.test(astroConfig) && !/label: "Math Concepts"/.test(astroConfig) && [/label: "pdmat"[\s\S]*label: "Matrix Operations"[\s\S]*documents\/reference\/pdmat\/matrix-operations/, /label: "pdvar"[\s\S]*label: "Matrix Operations"[\s\S]*documents\/reference\/pdvar\/matrix-operations/, /label: "pdlmi"[\s\S]*label: "Certificates"[\s\S]*label: "Pólya"[\s\S]*label: "Putinar"[\s\S]*label: "SparsePutinar"[\s\S]*label: "SparseFullBox"[\s\S]*label: "FullBox"/].every((pattern) => pattern.test(astroConfig)));
...
check("v1.3.3 current documentation and v1.3.0 latest tagged release", versionData.includes('current: "v1.3.3"') && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 10 && /version:\s*"v1\.3\.3"[\s\S]*2026-08-09[\s\S]*current documentation/.test(versionHistory) && /version:\s*"v1\.3\.0"[\s\S]*latest tagged GitHub Release/.test(versionHistory) && /version:\s*"v1\.2\.4"[\s\S]*final v1\.2 documentation snapshot/.test(versionHistory) && /Documentation v1\.3\.3/.test(versionPage) && /v1\.3\.0 remains the latest\s+tagged GitHub Release/.test(versionPage) && install.includes("Latest GitHub Release (v1.3.0)"));
