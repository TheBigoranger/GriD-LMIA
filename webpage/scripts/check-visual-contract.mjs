...
check("welcome uses the paper Eq. (5) without Step 01 prose", firstHomeStep.includes('"\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N,"') && firstHomeStep.includes('"\\\\boldsymbol y(\\\\boldsymbol\\\\rho)=\\\\bigl(y_1(\\\\boldsymbol\\\\rho),\\\\ldots,y_N(\\\\boldsymbol\\\\rho)\\\\bigr)^\\\\top,"') && firstHomeStep.includes('"\\\\mathcal F(\\\\boldsymbol\\\\rho)=\\\\sum_{k=1}^{N}\\\\sum_{s=1}^{\\\\ell}T_{k,s}(\\\\boldsymbol\\\\rho)\\\\frac{\\\\partial y_k}{\\\\partial\\\\rho_s}(\\\\boldsymbol\\\\rho)+F_0(\\\\boldsymbol\\\\rho)+\\\\sum_{k=1}^{N}F_k(\\\\boldsymbol\\\\rho)y_k(\\\\boldsymbol\\\\rho)\\\\prec0,\\\\qquad\\\\forall\\\\boldsymbol\\\\rho\\\\in\\\\mathcal P."') && /#general-dpd-lmi-template/.test(firstHomeStep) && !/text(?:Html)?:|\\\\text\{|Y_1|\\\\mathcal F_a|S_a|negative semidefinite|optional bounded rate box|\\\\mathcal P\\\\times\\\\mathcal R|\\\\mathsf T/.test(firstHomeStep));
check("homepage follows both paper assumptions before Bernstein", /number: "02"[\s\S]*Assumption 1 makes the parameter domain and scheduling-rate domain hyperrectangles[\s\S]*Under Assumption 2, the known matrices are[\s\S]*renderInlineMath\("C\^0"\)[\s\S]*renderInlineMath\("C\^\{0,1\}"\)[\s\S]*Lipschitz[\s\S]*polynomial on each cell[\s\S]*limiting derivatives from adjacent cells need not agree[\s\S]*visual: "regularity"[\s\S]*number: "03"[\s\S]*Expand each cell in the Bernstein basis[\s\S]*visual: "storage"/.test(home) && ["\\\\mathcal P=\\\\prod_{s=1}^{\\\\ell}[\\\\underline\\\\rho_s,\\\\overline\\\\rho_s]", "\\\\mathcal R=\\\\prod_{s=1}^{\\\\ell}[\\\\underline\\\\nu_s,\\\\overline\\\\nu_s]", "\\\\mathcal G_s", "\\\\mathcal H_{\\\\mathbf c}"].every((fragment) => home.includes(fragment)) && /InterfaceRegularityDiagram/.test(home) && /CellStorageExplorer/.test(home));
check("shared-boundary regularity visual", /continuous piecewise-polynomial decision variable/.test(interfaceRegularity) && /unequal one-sided derivatives/.test(interfaceRegularity) && /shared value/.test(interfaceRegularity) && /left slope/.test(interfaceRegularity) && /right slope/.test(interfaceRegularity) && /role="img"/.test(interfaceRegularity));
