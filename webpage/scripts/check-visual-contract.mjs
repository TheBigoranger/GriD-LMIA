...
const welcomeInfoStyles = home.slice(home.indexOf(".welcome-info {"), home.indexOf(".citation-code {"));
...
// The Welcome page keeps one general target while detailed derivations and
// interactive islands remain on the linked manual pages.
...
check("welcome renders exactly one centered build-time DPD-LMI target", (home.match(/<KaTeXMath\b/g)?.length ?? 0) === 1 && /import KaTeXMath from "\.\/KaTeXMath\.astro"/.test(home) && /<figure class="welcome-target" aria-labelledby="welcome-target-caption">[\s\S]*<KaTeXMath[\s\S]*class="welcome-target__math formula-block formula-one-line"[\s\S]*tex=\{targetTex\}[\s\S]*\sdisplay\s*\/>/.test(home) && /\\mathcal F\(\\vect\\rho,\\dot\{\\vect\\rho\};\\vect y\)[\s\S]*\\dot\\rho_s F_\{k,s\}\(\\vect\\rho\)\\frac\{\\partial y_k\}\{\\partial\\rho_s\}[\s\S]*\\preceq0,\\quad\\forall\(\\vect\\rho,\\dot\{\\vect\\rho\}\)\\in\\mathcal P\\times\\mathcal R\./.test(home) && !/\\begin\{|\\\\/.test(home.match(/const targetTex[^;]+;/)?.[0] ?? ""));
check("welcome remains static and excludes detailed mathematics", !/RenderedMath|renderMath|fullCode:|visibleCode:|client:(?:load|visible|idle)|GridPartitionExplorer|CellStorageExplorer|JourneyCurve|ExportSolveFlow|CertificateFlow/.test(home));
...
check("welcome target scrolling is local and width-aware", /\.welcome-target\s*\{[^}]*max-width:100%[^}]*overflow-x:auto[^}]*overscroll-behavior-inline:contain/.test(home) && /welcome-target__math\.formula-display/.test(home) && /width:100%; min-width:0; max-width:100%/.test(home));
...
check("welcome project information is exact, static, and vertically ordered", [
  "Created and maintained by Yicheng Xu.",
  "848 behavioral tests and a 171-entry API gate",
  "Direction-wise continuity and spline decisions",
  "Exact cell-preserving Bernstein refinement",
  "197 public API records",
].every((fragment) => home.includes(fragment)) && home.includes(`const citation = ${JSON.stringify(welcomeCitation)};`) && home.includes(`const bibtex = String.raw\`${welcomeBibtex}\`;`) && /href=\{local\("about\/"\)\}/.test(home) && /href=\{local\("version-history\/"\)\}/.test(home) && /href=\{local\("citing\/"\)\}/.test(home) && (home.match(/class="welcome-info__stream"/g)?.length ?? 0) === 1 && (home.match(/<article class="welcome-info__item"/g)?.length ?? 0) === 3 && (home.match(/class="welcome-info__separator"/g)?.length ?? 0) === 2 && /Author and maintainer[\s\S]*welcome-info__separator[\s\S]*Latest in v1\.5\.0[\s\S]*welcome-info__separator[\s\S]*Cite GriD-LMIA/.test(home) && /\.welcome-info__stream\s*\{[^}]*display:flex[^}]*flex-direction:column/.test(home) && !/display:grid|grid-template/.test(welcomeInfoStyles) && !/welcome-info__(?:card|column|grid|tab)|role="tablist"|<details\b/i.test(home.slice(home.indexOf('<section class="welcome-info"'))) && /<p class="citation-label" id="citation-plain-label">Plain text<\/p>[\s\S]*<pre class="citation-code" aria-labelledby="citation-plain-label" tabindex="0">/.test(home) && /<p class="citation-label" id="citation-bibtex-label">BibTeX<\/p>[\s\S]*<pre class="citation-code citation-code--bibtex" aria-labelledby="citation-bibtex-label" tabindex="0">/.test(home) && /\.citation-code\s*\{[^}]*box-sizing:border-box[^}]*max-width:100%[^}]*overflow-x:auto[^}]*overscroll-behavior-inline:contain/.test(home) && !/navigator\.clipboard|copy-button|client:/.test(home));
...
check("all six custom centered formulas use KaTeX display mode", centeredKatexTags.length === 6 && centeredKatexTags.every((tag) => /\sdisplay(?:\s|\n|\/?>)/.test(tag)) && !/<InlineMath\b[^>]*className="[^"]*\bformula-block\b/.test(allSource));
...
check("welcome links from its general target without embedding application detail", workflowRoutes[0]?.endsWith("#general-dpd-lmi-template") === true && /F_0\(\\vect\\rho\)/.test(home) && !/Model an LPV induced-L2-gain problem|x\(t\) is the plant state|\\\\lVert z\\\\rVert_2<\\\\gamma\\\\lVert w\\\\rVert_2/.test(home));
check("new mathematics page contract", detailedMathPages.every((source) => /^---[\s\S]*title:[^\n]+[\s\S]*description:[^\n]+[\s\S]*---/.test(source) && /manual-trail/.test(source) && /^## See Also$/m.test(source)) && /Weighted Product Contraction/.test(coefficientAlgebra) && /Reusable Bernstein.?Gram Map/.test(markovPutinarMath) && /Sliding Tensor Windows/.test(sparseFullboxMath));
...
check("source-indexed API inventory matches v1.5.0", referenceEntries.length === 197 && inventoryCounts["pdbase-backend"] === 50 && inventoryCounts.pdmat === 55 && inventoryCounts.pdvar === 52 && inventoryCounts.pdlmi === 24 && inventoryCounts["shared-helpers"] === 15 && inventoryCounts.setup === 1);
...
check("seven source-grounded examples", (examplesIndex.match(/github\.com\/TheBigoranger\/GriD-LMIA\/blob\/main\/example\//g)?.length ?? 0) === 7 && ["lpv_l2_gain_univariate", "lpv_l2_gain_three_parameter", "delay_stability_basic", "delay_stability_augmented", "discrete_time_robust_stability", "continuous_time_robust_stability", "mass_spring_damper_stability"].every((name) => examplesIndex.includes(name)) && /Manual Reproduction Boundary/.test(examplesIndex) && /api-horzcat/.test(examplesIndex) && /api-vertcat/.test(examplesIndex));
check("explicit source and publish build modes", /GRID_LMIA_DOC_BUILD_MODE/.test(prepareBuild) && /mode === "source"/.test(prepareBuild) && /validatePublicationManifest/.test(prepareBuild) && /86E50D7110108128A2D7386A35532315E5D93591B754FD7FD7F81F544774101F/.test(publicationContract) && /manualPageCount: 184/.test(publicationContract) && /src\/data\/version-history\.js/.test(publicationContract) && /validateVersionHistorySource/.test(publicationContract) && /terminologyTerms/.test(terminologyLinker) && !/doc\/support/.test(terminologyLinker));
...
check("synchronous SSR keeps useEffect limited to post-commit layout readiness, never a renderer queue", /renderMath/.test(katexAstro + katexReact + katexRenderer) && /set:html/.test(katexAstro) && /dangerouslySetInnerHTML/.test(katexReact) && /export const DisplayMath/.test(katexReact) && /export const InlineMath/.test(katexReact) && !/\buseEffect\b|readiness|queue/i.test(katexAstro + katexRenderer + astroConfig) && /data-formula-react/.test(katexReact) && /dataset.formulaReady/.test(katexReact));
...
check("outer formula frames contain overflow", /\.formula-display\s*\{[^}]*overflow-x:\s*auto/.test(manualCss) && /\.formula-fit-size/.test(manualCss));
check("local formula scrolling follows rendered width", /data-formula-fit/.test(geometryCheck) && /localNeedsScroll/.test(geometryCheck) && /local-formula-scroll-required/.test(geometryCheck) && /local-formula-scroll-unneeded/.test(geometryCheck) && /data-formula-fitted/.test(manualCss));
...
check("named formula regressions stay direct", /P\^\{\(c\)\}\(\\rho\)[\s\S]*&=B_0\^1\(\\alpha\)P\^\{\(c\)\}\[0\][\s\S]*\+B_1\^1\(\\alpha\)P\^\{\(c\)\}\[1\]\./.test(pdvarCtor) && !/\\mathcal F_\{\\mathrm\{alg\}\}|\\mathcal A_k|\\partial_s y_k|\\mathcal D_\{ks\}/.test(modelMath) && !/\\Gamma_\{p,q\}|T_[123]&:=|C_\{00\}&:=|\\mathcal J&:=/.test(pdlmiFullbox) && !/T_\{ij\}\^\{\(k\)\}|\\omega\(i,j;k\)/.test(pdvarAlgebra) && !/L_\{1[123]\}|L_\{2[23]\}|L_\{33\}/.test(solverSmoke));
...
check("v1.5.0 current documentation and latest tag", versionData.includes('current: "v1.5.0"') && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 12 && /version:\s*"v1\.5\.0"[\s\S]*2026-09-22[\s\S]*current documentation snapshot and latest tagged GitHub Release/.test(versionHistory) && /version:\s*"v1\.4\.0"[\s\S]*2026-08-14[\s\S]*earlier tagged GitHub Release/.test(versionHistory) && /Documentation v1\.5\.0/.test(versionPage) && /latest tagged GitHub\s+Release/.test(versionPage) && install.includes("Latest GitHub Release (v1.5.0)"));
