...
check("source-indexed API inventory matches v1.4.0", referenceEntries.length === 192 && inventoryCounts["pdbase-backend"] === 49 && inventoryCounts.pdmat === 55 && inventoryCounts.pdvar === 52 && inventoryCounts.pdlmi === 22 && inventoryCounts["shared-helpers"] === 13 && inventoryCounts.setup === 1);
...
check("v1.4.0 current documentation and latest tagged release", versionData.includes('current: "v1.4.0"') && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 9 && /version:\s*"v1\.4\.0"[\s\S]*2026-08-14[\s\S]*latest tagged GitHub Release/.test(versionHistory) && /version:\s*"v1\.3\.8"[\s\S]*final v1\.3 documentation snapshot/.test(versionHistory) && /version:\s*"v1\.2\.4"[\s\S]*final v1\.2 documentation snapshot/.test(versionHistory) && /Documentation v1\.4\.0/.test(versionPage) && /current manual and tagged GitHub Release/.test(versionPage) && install.includes("Latest GitHub Release (v1.4.0)"));
