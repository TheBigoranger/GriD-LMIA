...
check("source-indexed API inventory matches v1.4.3", referenceEntries.length === 194 && inventoryCounts["pdbase-backend"] === 49 && inventoryCounts.pdmat === 55 && inventoryCounts.pdvar === 52 && inventoryCounts.pdlmi === 24 && inventoryCounts["shared-helpers"] === 13 && inventoryCounts.setup === 1);
...
check("explicit source and publish build modes", /GRID_LMIA_DOC_BUILD_MODE/.test(prepareBuild) && /mode === "source"/.test(prepareBuild) && /validatePublicationManifest/.test(prepareBuild) && /7F3207AE01387EA2EDAC2D7677D8CB66B15C9B6EFC14B4C841101C0F6F2B5A97/.test(publicationContract) && /manualPageCount: 179/.test(publicationContract) && /src\/data\/version-history\.js/.test(publicationContract) && /validateVersionHistorySource/.test(publicationContract) && /terminologyTerms/.test(terminologyLinker) && !/doc\/support/.test(terminologyLinker));
...
check("v1.4.3 current documentation and v1.4.0 latest tag", versionData.includes('current: "v1.4.3"') && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 11 && /version:\s*"v1\.4\.3"[\s\S]*2026-09-04[\s\S]*current documentation snapshot/.test(versionHistory) && /version:\s*"v1\.4\.0"[\s\S]*2026-08-14[\s\S]*latest tagged GitHub Release/.test(versionHistory) && /Documentation v1\.4\.3/.test(versionPage) && /Package v1\.4\.0 remains[\s\S]*latest tagged GitHub Release/.test(versionPage) && install.includes("Latest GitHub Release (v1.4.0)"));
