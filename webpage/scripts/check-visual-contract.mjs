...
check("five welcome stages and required mathematics", stepCount === 5 && ["Model the general DPD-LMI problem", "Partition the supported parameter box", "Fix one cell and use the Bernstein basis", "Replace one continuous cell condition by Direct tests", "Assemble the panorama, solve, and validate status"].every((label) => home.includes(label)) && ["\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N", "\\\\dot\\\\rho_sT_{k,s}", "\\\\mathcal P=\\\\prod", "\\\\mathcal R=\\\\prod", "\\\\mathcal H_{\\\\mathbf c}", "\\\\boldsymbol\\\\alpha=\\\\phi_{\\\\mathbf c}", "B_{\\\\mathbf i}^{\\\\mathbf m}", "C^{(\\\\mathbf c)}[\\\\mathbf i]", "toYalmip", "optimize", "sol.problem == 0", "value(P)"].every((fragment) => home.includes(fragment)));
...
check("welcome separates exact partition and approximation", /axis-aligned parameter and rate boxes/.test(secondHomeStep) && /exactly partition the parameter box/.test(secondHomeStep) && /Gridding becomes an approximation only when it restricts the admissible decision-function space/.test(secondHomeStep) && /affine rate-vertex reduction is a separate exact step/.test(secondHomeStep) && /visual: "grid"/.test(secondHomeStep) && /GridPartitionExplorer client:visible/.test(home));
check("accessible nonuniform grid explorer", /role="tablist"/.test(gridExplorer) && /ArrowLeft[\s\S]*ArrowRight[\s\S]*Home[\s\S]*End/.test(gridExplorer) && /1: \[0\.35\][\s\S]*2: \[0\.4, 0\.25\][\s\S]*3: \[0\.3, 0\.55, 0\.75\]/.test(gridExplorer) && /type="range"/.test(gridExplorer) && /aria-live="polite"/.test(gridExplorer) && /onPointerDown[\s\S]*onPointerMove[\s\S]*onPointerUp/.test(gridExplorer) && /Rotate left[\s\S]*Rotate right[\s\S]*Tilt up[\s\S]*Tilt down[\s\S]*Reset view/.test(gridExplorer) && /enumerateCells[\s\S]*getCellBounds[\s\S]*projectPoint/.test(gridExplorer) && /KNOT_MIN = 0\.1[\s\S]*KNOT_MAX = 0\.9[\s\S]*PITCH_LIMIT = 70/.test(gridPartition));
...
check("certificate flow tabs on welcome and interactive reference overview", /CertificateFlow compact/.test(home) && /compact=\{compact\}/.test(certificateFlow) && /Positive target/.test(certificateFlowTabs) && /S\^\{\(\\\\mathbf c\)\}\(\\\\boldsymbol\\\\alpha\)\\\\succeq0/.test(certificateFlowTabs) && !/F\^\{\(\\\\mathbf c,v\)\}/.test(certificateFlowTabs) && /role="tablist"/.test(certificateFlowTabs) && /role="tabpanel"/.test(certificateFlowTabs) && /ArrowLeft[\s\S]*ArrowRight[\s\S]*Home[\s\S]*End/.test(certificateFlowTabs) && /compact && <p>/.test(certificateFlowTabs) && /Open the \{option\.label\} reference/.test(certificateFlowTabs) && /CertificateChooser/.test(pdlmiOverview) && /useState\(0\)/.test(chooser) && /role="tablist"/.test(chooser) && /selected\.toYalmip\(\)/.test(chooser));
...
check("cell-indexed DPD-LMI workflow", ["\\\\mathcal H_{\\\\mathbf c}", "\\\\boldsymbol\\\\alpha=\\\\phi_{\\\\mathbf c}", "S^{(\\\\mathbf c)}", "C^{(\\\\mathbf c)}[\\\\mathbf i]", "selected.toYalmip()"].every((fragment) => home.includes(fragment)) && /optional exact scheduling-rate vertex/.test(home) && !/F\^\{\(\\\\mathbf c,v\)\}|\\\\mathcal V|m\+p/.test(home + certData + certificateFlow));
...
check("v1.1.4 documentation and v1.1.0 Release metadata", /current:\s*"v1\.1\.4"/.test(versionData) && /Documentation v1\.1\.4/.test(pdlmiOverview) && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 7 && ["v0.1.0", "v0.2.12", "v0.3.6", "v0.4.7", "v1.0.0", "v1.1.0", "v1.1.4"].every((version) => versionHistory.includes(`version: "${version}"`)) && /version:\s*"v1\.1\.4"[\s\S]*status:\s*"current documentation snapshot"/.test(versionHistory) && /version:\s*"v1\.1\.0"[\s\S]*status:\s*"latest GitHub Release"/.test(versionHistory) && /No v1\.1\.4 tag or GitHub Release exists/.test(versionPage) && /Latest GitHub Release \(v1\.1\.0\)/.test(install) && /v1\.1\.0 release gate completed 280 of 280 tests/.test(install) && /version:\s*"v1\.0\.0"[\s\S]*date:\s*"2026-07-19"[\s\S]*6d19619/.test(versionHistory));
