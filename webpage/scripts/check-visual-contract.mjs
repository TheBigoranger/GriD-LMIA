...
check("labels reserved outside journey SVG", /<svg[\s\S]*<\/svg>[\s\S]*<ol class="journey-stage-band">/.test(journey) && !/<svg[\s\S]*<text/.test(journey) && /journey-leaders/.test(journey));
...
check("five welcome stages and required mathematics", stepCount === 5 && ["Form the DPD-LMI", "Grid the ρ-space", "Build constraints inside each hypercube", "Select a finite certificate", "Export, solve, and recover the decision matrix"].every((label) => home.includes(label)) && ["\\\\frac{dP}{dt}", "\\\\sum_{s=1}^{\\\\ell}", "\\\\mathcal H_{\\\\mathbf c}", "F^{(\\\\mathbf c,v)}", "toYalmip", "optimize", "value(P)"].every((fragment) => home.includes(fragment)));
...
check("cell-indexed DPD-LMI workflow", ["\\\\dot P(\\\\rho(t))=\\\\frac{dP}{dt}", "\\\\mathcal H_{\\\\mathbf c}=", "F^{(\\\\mathbf c,v)}(\\\\alpha)\\\\preceq0", "selected.toYalmip()"].every((fragment) => home.includes(fragment)) && /every cell-and-rate pair/.test(home));
