...
check("welcome wordmark uses a contained single-line fluid scale", /\.home-wordmark\s*\{[^}]*font-size:\s*clamp\(2\.25rem,\s*10vw,\s*7\.4rem\)[^}]*white-space:\s*nowrap/.test(home) && /home-wordmark-line/.test(geometryCheck) && /home-wordmark-bounds/.test(geometryCheck) && /document\.createRange\(\)/.test(geometryCheck));
...
check("source-indexed API inventory matches v1.3.8", referenceEntries.length === 192 && inventoryCounts["pdbase-backend"] === 49 && inventoryCounts.pdmat === 55 && inventoryCounts.pdvar === 52 && inventoryCounts.pdlmi === 22 && inventoryCounts["shared-helpers"] === 13 && inventoryCounts.setup === 1);
...
check("v1.3.8 current documentation and v1.3.0 latest tagged release", versionData.includes('current: "v1.3.8"') && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 9 && /version:\s*"v1\.3\.8"[\s\S]*2026-08-10[\s\S]*current documentation/.test(versionHistory) && /version:\s*"v1\.3\.0"[\s\S]*latest tagged GitHub Release/.test(versionHistory) && /version:\s*"v1\.2\.4"[\s\S]*final v1\.2 documentation snapshot/.test(versionHistory) && /Documentation v1\.3\.8/.test(versionPage) && /v1\.3\.0 remains the latest\s+tagged GitHub Release/.test(versionPage) && install.includes("Latest GitHub Release (v1.3.0)"));
