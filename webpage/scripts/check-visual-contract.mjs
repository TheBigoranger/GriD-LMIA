...
check("source-indexed API inventory matches v1.4.2", referenceEntries.length === 194 && inventoryCounts["pdbase-backend"] === 49 && inventoryCounts.pdmat === 55 && inventoryCounts.pdvar === 52 && inventoryCounts.pdlmi === 24 && inventoryCounts["shared-helpers"] === 13 && inventoryCounts.setup === 1);
...
check("local formula scrolling follows rendered width", (pdmatElevate.match(/className="elevate-formula-one-line"/g)?.length ?? 0) === 5 && (solverSmoke.match(/className="solver-one-line"/g)?.length ?? 0) === 3 && (cellExplorer.match(/className="cell-formula-one-line"/g)?.length ?? 0) === 3 && /\.elevate-formula-one-line\s*\{[^}]*max-width:\s*100%[^}]*overflow-x:\s*auto[^}]*\}/.test(manualCss) && /\.elevate-formula-one-line\s*>\s*\.formula-display\s*\{[^}]*width:\s*max-content[^}]*min-width:\s*100%/.test(manualCss) && /\.solver-one-line\s*\{[^}]*max-width:\s*100%[^}]*overflow-x:\s*auto[^}]*\}/.test(manualCss) && /\.cell-formula-one-line\s*\{[^}]*max-width:\s*100%[^}]*overflow-x:\s*auto[^}]*\}/.test(manualCss) && /"\.elevate-formula-one-line, \.solver-one-line, \.cell-formula-one-line, \.welcome-target"/.test(geometryCheck) && /localNeedsScroll\s*=\s*Boolean\([\s\S]*localScroller\.scrollWidth\s*>\s*localScroller\.clientWidth\s*\+\s*tolerance/.test(geometryCheck) && /localScrollerStyle\.overflowX\s*===\s*"auto"\s*&&\s*localNeedsScroll/.test(geometryCheck) && /localNeedsScroll\s*&&\s*!localScrollActive/.test(geometryCheck) && /localScrollerStyle\?\.overflowX\s*===\s*"scroll"\s*&&\s*!localNeedsScroll/.test(geometryCheck) && !/isElevationScroller/.test(geometryCheck) && /local-formula-scroll-bounds/.test(geometryCheck) && /local-formula-scroll-required/.test(geometryCheck) && /local-formula-scroll-unneeded/.test(geometryCheck) && !/\.formula-display\s*\{[^}]*overflow-x:\s*(?:auto|scroll)/.test(mathStyleSources));
...
check("v1.4.2 current documentation and v1.4.0 latest tag", versionData.includes('current: "v1.4.2"') && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 10 && /version:\s*"v1\.4\.2"[\s\S]*2026-08-31[\s\S]*current documentation snapshot/.test(versionHistory) && /version:\s*"v1\.4\.0"[\s\S]*2026-08-14[\s\S]*latest tagged GitHub Release/.test(versionHistory) && /Documentation v1\.4\.2/.test(versionPage) && /Package v1\.4\.0 remains[\s\S]*latest tagged GitHub Release/.test(versionPage) && install.includes("Latest GitHub Release (v1.4.0)"));
