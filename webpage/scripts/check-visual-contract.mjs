...
check("v1.3.0 current release and retained v1.2.4 snapshot", versionData.includes('current: "v1.3.0"') && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 8 && /version:\s*"v1\.3\.0"[\s\S]*latest tagged GitHub Release/.test(versionHistory) && /version:\s*"v1\.2\.4"[\s\S]*final v1\.2 documentation snapshot/.test(versionHistory) && /Documentation v1\.3\.0/.test(versionPage) && install.includes("Latest GitHub Release (v1.3.0)"));
