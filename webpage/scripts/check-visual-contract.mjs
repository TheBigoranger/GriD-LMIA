...
check("equal-height certificate tabs with overlay default", /grid-auto-rows:\s*1fr/.test(manualCss) && /block-size:\s*5\.7rem/.test(manualCss) && /certificate-default[\s\S]*position:\s*absolute/.test(manualCss) && /className="certificate-default"/.test(chooser));
...
check("categorized document and lookup views", /ReferenceCategoryHub/.test(documentsHub) && /referenceGroups/.test(referenceIndex) && ["pdmat", "pdvar", "pdlmi", "pdbase-backend", "shared-helpers"].every((group) => referenceIndex.includes(`id: "${group}"`)) && /referenceGroups\.map/.test(read("scripts/generate-reference-index.mjs")));
...
check("right-anchored header cluster", /site-header__links \{[^}]*margin-inline-start:\s*auto/.test(header) && /site-header__more \{[^}]*margin-inline-start:\s*auto/.test(header) && /site-header__search \{[^}]*margin-inline-start:\s*clamp/.test(header));
check("v0.4.2 public metadata and final-patch history", /current:\s*"v0\.4\.2"/.test(versionData) && /Version 0\.4\.2/.test(pdlmiOverview) && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 4 && ["v0.1.0", "v0.2.12", "v0.3.6", "v0.4.2"].every((version) => versionHistory.includes(`version: "${version}"`)) && ["63c06ee", "98b63e5", "fe74a65"].every((commit) => versionHistory.includes(commit)) && !["v0.2.0", "v0.3.0", "v0.3.5", "v0.4.1"].some((version) => versionHistory.includes(`version: "${version}"`)) && /version:\s*"v0\.4\.2"[\s\S]*date:\s*"2026-07-17"/.test(versionHistory));
