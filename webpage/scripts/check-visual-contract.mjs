...
check("pdvar mirrors pdmat mathematics with symbolic coefficient payloads", /P\^\{\(\\boldsymbol c\)\}\(\\boldsymbol\\rho\)[\s\S]*\\sum_\{\\boldsymbol i\\in\\prod_s\\\{0,\\ldots,m_s\\\}\}[\s\S]*X\^\{\(\\boldsymbol c\)\}\[\\boldsymbol i\][\s\S]*B_\{\\boldsymbol i\}\^\{\\boldsymbol m\}/.test(pdvarCtor) && /\\binom\{m_s\}\{i_s\}[\s\S]*\\alpha_s\^\{i_s\}[\s\S]*\(1-\\alpha_s\)\^\{m_s-i_s\}/.test(pdvarCtor) && /pdmat Constructor/.test(pdvarCtor) && /\\operatorname\{Aff\}\(\\texttt\{sdpvar\}\)/.test(pdvarStorage) && /\\widehat X\^\{\(\\boldsymbol c\)\}\[\\boldsymbol k\][\s\S]*\\sum_\{\\boldsymbol i\\in\\prod_\{s=1\}\^\{\\ell\}\\\{0,\\ldots,m_s\\\}\}[\s\S]*\\binom\{m_s\}\{i_s\}\\binom\{M_s-m_s\}\{k_s-i_s\}[\s\S]*\\binom\{M_s\}\{k_s\}[\s\S]*\\boldsymbol M\\ge\\boldsymbol m/.test(pdvarStorage) && /P\^\{\(\\boldsymbol c\)\}\(\\boldsymbol\\rho\)[\s\S]*\\sum_\{\\boldsymbol i\\in\\prod_\{s=1\}\^\{\\ell\}\\\{0,\\ldots,m_s\\\}\}[\s\S]*B_\{\\boldsymbol i\}\^\{\\boldsymbol m\}/.test(pdvarStorage) && /pdmat` elevation formula/.test(pdvarStorage) && /pdmat evaluate/.test(pdvarStorage) && !/client:visible|interactive-figure/.test(pdvarCtor + pdvarStorage));
...
check("pdmat evaluate gives the general tensor Bernstein formula", /\\boldsymbol\\rho\\in\\mathcal H_\{\\boldsymbol c\}[\s\S]*\\prod_\{s=1\}\^\{\\ell\}/.test(pdmatEval) && /\\alpha_s[\s\S]*\\rho_s\^\{\(c_s\)\}/.test(pdmatEval) && /\\boldsymbol m=\(m_1,\\ldots,m_\\ell\)/.test(pdmatEval) && /A\^\{\(\\boldsymbol c\)\}\(\\boldsymbol\\alpha\)[\s\S]*\\sum_\{\\boldsymbol i\\in\\prod_\{s=1\}\^\{\\ell\}\\\{0,\\ldots,m_s\\\}\}/.test(pdmatEval) && /B_\{\\boldsymbol i\}\^\{\\boldsymbol m\}\(\\boldsymbol\\alpha\)[\s\S]*\\prod_\{s=1\}\^\{\\ell\}[\s\S]*\\binom\{m_s\}\{i_s\}[\s\S]*\\alpha_s\^\{i_s\}\(1-\\alpha_s\)\^\{m_s-i_s\}/.test(pdmatEval) && !/For degree-one scalar data/.test(pdmatEval));
...
check("unambiguous tensor and derivative notation", /h_s\^\{\\mathbf c\}/.test(griddingMath) && /\\boldsymbol\\alpha=\\phi_\{\\mathbf c\}\^\{-1\}\(\\boldsymbol\\rho\)/.test(griddingMath) && /\\frac\{1\}\{h_s\^\{\\mathbf c\}\}/.test(rateInterfaceMath) && !/width \$h=/.test(griddingMath));
...
check("v1.2.0 current Release and retained v1.1.6 snapshot", /current:\s*"v1\.2\.0"/.test(versionData) && /Documentation v1\.2\.0/.test(pdlmiOverview) && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 7 && ["v0.1.0", "v0.2.12", "v0.3.6", "v0.4.7", "v1.0.0", "v1.1.6", "v1.2.0"].every((version) => versionHistory.includes(`version: "${version}"`)) && !/version:\s*"v1\.1\.0"/.test(versionHistory) && /version:\s*"v1\.2\.0"[\s\S]*status:\s*"current and latest GitHub Release"/.test(versionHistory) && /version:\s*"v1\.1\.6"[\s\S]*status:\s*"final v1\.1 documentation snapshot"/.test(versionHistory) && /v1\.2\.0 is the current manual and latest tagged GitHub Release/.test(versionPage) && /Latest GitHub Release \(v1\.2\.0\)/.test(install) && /release gate is `tests\.run_all\(\)`/.test(install) && /version:\s*"v1\.0\.0"[\s\S]*date:\s*"2026-07-19"[\s\S]*6d19619/.test(versionHistory));
