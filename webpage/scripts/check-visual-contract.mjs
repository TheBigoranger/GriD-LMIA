...
check("pdmat elevate follows the appendix definition, kernel, and tensor convolution", /## What degree elevation does/.test(pdmatElevate) && /\\widehat C\^\{\(\\boldsymbol c\)\}\[\\boldsymbol k\][\s\S]*\\binom\{M-m\}\{k_s-i_s\}/.test(pdmatElevate) && /\\mathcal E_\{M-m\}\[\\boldsymbol j\][\s\S]*\\prod_\{s=1\}\^\{\\ell\}[\s\S]*\\binom\{M-m\}\{j_s\}/.test(pdmatElevate) && /\\ast_\{\\ell\}/.test(pdmatElevate) && !/W_\{\\boldsymbol i\}|K_\{\\boldsymbol j\}|\\mathcal I\(\\boldsymbol k\)/.test(pdmatElevate) && /### One-dimensional coefficient rule and example[\s\S]*\\widetilde a_i[\s\S]*A = pdmat/.test(pdmatElevate) && pdmatElevate.includes("`ncoeff(A)` increases") && /not grid refinement, interpolation, or an approximation step/.test(pdmatElevate));
check("pdvar mirrors pdmat mathematics with symbolic coefficient payloads", /P\^\{\(\\boldsymbol c\)\}\(\\boldsymbol\\rho\)[\s\S]*X\^\{\(\\boldsymbol c\)\}\[\\boldsymbol i\][\s\S]*B_\{\\boldsymbol i\}\^\{m\}/.test(pdvarCtor) && /pdmat Constructor/.test(pdvarCtor) && /\\operatorname\{Aff\}\(\\texttt\{sdpvar\}\)/.test(pdvarStorage) && /\\widehat X\^\{\(\\boldsymbol c\)\}\[\\boldsymbol k\][\s\S]*\\binom\{M-m\}\{k_s-i_s\}/.test(pdvarStorage) && /pdmat` elevation formula/.test(pdvarStorage) && /pdmat evaluate/.test(pdvarStorage) && !/client:visible|interactive-figure/.test(pdvarCtor + pdvarStorage));
...
check("pdmat evaluate gives the general tensor Bernstein formula", /\\boldsymbol\\rho\\in\\mathcal H_\{\\boldsymbol c\}[\s\S]*\\prod_\{s=1\}\^\{\\ell\}/.test(pdmatEval) && /\\alpha_s[\s\S]*\\rho_s\^\{\(c_s\)\}/.test(pdmatEval) && /A\^\{\(\\boldsymbol c\)\}\(\\boldsymbol\\alpha\)[\s\S]*\\sum_\{\\boldsymbol i\\in\\\{0,\\ldots,m\\\}\^\{\\ell\}\}/.test(pdmatEval) && /B_\{\\boldsymbol i\}\^\{m\}\(\\boldsymbol\\alpha\)[\s\S]*\\prod_\{s=1\}\^\{\\ell\}[\s\S]*\\binom\{m\}\{i_s\}/.test(pdmatEval) && !/For degree-one scalar data/.test(pdmatEval));
...
check("v1.1.6 documentation and v1.1.0 Release metadata", /current:\s*"v1\.1\.6"/.test(versionData) && /Documentation v1\.1\.6/.test(pdlmiOverview) && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 7 && ["v0.1.0", "v0.2.12", "v0.3.6", "v0.4.7", "v1.0.0", "v1.1.0", "v1.1.6"].every((version) => versionHistory.includes(`version: "${version}"`)) && /version:\s*"v1\.1\.6"[\s\S]*status:\s*"current documentation snapshot"/.test(versionHistory) && /version:\s*"v1\.1\.0"[\s\S]*status:\s*"latest GitHub Release"/.test(versionHistory) && /No v1\.1\.6 tag or GitHub Release exists/.test(versionPage) && /Latest GitHub Release \(v1\.1\.0\)/.test(install) && /v1\.1\.0 release gate completed 280 of 280 tests/.test(install) && /version:\s*"v1\.0\.0"[\s\S]*date:\s*"2026-07-19"[\s\S]*6d19619/.test(versionHistory));
