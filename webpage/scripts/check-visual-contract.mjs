...
check("five welcome stages and required mathematics", stepCount === 5 && ["Model the general DPD-LMI problem", "Partition the supported parameter box", "Fix one cell and use the Bernstein basis", "Select a finite certificate for the positive target", "Assemble the panorama, solve, and validate status"].every((label) => home.includes(label)) && ["\\\\boldsymbol\\\\rho\\\\in\\\\mathcal P", "\\\\dot{\\\\boldsymbol\\\\rho}\\\\in\\\\mathcal R", "\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R^N", "\\\\mathcal F(\\\\boldsymbol\\\\rho,\\\\dot{\\\\boldsymbol\\\\rho};y)", "\\\\dot\\\\rho_sT_{k,s}", "\\\\mathcal P=\\\\prod", "\\\\mathcal R=\\\\prod", "\\\\mathcal G_s=\\\\{", "\\\\mathcal H_{\\\\mathbf c}=\\\\prod", "\\\\boldsymbol\\\\alpha=\\\\phi_{\\\\mathbf c}^{-1}", "B_{\\\\mathbf i}^{\\\\mathbf m}", "S^{(\\\\mathbf c)}", "toYalmip", "optimize", "sol.problem == 0", "value(P)"].every((fragment) => home.includes(fragment)));
...
check("welcome stage 01 names parameter and rate domains", /inlineMath:\s*"\\\\boldsymbol\\\\rho\\\\in\\\\mathcal P,\\\\qquad\\\\dot\{\\\\boldsymbol\\\\rho\}\\\\in\\\\mathcal R,\\\\qquad\\\\boldsymbol y:\\\\mathcal P\\\\to\\\\mathbb R\^N\."/.test(firstHomeStep) && /supported parameter domain[\s\S]*supported scheduling-rate domain/.test(firstHomeStep));
...
check("SparsePutinar card and reference keep parity, tensor windows, accumulation, and graph-chordal boundary", /key: "sparseputinar"/.test(certData) && /useSpPut\(\)/.test(certData) && /tensor windows/.test(certData) && [pdlmiSparsePutinar, sparsePutinarMath].every((source) => source.includes("parity-specific Markov–Lukács") && source.includes("singleton") && (source.includes("tensor-window") || source.includes("tensor windows")) && source.includes("coefficient identit") && source.includes("Zheng") && /does not\s+infer/.test(source)) && /Tensor-window side b/.test(chooser) && /cliqueSize/.test(chooser));
check("SparseFullBox card and references use sliding tensor windows", /sliding tensor windows/.test(sparseFullboxMath) && /sliding tensor-window/.test(pdlmiSparseFullbox) && /overlapping windows/.test(certData) && /tensor-label window/.test(sparseFlowNotation) && [sparseFullboxMath, pdlmiSparseFullbox, certData, sparseFlowNotation].every((source) => !/block[- ]band/i.test(source)) && certData.includes("Q_{J,\\\\mathbf u}") && sparseFlowNotation.includes("Q_{J,\\\\mathbf u}"));
...
check("pdmat evaluate remark exposes boundary and reconstruction mechanics", /Implementation remark: boundary ownership and row reconstruction[\s\S]*cell on the right[\s\S]*tensor[\s\S]*Bernstein weight[\s\S]*does not use de Casteljau/.test(pdmatEval));
...
check("Pólya card presents transform before coefficient test and index set", certData.includes('cardFormula: "\\\\tilde S^{(\\\\mathbf c)}(\\\\boldsymbol\\\\alpha)=S^{(\\\\mathbf c)}(\\\\boldsymbol\\\\alpha)\\\\prod_{s=1}^{\\\\ell}') && certificateFlow.includes('polya: "\\\\begin{gathered}\\\\tilde C') && certificateFlow.indexOf("\\\\tilde C") < certificateFlow.indexOf("\\\\mathcal I_{\\\\mathbf M+\\\\mathbf d}=") && certificateFlowTabs.indexOf('className="certificate-flow-panel__formula"') < certificateFlowTabs.indexOf('className="certificate-flow-panel__notation"') && certificateFlowTabs.indexOf("option.formula") < certificateFlowTabs.indexOf("option.notation"));
...
check("welcome storage uses build-time formula markup wrappers", /import \{ InlineMath \} from "\.\/RenderedMath\.tsx"/.test(cellExplorer) && /<InlineMath[\s\S]*markup=/.test(cellExplorer) && !/\btex=|renderMath|renderToString|dangerouslySetInnerHTML/.test(cellExplorer) && /basis:\s*renderMath\("B_\{\\\\mathbf i\}\^\{\\\\mathbf m\}\(\\\\boldsymbol\\\\alpha\)=\\\\prod/.test(home) && /bernstein:\s*renderMath\(`A\^\{\(\$\{cell\.c1\},1\)\}/.test(home) && /storageMath: "A\(\\\\rho_1,\\\\rho_2\)=\\\\begin\{bmatrix\}1\+\\\\rho_1\+\\\\rho_2/.test(home) && !/a_\{11\}|a_\{12\}|a_\{22\}/.test(thirdHomeStep));
...
check("canonical notation", /id="domains-and-rate-vertices"/.test(notationMath) && /id="grids-cells-and-local-coordinates"/.test(notationMath) && /id="bernstein-labels-and-degrees"/.test(notationMath) && /id="residual-and-positive-target"/.test(notationMath) && /E_\{\\mathbf M\\leftarrow\\mathbf m\}/.test(coefficientAlgebra) && /\\boldsymbol\\nu\(t\)=\\dot\{\\boldsymbol\\rho\}\(t\)/.test(modelMath));
...
check("canonical cell-local function notation", !/P_c|Q_c|dP_c/.test(bernsteinMath + coefficientAlgebra) && /P\^\{\(c\)\}/.test(pdvarCtor) && !/P_\{?[0-9i]/.test(pdvarCtor) && /X\^\{\(\\boldsymbol c\)\}\[\\boldsymbol i\]/.test(pdvarCtor));
...
check("legacy mathematics routes and anchors", [bernsteinMath, griddingMath, sosMath].every((source) => /manual-trail/.test(source) && /^## Further Reading$/m.test(source) && /^## See Also$/m.test(source)) && [griddingMath, sosMath].every((source) => /^## Limitations$/m.test(source)) && /id="limitations"/.test(bernsteinMath) && /id="polya-type-coefficient-relaxation"/.test(sosMath) && /id="worked-full-box-example-two-parameters-degree-three"/.test(sosMath) && /id="sparse-full-box-tensor-windows"/.test(sosMath) && /id="fixed-order-full-box-preordering"/.test(bernsteinMath));
...
check("source-indexed API inventory matches v1.3.0", referenceEntries.length === 181 && inventoryCounts["pdbase-backend"] === 52 && inventoryCounts.pdmat === 55 && inventoryCounts.pdvar === 53 && inventoryCounts.pdlmi === 8 && inventoryCounts["shared-helpers"] === 13);
...
check("peer Math Concepts and nested API sidebars", /label: "Documents"[\s\S]*Reference Lookup[\s\S]*label: "Examples"[\s\S]*label: "About me"[\s\S]*label: "Thanks"[\s\S]*label: "Math Concepts"[\s\S]*label: "Notation"/.test(astroConfig) && [/label: "Math Concepts"[\s\S]*label: "Notation"[\s\S]*label: "Modeling And Analysis"[\s\S]*DPD-LMI And LPV L2-Gain Model[\s\S]*Rate-Box Reduction/, /label: "Coordinates And Bernstein"[\s\S]*Gridding And Local Coordinates[\s\S]*Coefficient Algebra/, /label: "Finite Certificates"[\s\S]*Certificate Map And Selection Guide[\s\S]*Direct And Pólya[\s\S]*Markov–Lukács And Putinar[\s\S]*SparsePutinar Tensor Windows[\s\S]*SparseFullBox And FullBox/, /label: "pdmat"[\s\S]*label: "Matrix Operations"[\s\S]*documents\/reference\/pdmat\/matrix-operations/, /label: "pdvar"[\s\S]*label: "Matrix Operations"[\s\S]*documents\/reference\/pdvar\/matrix-operations/, /label: "pdlmi"[\s\S]*label: "Certificates"[\s\S]*label: "Pólya"[\s\S]*label: "Putinar"[\s\S]*label: "SparsePutinar"[\s\S]*label: "SparseFullBox"[\s\S]*label: "FullBox"/].every((pattern) => pattern.test(astroConfig)));
...
check("v1.3.1 current documentation and v1.3.0 latest tagged release", versionData.includes('current: "v1.3.1"') && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 9 && /version:\s*"v1\.3\.1"[\s\S]*current documentation/.test(versionHistory) && /version:\s*"v1\.3\.0"[\s\S]*latest tagged GitHub Release/.test(versionHistory) && /version:\s*"v1\.2\.4"[\s\S]*final v1\.2 documentation snapshot/.test(versionHistory) && /Documentation v1\.3\.1/.test(versionPage) && /v1\.3\.0 remains the latest\s+tagged GitHub Release/.test(versionPage) && install.includes("Latest GitHub Release (v1.3.0)"));
