...
check("closed six-stage hero", /LPV model[\s\S]*cell-local Bernstein[\s\S]*finite certificate[\s\S]*YALMIP \/ SDP solve[\s\S]*solved scheduled[\s\S]*feedback into LPV model/.test(journey) && /journey-return/.test(journey));
...
check("accepted v0.3.6 plots synchronized", plotHash("pdmat-plot-1d.png") === "092EEBE42836070396B6C8C73C779BAD3296AC7E03201062A859AEAFEFB07761" && plotHash("pdmat-plot-2d.png") === "0FB009CC7C5D02657FF08546EB64F166C29A87212C2CCDAB49C5E43F195EDDB1" && /Crossing higher-order|crossing higher-order/.test(pdmatPlot) && /intersect/.test(pdmatPlot));
