...
check("five-node forward-only journey", (journey.match(/<circle cx=/g)?.length ?? 0) === 5 && ["Model the general DPD-LMI", "Reduce rates and grid ", "Pull back and assemble coefficients", "Select a certificate", "Export, solve, recover"].every((label) => journey.includes(label)) && /general continuum DPD-LMI/.test(journey) && /#general-dpd-lmi-template/.test(journey) && /math: "\\\\boldsymbol\\\\rho"/.test(journey) && /marker-end: url\(#journey-arrow\)/.test(journey) && !/feedback|journey-return|solved matrix|LPV induced-L2-gain model/.test(journey));
...
check("five welcome stages and required mathematics", stepCount === 5 && ["Model the general DPD-LMI problem", "Reduce the rate box and grid ", "titleMath: \"\\\\boldsymbol\\\\rho\"", "Pull back, differentiate, and assemble coefficients", "Select a finite certificate", "Export, solve, and recover the decision matrix"].every((label) => home.includes(label)) && ["Y_1(\\\\boldsymbol\\\\rho),\\\\ldots,Y_N(\\\\boldsymbol\\\\rho)", "\\\\mathcal F_a", "\\\\mathcal P\\\\times\\\\mathcal R", "S_a=-\\\\mathcal F_a", "\\\\mathcal H_{\\\\mathbf c}", "\\\\boldsymbol\\\\alpha=\\\\phi_{\\\\mathbf c}", "S^{(\\\\mathbf c)}", "toYalmip", "optimize", "diagnostics.problem == 0", "value(P)"].every((fragment) => home.includes(fragment)));
check("welcome states the canonical general DPD-LMI", /textHtml: `Choose continuous piecewise-polynomial decision matrices/.test(home) && /renderInlineMath\("Y_1,\\\\ldots,Y_N"\)/.test(home) && /negative semidefinite/.test(home) && /renderInlineMath\("S_a=-\\\\mathcal F_a\\\\succeq0"\)/.test(home) && /\\\\forall\(\\\\boldsymbol\\\\rho,\\\\boldsymbol\\\\nu\)\\\\in\\\\mathcal P\\\\times\\\\mathcal R,\\\\quad a=1,\\\\ldots,q/.test(home) && /optional bounded rate box/.test(home) && /\\\\mathcal R\\\\text\{ is included only for rate-dependent models\}/.test(home) && /#general-dpd-lmi-template/.test(home));
...
check("welcome begins from the general DPD-LMI template", /general symmetric residual inequalities/.test(home) && /From a continuum DPD-LMI to YALMIP and recovered decisions/.test(home) && !/Model an LPV induced-L2-gain problem|x\(t\) is the plant state|\\\\lVert z\\\\rVert_2<\\\\gamma\\\\lVert w\\\\rVert_2/.test(home));
