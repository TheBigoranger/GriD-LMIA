...
check("canonical certificate names and symbols", !/Sparse Full Box|Full Box/.test(allSource) && /sparse full-box tensor-window certificate/.test(sparseFullboxMath) && /full-box preordering/.test(sparseFullboxMath) && !/F\^\{\(\\\\mathbf c,v\)\}|\\\\mathcal I_\{m\+p\}|applyPolya\(p\)/.test(certData + certificateFlow + certificateFlowTabs + sosMath + directPolyaMath));
check("source-indexed API inventory remains 169", referenceEntries.length === 169 && inventoryCounts["pdbase-backend"] === 50 && inventoryCounts.pdmat === 52 && inventoryCounts.pdvar === 54 && inventoryCounts.pdlmi === 7 && inventoryCounts["shared-helpers"] === 6);
...
check("v1.1.1 documentation and v1.1.0 Release metadata", /current:\s*"v1\.1\.1"/.test(versionData) && /Documentation v1\.1\.1/.test(pdlmiOverview) && (versionHistory.match(/version:\s*"v/g)?.length ?? 0) === 7 && ["v0.1.0", "v0.2.12", "v0.3.6", "v0.4.7", "v1.0.0", "v1.1.0", "v1.1.1"].every((version) => versionHistory.includes(`version: "${version}"`)) && /version:\s*"v1\.1\.1"[\s\S]*status:\s*"current documentation snapshot"/.test(versionHistory) && /version:\s*"v1\.1\.0"[\s\S]*status:\s*"latest GitHub Release"/.test(versionHistory) && /No v1\.1\.1 tag or GitHub Release exists/.test(versionPage) && /Latest GitHub Release \(v1\.1\.0\)/.test(install) && /v1\.1\.0 release gate completed 280 of 280 tests/.test(install) && /version:\s*"v1\.0\.0"[\s\S]*date:\s*"2026-07-19"[\s\S]*6d19619/.test(versionHistory));
