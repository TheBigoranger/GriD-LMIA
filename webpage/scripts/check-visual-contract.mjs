...
check(
  "constructor separates global, cell-local, and coefficient-level Bernstein expressions",
  /## Bernstein Basis Expression/.test(pdmatCtor) &&
    ...
    /globalLhs:\s*renderMath\(`A\(\\\\boldsymbol\\\\rho\)=`\)/.test(cellBasisWrapper) &&
    ...
    /globalDomain:\s*renderMath\(`\\\\boldsymbol\\\\rho\\\\in\[0,1\]\^2\.`\)/.test(cellBasisWrapper) &&
    /representationLhs:\s*renderMath\(`A\^\{\(\$\{cell\.c1\},1\)\}[\s\S]*=`\)/.test(cellBasisWrapper) &&
    /representationSum:\s*renderMath\(`\\\\sum_\{\\\\mathbf i/.test(cellBasisWrapper) &&
    /basisDefinition:\s*renderMath\(`B_\{\\\\mathbf i\}\^\{2\}[\s\S]*B_\{i_1\}\^\{2\}/.test(cellBasisWrapper) &&
...
check("pdmat elevate preserves both bases and direct normalized convolution", /## What degree elevation does/.test(pdmatElevate) && /B_\{\\boldsymbol i\}\^\{\\boldsymbol m\}[\s\S]*B_\{\\boldsymbol k\}\^\{\\boldsymbol M\}/.test(pdmatElevate) && /<div className="elevate-direct-coefficient-scroll">[\s\S]*\\binom\{m_s\}\{i_s\}\\binom\{M_s-m_s\}\{k_s-i_s\}[\s\S]*\\binom\{M_s\}\{k_s\}[\s\S]*<\/div>/.test(pdmatElevate) && /Let \$\\boldsymbol d:=\\boldsymbol M-\\boldsymbol m\$/.test(pdmatElevate) && /\\mathcal E_\{\\boldsymbol d\}\[\\boldsymbol j\]:=\\prod_\{s=1\}\^\{\\ell\}\\binom\{d_s\}\{j_s\},\\qquad\\boldsymbol j\\in\\prod_s\\\{0,\\ldots,d_s\\\}/.test(pdmatElevate) && (pdmatElevate.match(/\\mathcal E_\{\\boldsymbol d\}\[\\boldsymbol j\]/g)?.length ?? 0) === 2 && /\\ast_\{\\ell\}[\s\S]*\\mathcal E_\{\\boldsymbol d\}\[\\boldsymbol j\]/.test(pdmatElevate) && /\\prod_s\\\{0,\\ldots,m_s\\\}/.test(pdmatElevate) && /\\prod_s\\\{0,\\ldots,M_s\\\}/.test(pdmatElevate) && /\\left\\\{\\left\(\\prod[\s\S]*\\right\\\}_\{\\boldsymbol k\}=[\s\S]*\\right\\\}_\{\\boldsymbol i\}\\ast_\{\\ell\}\\left\\\{\\mathcal E/.test(pdmatElevate) && !/\\mathcal E_\{\\boldsymbol M-\\boldsymbol m\}|\\mathcal I_\{\\boldsymbol [mM]\}|\\mathcal U|w_\{\\boldsymbol i,\\boldsymbol k\}|\\widehat C|\\widetilde a/.test(pdmatElevate) && /### One-dimensional coefficient rule and example[\s\S]*\\tilde a_i[\s\S]*A = pdmat/.test(pdmatElevate) && /Implementation remark: copy, do not reconstruct/.test(pdmatElevate) && /out = obj[\s\S]*out\.Degree[\s\S]*out\.LocalValues[\s\S]*FunctionHandle[\s\S]*rate-row ordering[\s\S]*YALMIP variables/.test(pdmatElevate) && pdmatElevate.includes("`ncoeff(A)` increases") && /not grid refinement, interpolation, or an approximation step/.test(pdmatElevate));
check("pdvar mirrors pdmat mathematics with symbolic coefficient payloads", /\\mathcal I_\{\\boldsymbol m\}[\s\S]*\\prod_\{s=1\}\^\{\\ell\}\\\{0,\\ldots,m_s\\\}/.test(pdvarCtor) && /P\^\{\(\\boldsymbol c\)\}\(\\boldsymbol\\rho\)[\s\S]*\\sum_\{\\boldsymbol i\\in\\mathcal I_\{\\boldsymbol m\}\}[\s\S]*X\^\{\(\\boldsymbol c\)\}\[\\boldsymbol i\][\s\S]*B_\{\\boldsymbol i\}\^\{\\boldsymbol m\}/.test(pdvarCtor) && /\\binom\{m_s\}\{i_s\}[\s\S]*\\alpha_s\^\{i_s\}[\s\S]*\(1-\\alpha_s\)\^\{m_s-i_s\}/.test(pdvarCtor) && /pdmat Constructor/.test(pdvarCtor) && /\\operatorname\{Aff\}\(\\texttt\{sdpvar\}\)/.test(pdvarStorage) && /\\boldsymbol M\\ge\\boldsymbol m[\s\S]*\\omega_s\(i_s,k_s\)[\s\S]*\\binom\{m_s\}\{i_s\}[\s\S]*\\binom\{M_s-m_s\}\{k_s-i_s\}[\s\S]*\\binom\{M_s\}\{k_s\}[\s\S]*\\Omega\(\\boldsymbol i,\\boldsymbol k\)[\s\S]*\\hat X\^\{\(\\boldsymbol c\)\}\[\\boldsymbol k\][\s\S]*\\sum_\{\\boldsymbol i\\in\\mathcal I_\{\\boldsymbol m\}\}/.test(pdvarStorage) && !/\\widehat X/.test(pdvarStorage) && /id="pdvar-evaluate"[\s\S]*\\mathcal I_\{\\boldsymbol m\}[\s\S]*\\prod_\{s=1\}\^\{\\ell\}\\\{0,\\ldots,m_s\\\}[\s\S]*P\^\{\(\\boldsymbol c\)\}\(\\boldsymbol\\rho\)[\s\S]*\\sum_\{\\boldsymbol i\\in\\mathcal I_\{\\boldsymbol m\}\}[\s\S]*B_\{\\boldsymbol i\}\^\{\\boldsymbol m\}/.test(pdvarStorage) && /pdmat` elevation formula/.test(pdvarStorage) && /pdmat evaluate/.test(pdvarStorage) && !/client:visible|interactive-figure/.test(pdvarCtor + pdvarStorage));
...
check("welcome storage uses build-time formula markup wrappers", /import \{ InlineMath \} from "\.\/RenderedMath\.tsx"/.test(cellExplorer) && /<InlineMath[\s\S]*markup=/.test(cellExplorer) && !/\btex=|renderMath|renderToString|dangerouslySetInnerHTML/.test(cellExplorer) && /basis:\s*renderMath\("B_\{\\\\mathbf i\}\^\{m\}\(\\\\boldsymbol\\\\alpha\)=\\\\prod/.test(home) && /bernstein:\s*renderMath\(`A\^\{\(\$\{cell\.c1\},1\)\}/.test(home) && /storageMath: "A\(\\\\rho_1,\\\\rho_2\)=\\\\begin\{bmatrix\}1\+\\\\rho_1\+\\\\rho_2/.test(home) && !/a_\{11\}|a_\{12\}|a_\{22\}/.test(thirdHomeStep));
...
check("one page-contained local scroller is reserved for the indivisible elevation coefficient formula", (pdmatElevate.match(/className="elevate-direct-coefficient-scroll"/g)?.length ?? 0) === 1 && /\.elevate-direct-coefficient-scroll\s*\{[^}]*max-width:\s*100%[^}]*overflow-x:\s*auto[^}]*\}/.test(manualCss) && /closest\("\.elevate-direct-coefficient-scroll"\)/.test(geometryCheck) && /\["auto",\s*"scroll"\]\.includes\(localScrollerStyle\.overflowX\)/.test(geometryCheck) && /localScroller\.scrollWidth\s*>\s*localScroller\.clientWidth/.test(geometryCheck) && /innerWidth\s*<=\s*320\s*&&\s*!localScrollActive/.test(geometryCheck) && /innerWidth\s*>=\s*390\s*&&\s*localScrollActive/.test(geometryCheck) && /local-formula-scroll-bounds/.test(geometryCheck) && /local-formula-scroll-required/.test(geometryCheck) && /local-formula-scroll-unneeded/.test(geometryCheck) && !/\.formula-display\s*\{[^}]*overflow-x:\s*(?:auto|scroll)/.test(mathStyleSources));
