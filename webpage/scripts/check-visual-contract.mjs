...
check("four-node wave and solved-matrix feedback topology", /<svg class="journey-desktop" viewBox="0 0 1120 390"/.test(journey) && (journey.match(/<g class="journey-node">/g)?.length ?? 0) === 4 && /LPV model[\s\S]*Bernstein data[\s\S]*finite certificate[\s\S]*YALMIP[\s\S]*journey-arrow[\s\S]*solved matrix[\s\S]*journey-return/.test(journey) && /marker-end: url\(#journey-arrow\)/.test(journey));
check("accessible responsive journey SVG", /aria-labelledby="journey-title"[\s\S]*aria-describedby="journey-desc"/.test(journey) && /journey-desktop[\s\S]*width: 100%[\s\S]*height: auto/.test(journey) && /journey-desktop[\s\S]*aria-hidden="true"/.test(journey));
check("separate vertical mobile journey", /class="journey-mobile" aria-hidden="true"/.test(journey) && /@media \(max-width: 700px\)[\s\S]*journey-desktop \{ display: none[\s\S]*journey-mobile \{ display: grid[\s\S]*journey-mobile ol \{ display: grid/.test(journey));
...
check("cell-indexed piecewise derivative", ["D^{(c)}_{+}(\\\\rho)=\\\\begin{cases}", "5.6(1-\\\\alpha_1)-3.2\\\\alpha_1", "c=1", "\\\\alpha_1=2\\\\rho", "-6.4(1-\\\\alpha_2)+8\\\\alpha_2", "c=2", "\\\\alpha_2=2\\\\rho-1", "D^{(c)}_{-}(\\\\rho)=-D^{(c)}_{+}(\\\\rho)"].every((fragment) => home.includes(fragment)) && /ρ ∈ \[0, 0\.5\][\s\S]*ρ ∈ \[0\.5, 1\]/.test(home));
check("stacked mobile display math", home.includes('number: "04"') && home.includes("\\\\begin{cases}") && home.includes('math: "E(\\\\alpha)\\\\succeq0"') && /item\.formula\.map\(renderDisplayMath\)/.test(chooserWrapper) && /option\.mathHtml\.map/.test(chooser) && /key:\s*"putinar"[\s\S]*?formula:\s*\[/.test(certData) && !/key:\s*"putinar"[^\n]*\\qquad/.test(certData));
...
check("accepted v0.4.1 plots synchronized", plotHash("pdmat-plot-1d.png") === "904626761709877ABE7C645BC7A68CFE63B6FF361390C45175718845927152EC" && plotHash("pdmat-plot-2d.png") === "76A290B76AEFD5D771A682460213DFA669B3F4B6BBF38DF4BC7F89CDDED54BDF" && plotHash("pdmat-plot-3d-slice.png") === "2B0B89DB44F65AF6F787C8BCE5971E59412274F6FB2F73FED1822DDDA6B79FF9" && ["two-by-two three-parameter", "FaceAlpha=0.55", "eastoutside", "fixedThirdParameter", "    -2", "handleCount", "     4"].every((fragment) => pdmatPlot.includes(fragment)));
...
check("v0.4.1 public metadata and full history", /current:\s*"v0\.4\.1"/.test(versionData) && /Version 0\.4\.1/.test(pdlmiOverview) && ["v0.1.0", "v0.2.0", "v0.3.0", "v0.3.1", "v0.3.2", "v0.3.3", "v0.3.4", "v0.3.5", "v0.4.0", "v0.4.1"].every((version) => versionHistory.includes(`version: "${version}"`)) && ["63c06ee", "b1e0d51", "7fc820f", "aa71a2a", "eb5ac24", "d896146", "2d420cc", "75d70dc", "9b8a34c"].every((commit) => versionHistory.includes(commit)) && /version:\s*"v0\.4\.1"[\s\S]*date:\s*"2026-07-16"/.test(versionHistory));
