...
if (documentationRecords.length !== 197) failures.push(`Expected 197 public API records, found ${documentationRecords.length}.`);
if (referenceEntries.length !== 197) failures.push(`Expected 197 reference entries, found ${referenceEntries.length}.`);
...
if (declaredWebTargets.length !== 394 || declaredTargetOwners.size !== 394) {
  failures.push(
    `Expected 394 unique API and example targets, found ${declaredTargetOwners.size}: ${duplicateDeclaredTargets.join(" | ")}`,
  );
}
...
console.log("Documentation audit passed: 197 public API records, 8 governed terms, semantic vectors, canonical definitions, contextual boundary prose, and style checks.");
