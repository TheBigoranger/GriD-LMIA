...
test("enumerates tensor and rate rows with the last axis fastest", () => {
  ...
  assert.deepEqual(parseRateBounds("3 3", 1).vertices, [[3]]);
  assert.deepEqual(parseRateBounds("1 1; -3 5", 2).vertices, [[1, -3], [1, 5]]);
});
...
test("matches sparse full-box sliding-window shapes and canonical endpoints", () => {
...
test("matches SparsePutinar defaults, intermediate windows, and endpoints", () => {
  const scalar = { selector: "sparseputinar" as const, cells: 1, nPar: 1,
    degree: 4, rateRows: 1, order: 2, matrixSize: 1, mode: "semidefinite" as const };
  const sparse = buildCertificateShape(scalar);
  assert.deepEqual(
    [sparse.effectiveSelector, sparse.cliqueSize, sparse.psdBlocks, sparse.coefficientIdentities, sparse.totalConstraints],
    ["sparseputinar", 2, 3, 5, 8],
  );
  assert.deepEqual(sparse.blocks.map((block) => block.dimension), [2, 2, 2]);

  const direct = buildCertificateShape({ ...scalar, cliqueSize: 1 });
  assert.deepEqual([direct.effectiveSelector, direct.coefficientTests, direct.psdBlocks], ["direct", 5, 0]);
  const dense = buildCertificateShape({ ...scalar, cliqueSize: 3 });
  assert.deepEqual([dense.effectiveSelector, dense.psdBlocks, dense.coefficientIdentities], ["putinar", 2, 5]);
  assert.deepEqual(dense.blocks.map((block) => block.dimension), [3, 2]);

  const odd = buildCertificateShape({ ...scalar, degree: 3, cliqueSize: 2 });
  assert.deepEqual([odd.psdBlocks, odd.coefficientIdentities], [4, 6]);
  assert.deepEqual(odd.blocks.map((block) => block.dimension), [2, 2, 2, 2]);
});

test("matches multidimensional and anisotropic SparsePutinar windows", () => {
  const scalar = buildCertificateShape({ selector: "sparseputinar", cells: 1, nPar: 2,
    degree: 4, rateRows: 1, order: 2, cliqueSize: 1, matrixSize: 1,
    mode: "semidefinite" });
  assert.deepEqual(
    [scalar.effectiveSelector, scalar.psdBlocks, scalar.coefficientIdentities],
    ["sparseputinar", 21, 25],
  );
  assert.ok(scalar.blocks.every((block) => block.dimension === 1));

  const width2 = buildCertificateShape({ selector: "sparseputinar", cells: 1, nPar: 2,
    degree: 4, rateRows: 1, order: 2, cliqueSize: 2, matrixSize: 1,
    mode: "semidefinite" });
  assert.deepEqual([width2.psdBlocks, width2.coefficientIdentities], [8, 25]);
  assert.ok(width2.blocks.every((block) => block.dimension === 4));

  const anisotropic = buildCertificateShape({ selector: "sparseputinar", cells: 1, nPar: 2,
    degree: [2, 6], rateRows: 1, order: [1, 3], cliqueSize: 2, matrixSize: 1,
    mode: "semidefinite" });
  assert.deepEqual(anisotropic.targetDegree, [2, 6]);
  assert.deepEqual(anisotropic.blocks.map((block) => block.dimension), [4, 4, 4, 2, 2, 2, 4, 4]);

  const zeroAxis = buildCertificateShape({ selector: "sparseputinar", cells: 1, nPar: 2,
    degree: [0, 4], rateRows: 1, order: [0, 2], cliqueSize: 2, matrixSize: 1,
    mode: "semidefinite" });
  assert.deepEqual(
    [zeroAxis.targetDegree, zeroAxis.psdBlocks, zeroAxis.coefficientIdentities, zeroAxis.totalConstraints],
    [[0, 4], 3, 5, 8],
  );
  assert.deepEqual(zeroAxis.blocks.map((block) => block.dimension), [2, 2, 2]);
  assert.ok(zeroAxis.blocks.every((block) => block.gramDegree[0] === 0));
});

test("counts independent SparsePutinar copies and rejects bad clique inputs", () => {
  const entrywise = buildCertificateShape({ selector: "sparseputinar", cells: 2, nPar: 2,
    degree: 4, rateRows: 4, order: 2, cliqueSize: 2, matrixSize: 3,
    mode: "elementwise" });
  assert.equal(entrywise.copies, 72);
  assert.equal(entrywise.psdBlocks, 576);
  assert.equal(entrywise.coefficientIdentities, 1800);
  assert.ok(entrywise.blocks.every((block) => block.dimension === 4));

  assert.throws(() => buildCertificateShape({ selector: "sparseputinar", cells: 1, nPar: 1,
    degree: 4, rateRows: 1, order: 2, cliqueSize: 0, matrixSize: 1,
    mode: "semidefinite" }), /CliqueSize b.*1 to 9/i);
  assert.throws(() => buildCertificateShape({ selector: "sparseputinar", cells: 1, nPar: 2,
    degree: 4, rateRows: 1, order: 1, cliqueSize: 2, matrixSize: 1,
...
test("counts independent elementwise certificates per matrix entry", () => {
  ...
  assert.equal(buildCertificateShape({ selector: "direct", cells: 1, nPar: 2, degree: 1,
    rateRows: 2, order: 0, matrixSize: 1, mode: "semidefinite" }).copies, 2);
  assert.throws(() => buildCertificateShape({ selector: "direct", cells: 1, nPar: 2, degree: 1,
    rateRows: 5, order: 0, matrixSize: 1, mode: "semidefinite" }), /between 1 and 4 distinct rhodiff vertices/i);
});
