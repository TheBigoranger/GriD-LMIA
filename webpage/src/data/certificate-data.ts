...
export const certificateSources: CertificateSource[] = [
  { key: "direct", anchor: "direct", label: "Direct", description: "The default tests every sign-normalized residual coefficient in every physical cell and stored rate row.", command: "L", exportCommand: "C = L.toYalmip();", constraintCount: "Coefficient count follows the assembled residual degree.", boundaryNote: "The finite test is sufficient on the parameter cell. Exact rate-box vertex reduction has already occurred.", formula: ["C^{(\\vect c)}[\\vect i]\\succeq0"], cardFormula: "C^{(\\vect c)}[\\vect i]\\succeq0,\\quad\\vect i\\in\\prod_{s=1}^{\\ell}\\{0,\\ldots,M_s\\}", detailRoute: "documents/math/finite-certificates/direct-and-polya/" },
  { key: "polya", anchor: "polya", label: "Pólya", description: "Pólya elevation changes the Bernstein representation degree while preserving the residual polynomial.", command: "L.usePolya(d)", exportCommand: "C = L.usePolya(d).toYalmip();", constraintCount: "Coefficient count depends on the assembled degree and elevation increment.", boundaryNote: "A failed fixed increment is inconclusive for the continuous inequality.", formula: ["\\tilde C^{(\\vect c)}[\\vect i]\\succeq0"], cardFormula: "\\tilde S^{(\\vect c)}(\\vect\\alpha)=S^{(\\vect c)}(\\vect\\alpha)\\prod_{s=1}^{\\ell}[\\alpha_s+(1-\\alpha_s)]^{d_s}", detailRoute: "documents/math/finite-certificates/direct-and-polya/" },
  {
    key: "putinar",
    anchor: "putinar",
    label: "Putinar",
    ...
    formula: [
      ...
      "\\ell\\ge2:\\;S=S_0+\\sum_{s=1}^{\ell} g_sS_s,\\quad \\vect r_{\\min}=\\lceil \\vect M/2\\rceil",
  ...
  },
  {
    ...
    formula: [
      "w_s=\\min(b,d_s+1),\\quad \\dim Q_{\\vect u}=n\\prod_{s=1}^{\ell} w_s",
      "N_{\\mathrm{win}}=\\prod_{s=1}^{\ell}(d_s-w_s+2)",
      "C[\\vect k]=\\sum_{q\\in\\mathcal Q_{\\mathrm P}}\\sum_{\\vect u}\\mathcal A_{q,\\vect u}(Q_{q,\\vect u})[\\vect k]",
    ],
    cardFormula: "S^{(\\vect c)}=\\sum_{q\\in\\mathcal Q_{\\mathrm P}}g_q\\sum_{\\vect u}Z_{q,\\vect u}^\\top Q_{q,\\vect u}Z_{q,\\vect u},\\quad Q_{q,\\vect u}\\succeq0",
  ...
  },
  {
    ...
    boundaryNote: "Every physical cell, stored rate row, and column-major matrix entry receives an independent certificate. Residual boundary handles remain shared.",
    formula: [
      "w=1:\\;\\text{Direct}",
      "1<w<\\max_{s=1}^{\\ell}(r_s+1):\\;Q_{J,\\vect u}\\succeq0,\\;\\text{sliding tensor windows}",
      "w\\ge\\max_{s=1}^{\\ell}(r_s+1):\\;\\text{FullBox}",
    ],
    cardFormula: "S^{(\\vect c)}(\\vect\\alpha)=\\sum_{J\\subseteq[\\ell]}g_J(\\vect\\alpha)\\sum_{\\vect u}Z_{J,\\vect u}^\\top Q_{J,\\vect u}Z_{J,\\vect u},\\quad Q_{J,\\vect u}\\succeq0",
    detailRoute: "documents/math/finite-certificates/sparsefullbox-and-fullbox/",
  },
  { key: "fullbox", anchor: "full-box", label: "FullBox", description: "FullBox uses every square-free product of the box generators. In one parameter, it coincides with the Markov–Lukács form.", command: "L.useFullBox()", exportCommand: "C = L.useFullBox().toYalmip();", constraintCount: "Constraint count depends on dimension, assembled degree, and absolute order.", boundaryNote: "Putinar and FullBox coincide in one parameter, while their multivariate generator families differ.", formula: ["\\ell=1:\\;S=S_0+\\alpha(1-\\alpha)S_1"], cardFormula: "S^{(\\vect c)}(\\vect\\alpha)=\\sum_{J\\subseteq[\\ell]}g_J(\\vect\\alpha)Z_J(\\vect\\alpha)^\\top Q_JZ_J(\\vect\\alpha),\\quad Q_J\\succeq0", detailRoute: "documents/math/finite-certificates/sparsefullbox-and-fullbox/" },
];
