...
export const certificateSources: CertificateSource[] = [
  { key: "direct", anchor: "direct", label: "Direct", description: "The default tests all three degree-two coefficients in each of the two physical cells.", command: "L", exportCommand: "C = L.toYalmip();", constraintCount: "6 constraints: 2 cells × 3 local coefficients.", boundaryNote: "The shared residual boundary handle is reused by both cells; no extra continuity constraint is generated.", formula: ["C^{(c)}[i]\\succeq0,\\quad c=1,2,\\;i=0,1,2"], cardFormula: "C_{\\mathbf i}^{(\\mathbf c,v)}\\preceq0,\\quad\\mathbf i\\in\\mathcal I_m", detailRoute: "documents/reference/pdlmi/constructor/" },
  { key: "polya", anchor: "polya", label: "Pólya", description: "Pólya elevation by p raises the Bernstein degree before the direct coefficient test.", command: "L.applyPolya(p)", exportCommand: "C = L.applyPolya(p).toYalmip();", constraintCount: "Coefficient count depends on the elevation p.", boundaryNote: "Degree elevation preserves the shared residual boundary value; it does not create cross-cell certificate variables.", formula: ["\\widetilde C_{\\mathbf i}^{(\\mathbf c,v)}\\preceq0,\\quad\\mathbf i\\in\\mathcal I_{m+p}"], cardFormula: "\\widetilde F^{(\\mathbf c,v)}(\\boldsymbol\\alpha)=F^{(\\mathbf c,v)}(\\boldsymbol\\alpha)\\prod_{s=1}^{\\ell}\\bigl[\\alpha_s+(1-\\alpha_s)\\bigr]^p,\\quad p\\in\\mathbb N_0", detailRoute: "documents/reference/pdlmi/applypolya/" },
  {
    key: "putinar",
    anchor: "putinar",
    label: "Putinar",
