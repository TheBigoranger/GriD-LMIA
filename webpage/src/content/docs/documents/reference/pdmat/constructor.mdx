...
import CellBernsteinExpressionExplorer from "../../../../../components/CellBernsteinExpressionExplorer.tsx";
...
<CellBernsteinExpressionExplorer client:visible />
...
| `RateBounds` | Finite $\ell\times2$ lower/upper scheduling-rate bounds. With ordinary one-row data this is metadata for `rhodiff`; explicit nested leaves may instead contain all $2^\ell$ rate-vertex rows. |
...
Unsupported metadata options: `IsContinuous`, `ContainsDecision`, and
`HasRateDependence`.
...
| `HasRateDependence` | `A.HasRateDependence` | `true` when rate metadata or explicit rate rows are present. |
...
Each physical cell must use either one ordinary row or every rate-box vertex
row, uniformly. Vertex order is
