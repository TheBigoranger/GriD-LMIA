...
| `Degree` | One finite nonnegative integer scalar shorthand or an `ell`-element direction-wise vector, such as `Degree=[1 3]`. The accepted value is stored as a `1 × ell` row. Required when a function handle should be validated as Bernstein coefficient evidence. |
| `RateBounds` | Finite `ell`-by-2 lower/upper scheduling-rate bounds. With ordinary one-row data this is metadata for `rhodiff`; explicit nested leaves may instead contain all `2^ell` rate-vertex rows. |
...
| `Degree` | `A.Degree` | Direction-wise local Bernstein degree stored as a `1 × ell` row. |
