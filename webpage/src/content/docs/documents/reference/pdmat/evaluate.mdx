...
export const evalSeries = [{ label: "A(ρ)", path: "M58 210 C250 190 475 120 680 62", tone: "first" }];
export const evalMarker = { x: 369, y: 151, label: "A(0.5)" };
...
<ParameterCurveDiagram series={evalSeries} marker={evalMarker} label="A parameter-dependent entry with an evaluation marker at rho one half" caption="evaluate reads the represented function at one physical parameter point; it does not expose a nearest stored label." />
