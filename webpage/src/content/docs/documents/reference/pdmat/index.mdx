...
<nav class="manual-trail"><a href="/GriD-LMIA/documents/">Documents</a><span>/</span><a href="/GriD-LMIA/documents/reference-index/">Reference</a><span>/</span><span>pdmat</span></nav>
...
| Reference | Task |
| :--- | :--- |
| [`Storage`](/GriD-LMIA/documents/reference/pdmat/storage-and-elevation/) | Traverse known coefficient evidence and inspect its cell-wise labels and counts. |
| [`elevate`](/GriD-LMIA/documents/reference/pdmat/elevate/) | Exactly elevate the Bernstein representation while preserving the represented function. |
...
- Function-only `pdmat` objects that omit explicit Bernstein evidence evaluate
  exactly and remain in function-evaluation workflows. Coefficient-backed
  objects support coefficient algebra, differentiation, tables, and inequalities.
...
- Plotting and `bernTable` output are diagnostics. Solver-facing behavior begins with comparison assembly.
