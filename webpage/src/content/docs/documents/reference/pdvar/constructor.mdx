...
| `Degree` | One finite nonnegative integer scalar shorthand or an `ell`-element direction-wise vector. The default is `ones(1,ell)`; use an all-zero row for a parameter-independent decision. The accepted value is stored as a `1 × ell` row. |
...
| `Degree` | `P.Degree` | Constructor-created objects accept every nonnegative direction-wise integer degree; algebra can also raise components. Stored as a `1 × ell` row. |
