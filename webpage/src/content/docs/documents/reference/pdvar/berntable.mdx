...
by [`rhodiff`](/GriD-LMIA/documents/reference/pdvar/rhodiff/). The view
preserves the decision expression, and solver handoff remains a separate step.
...
T = bernTable(P, cellSubscript)
T = bernTable(P, cellSubscript, "oneLine")
T = P.bernTable(...)
```
...
| :--- | :--- |
| `P` | A `pdvar` object. |
| `cellSubscript` | Nonempty numeric or cell-array matrix with one finite-real integer physical-cell subscript per row and one column per parameter. Cell-array entries are numeric scalars. Omit it to inspect every cell. |
...
snapshot of the object state. Editing the table preserves `P`.
...
These blocks reproduce one clean MATLAB session in its original command-window
format. The `internal(4)`, `internal(5)`, `internal(7)`, and
`internal(8)` identifiers are session-local YALMIP names. The stable contract
is the four-slot storage order and its table metadata, independently of those
identifier numbers.

<DiagramFrame caption="The four LocalValues slots and the four table rows use the same last-axis-fastest tensor order." label="LocalValues slots compared with bernTable rows">
...
<span id="rate-vertex-metadata-without-unstable-symbolic-names"></span>

### Stable Rate-Vertex Metadata
...
label. Selecting metadata columns makes the ordering reproducible while
...
- The table helper is diagnostic. Product and relaxation support follows the
  owning APIs.

`pdvar` accepts any nonnegative constructor degree, and products may elevate
coefficient degree further. `bernTable` reports the resulting stored rows.
It is an inspection aid. Serialization uses the documented object state and
...
yalmip('clear')
P1 = pdvar(1,[0 1 3]);
P2 = pdvar(1,{[0 1 3],[-2 0 4]},Degree=[1 1]);
T1 = bernTable(P1, [2;1;2], "oneLine");
T2 = bernTable(P2, [2 1;1 2;2 1]);
T3 = bernTable(P2, {2,1;1,2;2,1});
...
[`pdmat bernTable`](/GriD-LMIA/documents/reference/pdmat/berntable/)
