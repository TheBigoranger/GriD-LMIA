...
export const residualSeries = [{ label: "R(ρ)=lhs−rhs", path: "M58 104 C230 138 470 174 680 190", tone: "result" }];
...
<ParameterCurveDiagram series={residualSeries} label="A comparison residual traced over the parameter interval" caption="A comparison stores the residual lhs minus rhs. The selected certificate subsequently proves the required matrix sign over every physical cell." />
