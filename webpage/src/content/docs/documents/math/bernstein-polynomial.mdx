...
implemented input and validation boundaries beside the call they affect.
...
On one-dimensional physical cell $c$, with endpoints
$[\rho_1^{(c)},\rho_1^{(c+1)}]$, the package uses
...
- at the left endpoint $\rho=\rho_1^{(c)}$, $\alpha=0$ and local label `0` is selected;
- at the right endpoint $\rho=\rho_1^{(c+1)}$, $\alpha=1$ and local label `m` is selected.

For degree two,

$$
P^{(c)}(\alpha)=(1-\alpha)^2C^{(c)}[0]
...
$$
P^{(c)}(\alpha)=\sum_i B_i^m(\alpha)P^{(c)}[i]
$$

lies in the convex hull of its coefficients. The statement applies to several
payload types:

- scalar: $\min_i P^{(c)}[i]\le P^{(c)}(\alpha)\le\max_iP^{(c)}[i]$;
...
$P^{(c)}(\alpha)\preceq0$ throughout the cell. Apply the scalar convex-combination
argument to $x^{\mathsf T}P^{(c)}(\alpha)x$ for every vector $x$. Strictly negative
...
dimensions, a physical node has multi-index
$\mathbf j=(j_1,\ldots,j_\ell)$ and coordinates
$\boldsymbol\rho^{(\mathbf j)}=(\rho_1^{(j_1)},\ldots,\rho_\ell^{(j_\ell)})$.
A cell has identity $\mathbf c$, and a local Bernstein label is
$\mathbf i=(i_1,\ldots,i_\ell)$. A stored matrix is therefore
$C^{(\mathbf c)}[\mathbf i]$.
...
$$
P^{(c)}(\alpha)=\sum_{i=0}^{m}P^{(c)}[i]B_i^m(\alpha),
$$

the forward coordinate gives the physical derivative on one-dimensional cell
$c$ of width $h_c=\rho_1^{(c+1)}-\rho_1^{(c)}$:
...
$$
\begin{aligned}
P^{(c)}(\alpha)&=\sum_{i=0}^{m}B_i^m(\alpha)P^{(c)}[i],\\
Q^{(c)}(\alpha)&=\sum_{j=0}^{n}B_j^n(\alpha)Q^{(c)}[j].
\end{aligned}
$$
...
With axis node counts $k_1,\ldots,k_\ell$, the number of physical
hypercubes is
...
path is dimension-aware. In one parameter it dispatches to the same
parity-specific Markov–Lukács form as the full-box selector, with minimum and
default order $\lfloor m/2\rfloor$. In two or more parameters it replaces
coefficient-wise sign tests with an unweighted Bernstein-Gram term plus one
$\alpha_s(1-\alpha_s)$-weighted term per axis. At absolute order
$r\ge\lceil m/2\rceil$, those terms use tensor basis degrees $r\mathbf1$ and
$r\mathbf1-\mathbf e_s$, respectively. Exact coefficient identities are
matched at tensor degree $2r\mathbf1$ independently for every physical cell
and rate row.

For two or more parameters, this is the implemented Putinar specialization
for the local parameter box. It is not a general-domain or general-generator
interface, does not search orders automatically, and does not insert a
strictness margin or call a solver.
...
This is a fixed-order certificate on the local parameter box. In one parameter
it is the same Markov–Lukács assembly selected by Putinar. In two or more
parameters it is distinct from the Putinar box module because it includes
every generator-subset product. It is not a general-domain SOS interface or an
