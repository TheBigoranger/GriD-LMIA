function c = coeffs(obj, cellSubs)
    ...
    %
    %   Example:
    %     obj = dpbase({[0 1 2]}, [1 1], 1);
    %     c = obj.coeffs(2);

    nCell = obj.GridInfo.NumNodes - 1;
    if iscell(cellSubs)
        helper.chk(cellSubs, "dpbase:InvalidCellSubs", ...
    ...
    % storage contract shared by future dpmat/dpvar subclasses.
    c = obj.LocalValues;
    for k = 1:numel(subs)
        c = c{subs(k)};
    end
