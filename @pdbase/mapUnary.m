function out = mapUnary(obj, fcn, sz)
    %MAPUNARY Map a unary operation over every stored coefficient payload.
    %
    %   Syntax:
    %     out = obj.mapUnary(fcn)
    %     out = obj.mapUnary(fcn, sz)
    %
    %   Arguments:
    %     fcn - Function handle applied to every numeric or affine payload.
    %     sz  - Optional output matrix size. If omitted, one mapped sample is
    %           inspected.
    %
    %   Output:
    %     out - Same dynamic class rebuilt from the mapped coefficient tree.
    %
    %   Example:
    %     out = obj.mapUnary(@transpose, fliplr(obj.MatrixSize));
    %
