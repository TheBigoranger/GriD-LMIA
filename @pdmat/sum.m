function out = sum(obj, varargin)
    %SUM Sum pdmat coefficient payloads along a matrix dimension.
    %
    %   Syntax:
    %     S = sum(A)
    %     S = sum(A, dim)
    ...
    if numel(varargin) ~= 1
        error("pdmat:InvalidSum", "pdmat sum accepts at most one dimension argument.");
    end
    ...
    dim = helper.chk(dim, "pdmat:InvalidSum", ...
