function out = repmat(obj, varargin)
    ...
    %
    %   Example:
    %     A = pdmat({[0 1]}, {[1 2], 2 * [1 2]}, Degree=1);
    %     B = repmat(A, 2, 1);
...
function sz = parseRep(args)
    ...
    elseif numel(args) == 2
        sz = zeros(1, 2);
        for k = 1:2
            sz(k) = helper.chk(args{k}, "pdmat:InvalidRepmat", ...
                "Repetition counts must be positive integer scalars.", ...
                "numeric", "real", "scalar", "finite", "integer", "positive");
        end
        return
    else
        error("pdmat:InvalidRepmat", ...
            "pdmat repmat expects a scalar, a two-element size vector, or two size scalars.");
    end

    if numel(sz) ~= 2
        error("pdmat:InvalidRepmat", ...
            "pdmat repmat supports exactly two matrix repetition counts.");
    end
    for k = 1:2
        sz(k) = helper.chk(sz(k), "pdmat:InvalidRepmat", ...
            "Repetition counts must be positive integer scalars.", ...
            "numeric", "real", "scalar", "finite", "integer", "positive");
    end
end
